      ...
      <div class="modal-dialog modal-lg">
        ...
        <div class="modal-body">
          ...
          >
            ...
            请至少勾选2条报价进行对比
            <br />
            <span style="font-size: 12px">最多同时对比3条报价</span>
          </div>
          <div
            v-else-if="selectedIds.length > 3"
            style="text-align: center; padding: 40px; color: var(--color-text-tertiary)"
          >
...
<script>
export default { name: 'QuotationComparisonModal' }
</script>
<script setup>
...
const comparisonRows = [
  ...
  { label: '不含税金额', fn: (q) => '¥' + formatNumber(q.subtotal || 0) },
  {
    ...
    fn: (q) => {
      ...
      if (q.total === maxTotal && maxTotal !== minTotal)
        return '<span style="color:var(--color-danger)">' + val + ' <Icon name="chevronUp" :size="14" /></span>'
      if (q.total === minTotal && maxTotal !== minTotal)
        return '<span style="color:var(--color-success)">' + val + ' <Icon name="chevronDown" :size="14" /></span>'
      return val
    }
  },
  ...
  {
    label: '状态',
    fn: (q) => '<span class="status-badge status-' + q.status + '">' + (statusLabels[q.status] || q.status) + '</span>'
  },
