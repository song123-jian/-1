<template>
  <div class="panel-card">
    <div class="panel-card-header" style="justify-content: space-between">
      ...
      <div class="cal-nav">
        <button class="dp-nav-btn" @click="custWeekPrev">
          <Icon name="chevronLeft" :size="14" />
          上一周
        </button>
        <button class="dp-today-btn" @click="custWeekGoToday">本周</button>
        <button class="dp-nav-btn" @click="custWeekNext">
          下一周
    ...
    <div class="panel-card-body">
      <div class="cust-week-grid">
        <div v-for="day in custWeekDays" :key="day.date" class="cust-week-col" :class="{ today: day.isToday }">
...
<script>
export default { name: 'CustomerWeekView' }
</script>
<script setup>
...
function custWeekPrev() {
  custWeekOffset.value--
}
function custWeekNext() {
  custWeekOffset.value++
}
function custWeekGoToday() {
  custWeekOffset.value = 0
}
</script>
...
.cust-week-num.is-today {
  color: var(--color-accent);
}
.cust-week-date-label {
  font-size: 10px;
  color: var(--color-text-tertiary);
  margin-top: var(--space-1);
}
