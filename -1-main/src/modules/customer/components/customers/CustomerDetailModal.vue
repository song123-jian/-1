  ...
  <Teleport to="body">
    <div v-if="showDetail" class="modal-overlay" @click.self="emit('close')">
      <div class="modal-dialog modal-xl">
        ...
        <div v-if="currentCustomer" class="modal-body">
          <div class="detail-top-bar">
            <div class="detail-avatar-lg" :style="{ background: levelColors[currentCustomer.level] || '#94a3b8' }">
              {{ (currentCustomer.fullName || currentCustomer.name || '?').charAt(0) }}
            </div>
            <div class="detail-top-info">
              <div class="detail-name">{{ currentCustomer.fullName || currentCustomer.name }}</div>
              <div class="detail-top-badges">
                <span class="level-badge" :class="'level-' + currentCustomer.level">
                  {{ levelLabel(currentCustomer.level) }}
                </span>
                <span class="status-badge" :class="'status-' + currentCustomer.status">
                  {{ currentCustomer.status === 'active' ? '活跃' : '休眠' }}
                </span>
                <span
                  v-for="tagId in currentCustomer.tags || []"
                  :key="tagId"
                  class="mini-tag"
                  :style="_getTagStyle(tagId)"
                >
                  {{ _getTagName(tagId) }}
                </span>
              </div>
              <div class="detail-top-meta">
                {{ currentCustomer.customerNo }} · {{ currentCustomer.region || '未知区域' }} ·
                {{ currentCustomer.contactName || currentCustomer.contact || '未指定联系人' }}
              </div>
            </div>
            <div class="detail-top-kpis">
              ...
              <div class="kpi-card">
                ...
                <div
                  ...
                  :class="
                    creditUtilization > 80 ? 'text-danger' : creditUtilization > 50 ? 'text-warning' : 'text-success'
                  "
                >
                  {{ creditUtilization }}%
                </div>
          ...
          <div v-if="activeDetailTab === 'basic'" class="detail-tab-content">
            <div class="detail-grid-2col">
              <div class="detail-section">
                ...
                <div class="detail-fields">
                  <div class="detail-field">
                    <span class="df-label">客户编号</span>
                    <span class="mono">{{ currentCustomer.customerNo || '-' }}</span>
                  </div>
                  <div class="detail-field">
                    <span class="df-label">联系人</span>
                    <span>{{ currentCustomer.contactName || currentCustomer.contact || '-' }}</span>
                  </div>
                  <div class="detail-field">
                    <span class="df-label">部门</span>
                    <span>{{ currentCustomer.department || '-' }}</span>
                  </div>
                  <div class="detail-field">
                    <span class="df-label">职位</span>
                    <span>{{ currentCustomer.position || '-' }}</span>
                  </div>
                  <div class="detail-field">
                    <span class="df-label">手机</span>
                    <span class="mono">{{ currentCustomer.phone || '-' }}</span>
                  </div>
                  <div class="detail-field">
                    <span class="df-label">邮箱</span>
              ...
              <div class="detail-section">
                ...
                <div class="detail-fields">
                  <div class="detail-field">
                    <span class="df-label">决策权限</span>
                    <span>{{ currentCustomer.decisionAuthority || '-' }}</span>
                  </div>
                  <div class="detail-field">
                    <span class="df-label">核心关注点</span>
                    <span>{{ currentCustomer.coreConcerns || '-' }}</span>
                  </div>
                  <div class="detail-field">
                    <span class="df-label">区域</span>
                    <span>{{ currentCustomer.region || '-' }}</span>
                  </div>
                  <div class="detail-field">
                    <span class="df-label">信用额度</span>
                    <span class="mono">¥{{ formatNumber(currentCustomer.creditLimit) }}</span>
                  </div>
                  <div class="detail-field">
                    <span class="df-label">创建时间</span>
                    <span>{{ currentCustomer.createdAt || '-' }}</span>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div v-if="activeDetailTab === 'trade'" class="detail-tab-content">
            <div class="trade-summary">
              ...
              <div class="trade-kpi">
                <div
                  class="trade-kpi-value mono"
                  :class="customerTradeStats.avgProfitMargin > 15 ? 'text-success' : 'text-warning'"
                >
                  {{ customerTradeStats.avgProfitMargin }}%
                </div>
              ...
              <div class="trade-kpi">
                <div
                  class="trade-kpi-value mono"
                  :class="
                    customerTradeStats.collectionRate >= 80
                      ? 'text-success'
                      : customerTradeStats.collectionRate >= 50
                        ? 'text-warning'
                        : 'text-danger'
                  "
                >
                  {{ customerTradeStats.collectionRate }}%
                </div>
              ...
              <table v-if="customerQuotations.length > 0" class="detail-table">
                ...
                <tbody>
                  <tr v-for="q in customerQuotations" :key="q.id">
                    ...
                    <td>
                      <span class="status-badge" :class="quoteStatusClass(q.status)">
              ...
              <table v-if="customerContracts.length > 0" class="detail-table">
                ...
                <tbody>
                  <tr v-for="ct in customerContracts" :key="ct.id">
                    ...
                    <td>
                      <span class="status-badge" :class="contractStatusClass(ct.status)">
          ...
          <div v-if="activeDetailTab === 'tags'" class="detail-tab-content">
            <div class="detail-section">
              ...
              <div v-if="(currentCustomer.tags || []).length > 0" class="detail-tags-area">
                <span
                  v-for="tagId in currentCustomer.tags || []"
                  :key="tagId"
                  class="detail-tag"
              ...
              <div v-else class="inline-tag-create">
                ...
                <div style="display: flex; gap: var(--space-2); margin-top: var(--space-2); align-items: center">
                  <div class="color-picker-row">
                    <span
                      v-for="c in inlinePresetColors"
                      :key="c"
                      class="color-dot-sm"
                      :class="{ active: inlineTagForm.color === c }"
                      :style="{ background: c }"
                      @click="inlineTagForm.color = c"
                    ></span>
                  </div>
                  <button
                    class="btn btn-primary btn-sm"
                    :disabled="!inlineTagForm.name.trim()"
            ...
            <div class="detail-section" style="margin-top: var(--space-4)">
              <div style="display: flex; align-items: center; justify-content: space-between">
                ...
                <router-link to="/tag-category" class="btn btn-ghost btn-sm">
                  前往标签分类
          ...
          <div v-if="activeDetailTab === 'timeline'" class="detail-tab-content">
            <div class="timeline">
              <div
                v-for="(evt, idx) in customerTimeline"
                :key="idx"
                class="timeline-item"
                :style="{ animationDelay: idx * 80 + 'ms' }"
              >
                <div class="timeline-dot" :class="evt.dotClass"></div>
                <div v-if="idx < customerTimeline.length - 1" class="timeline-line"></div>
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-outline" @click="emit('open360', currentCustomer)">
...
<script>
export default { name: 'CustomerDetailModal' }
</script>
<script setup>
...
const customerTradeStats = computed(() => {
  ...
  const avgProfitMargin =
    profitMargins.length > 0
      ? Math.round((profitMargins.reduce((s, m) => s + m, 0) / profitMargins.length) * 10) / 10
      : 0
...
const customerTimeline = computed(() => {
  ...
  quotationStore.quotations
    .filter((q) => q.customerId === cid || q.customerName === cname)
    .forEach((q) => {
      events.push({
        date: q.date,
        text: `报价 ${q.quoteNo} · ¥${formatNumber(q.total)} · ${quoteStatusLabel(q.status)}`,
        dotClass: 'dot-quote'
      })
    })

  dataStore.collections
    .filter((col) => col.customerId === cid || col.customerName === cname)
    .forEach((col) => {
      events.push({
        date: col.date,
        text: `回款 ${col.collectionNo} · ¥${formatNumber(col.amount)} · ${collectionMethodLabel(col.method)}`,
        dotClass: 'dot-money'
      })
    })

  dataStore.contracts
    .filter((ct) => ct.customerId === cid || ct.customerName === cname)
    .forEach((ct) => {
      events.push({
        date: ct.startDate,
        text: `签约 ${ct.contractNo} · ¥${formatNumber(ct.amount)} · ${contractStatusLabel(ct.status)}`,
        dotClass: 'dot-contract'
      })
    })
