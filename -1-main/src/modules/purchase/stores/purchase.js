...
export const usePurchaseStore = defineStore('purchase', () => {
  ...
  const pendingCount = computed(() => purchaseOrders.value.filter((o) => o.status === 'pending').length)
  ...
  /* 采购退货单 */
  const returnOrders = computed(() => purchaseOrders.value.filter((o) => o.type === 'return'))
  ...
  function approvePurchaseOrder(id, approver) {
    ...
    if (idx !== -1 && purchaseOrders.value[idx].status === 'pending') {
      ...
      purchaseOrders.value[idx].approveDate = toLocalDateStr()
      persist()
      return true
    }
    return false
  ...
  function completePurchaseOrder(id) {
    ...
    if (idx !== -1 && purchaseOrders.value[idx].status === 'inspecting') {
      ...
      purchaseOrders.value[idx].actualDate = toLocalDateStr()
      persist()
      /* 调用inventoryStore入库 */
      try {
