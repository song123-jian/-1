...
export const useMonthlyStatsStore = defineStore('monthlyStats', () => {
  ...
  const dateRange = computed(() => {
    ...
    if (quickRange.value === 'curr') {
      ...
      return {
        ...
        to: py + '-' + String(pm).padStart(2, '0') + '-31'
      }
    } else if (quickRange.value === '3m') {
      const d = new Date(y, m - 3, 1)
      return { from: toLocalDateStr(d), to: y + '-' + String(m).padStart(2, '0') + '-31' }
  ...
  const kpiData = computed(() => {
    ...
    const pending =
      inOrders.filter((o) => o.status === 'pending' || o.status === 'pending_review').length +
      outOrders.filter((o) => o.status === 'pending' || o.status === 'pending_review' || o.status === 'approved').length
    const confirmed =
      inOrders.filter((o) => o.status === 'confirmed').length + outOrders.filter((o) => o.status === 'confirmed').length
  ...
  const topItems = computed(() => {
    ...
    for (const o of filteredInbound.value) {
      ...
      for (const it of items) {
        ...
        if (!code) continue
        if (!itemMap[code])
          itemMap[code] = { code, name: it.name || code, inWeight: 0, inAmount: 0, outWeight: 0, outAmount: 0 }
    ...
    for (const o of filteredOutbound.value) {
      ...
      if (!code) continue
      if (!itemMap[code])
        itemMap[code] = { code, name: o.materialName || code, inWeight: 0, inAmount: 0, outWeight: 0, outAmount: 0 }
