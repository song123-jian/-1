<template>
  <div>
    ...
    <div v-if="showDictModal" class="modal-overlay" @click.self="showDictModal = false">
      <div class="modal-dialog" @click.stop>
        <div class="modal-header">
          <span class="modal-title">{{ editingDict ? '编辑字典' : '新增字典' }}</span>
    ...
    <div v-if="showEntryModal" class="modal-overlay" @click.self="showEntryModal = false">
      <div class="modal-dialog" @click.stop>
        <div class="modal-header">
          <span class="modal-title">{{ editingDictEntry ? '编辑字典项' : '添加字典项' }}</span>
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <select v-model="entryForm.category" class="form-select" style="width: 100%">
              <option value="">请选择分类</option>
              <option value="客户等级">客户等级</option>
              <option value="业务类型">业务类型</option>
              <option value="仓库">仓库</option>
              <option value="付款方式">付款方式</option>
              <option value="送货状态">送货状态</option>
              <option value="报价状态">报价状态</option>
            </select>
          </div>
...
function deleteDict(id) {
  if (confirm('确认删除该字典？')) sysStore.deleteDictionary(id)
}

function openEntryModal() {
  Object.assign(entryForm, { label: '', value: '', sort: 1, enabled: true })
...
function removeEntry(entryId) {
  sysStore.removeDictEntry(selectedDictId.value, entryId)
}
