    ...
    <div class="filter-bar" style="margin-bottom: var(--space-4); flex-wrap: wrap; gap: var(--space-2)">
      <div style="display: flex; gap: var(--space-2); flex-wrap: wrap; align-items: center">
        <button
          ...
          :key="p.id"
          class="btn"
          :class="activeProposal === p.id ? 'btn-primary' : 'btn-secondary'"
      ...
      <div style="display: flex; gap: var(--space-2); align-items: center; margin-left: auto">
        ...
        >
          <button
            ...
            :class="{ 'active-device': activeDevice === d.key }"
        ...
        >
          ...
          <label style="position: relative; display: inline-block; width: 36px; height: 20px; cursor: pointer">
            <input v-model="realtimePreview" type="checkbox" style="opacity: 0; width: 0; height: 0" />
            <span
              style="
                position: absolute;
                cursor: pointer;
            ...
            <span
              style="
                ...
                transition: 0.3s;
              "
              :style="{ left: realtimePreview ? '18px' : '2px' }"
            ></span>
          </label>
          <span style="font-size: 12px; color: var(--color-text-secondary)">实时预览</span>
        </div>
        <button class="btn btn-secondary btn-sm" title="导出设计规格" @click="exportDesignSpec">
            ...
            <div class="phone-frame" :style="phoneFrameStyle">
              ...
              <div class="phone-content" :style="phoneContentStyle">
                <div style="padding: 16px; height: 100%; overflow-y: auto">
                  <div
                    :style="{
                      background: currentSpec.colors.primary,
                      color: '#fff',
                      padding: '12px 16px',
                      borderRadius: currentSpec.components.borderRadius,
                      marginBottom: '12px',
                      fontWeight: 600
                    }"
                  >
                  ...
                  <div
                    style="background: var(--color-bg-tertiary); border-radius: 8px; padding: 16px; margin-bottom: 12px"
                  >
                    <div :style="{ color: currentSpec.colors.text, fontWeight: 600, marginBottom: '8px' }">
                      客户列表
                    </div>
                    <div
                      :style="{
                        color: currentSpec.colors.text,
                        opacity: 0.7,
                        fontSize: '13px',
                        padding: '8px 0',
                    ...
                    <div :style="{ color: currentSpec.colors.text, opacity: 0.7, fontSize: '13px', padding: '8px 0' }">
                      东莞某某贸易公司
                    </div>
                  </div>
                  <div
                  ...
                  <button
                    :style="{
                      width: '100%',
                      height: currentSpec.components.buttonHeight,
                      background: currentSpec.colors.primary,
                      color: '#fff',
                      border: 'none',
                      borderRadius: currentSpec.components.borderRadius,
              ...
              <div class="phone-nav-bar" :style="{ background: currentSpec.colors.darkMode ? '#1a1a2e' : '#fff' }">
                <div class="phone-nav-item">
                  <span class="phone-nav-icon">[首页]</span>
                  <span class="phone-nav-label" :style="{ color: currentSpec.colors.primary }">首页</span>
                </div>
                <div class="phone-nav-item">
                  <span class="phone-nav-icon"><Icon name="package" :size="14" /></span>
                  <span class="phone-nav-label" :style="{ color: currentSpec.colors.primary }">产品</span>
                </div>
                <div class="phone-nav-item">
                  <span class="phone-nav-icon"><Icon name="list" :size="14" /></span>
                  <span class="phone-nav-label" :style="{ color: currentSpec.colors.primary }">订单</span>
                </div>
                <div class="phone-nav-item">
                  <span class="phone-nav-icon"><Icon name="table" :size="14" /></span>
                  <span class="phone-nav-label" :style="{ color: currentSpec.colors.primary }">报表</span>
                </div>
                <div class="phone-nav-item">
                  <span class="phone-nav-icon"><Icon name="checkCircle" :size="14" /></span>
                  <span class="phone-nav-label" :style="{ color: currentSpec.colors.primary }">审批</span>
                </div>
              </div>
            </div>
          </div>
          ...
          <div class="panel-card-body">
            <div style="margin-bottom: var(--space-3)">
              ...
              <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 6px">
                <div
                  v-for="(color, key) in currentSpec.colors"
                  :key="key"
                ...
                >
                  ...
                  <div style="font-size: 11px">
                    ...
                    <div style="font-family: var(--font-mono); color: var(--color-text-primary)">{{ color }}</div>
                  </div>
                </div>
              </div>
            </div>
            <div style="margin-bottom: var(--space-3)">
              ...
              <div
                v-for="(val, key) in currentSpec.typography"
                :key="key"
                style="
                  display: flex;
              ...
              >
                ...
                <span style="font-family: var(--font-mono); color: var(--color-text-primary)">
                  {{ val.split('/')[0] }} / {{ val.split('/')[1] || '400' }}
                </span>
              </div>
            </div>
            <div style="margin-bottom: var(--space-3)">
              ...
              <div style="display: flex; gap: 4px; align-items: flex-end">
                <div v-for="(val, key) in currentSpec.spacing" :key="key" style="text-align: center; flex: 1">
                  <div
                    :style="{
                      height: Math.max(8, parseInt(val)) + 'px',
                      background: 'var(--color-accent)',
                      borderRadius: '2px',
                      margin: '0 auto 4px',
                      width: Math.max(12, parseInt(val)) + 'px'
                    }"
                  ></div>
                  <div style="font-size: 10px; color: var(--color-text-secondary)">{{ spaceLabels[key] || key }}</div>
                  <div style="font-size: 10px; font-family: var(--font-mono); color: var(--color-text-primary)">
                    {{ val }}
                  </div>
                </div>
              </div>
            ...
            <div style="margin-bottom: var(--space-3)">
              ...
              <div
                v-for="(val, key) in currentSpec.components"
                :key="key"
                style="
                  display: flex;
                  justify-content: space-between;
              ...
              >
                ...
                <span style="font-family: var(--font-mono); color: var(--color-text-primary)">{{ val }}</span>
              </div>
            </div>
            <div>
              ...
              >
                ...
                <span style="font-family: var(--font-mono); color: var(--color-text-primary)">
                  {{ currentSpec.animation.duration }}
                </span>
              </div>
              <div
                style="
              ...
              >
                ...
                <span style="font-family: var(--font-mono); color: var(--color-text-primary); font-size: 10px">
                  {{ currentSpec.animation.easing }}
                </span>
              </div>
            </div>
          </div>
...
<script>
export default { name: 'MobileDesign' }
</script>
<script setup>
...
const designSpecs = {
  1: {
    ...
    typography: { heading: '20px/700', body: '14px/400', caption: '12px/400' },
    spacing: { xs: '4px', sm: '8px', md: '16px', lg: '24px', xl: '32px' },
    components: { buttonHeight: '44px', inputHeight: '40px', cardPadding: '16px', borderRadius: '8px' },
    animation: { duration: '250ms', easing: 'cubic-bezier(0.4, 0, 0.2, 1)' }
  },
  2: {
    name: '科技活力',
    colors: {
    ...
    typography: { heading: '22px/700', body: '14px/400', caption: '11px/400' },
    spacing: { xs: '4px', sm: '8px', md: '16px', lg: '24px', xl: '32px' },
    components: { buttonHeight: '46px', inputHeight: '42px', cardPadding: '14px', borderRadius: '12px' },
    animation: { duration: '200ms', easing: 'cubic-bezier(0.4, 0, 0.2, 1)' }
  },
  3: {
    name: '温暖企业',
    colors: {
    ...
    typography: { heading: '20px/600', body: '15px/400', caption: '12px/400' },
    spacing: { xs: '6px', sm: '10px', md: '18px', lg: '26px', xl: '34px' },
    components: { buttonHeight: '44px', inputHeight: '40px', cardPadding: '18px', borderRadius: '10px' },
  ...
  4: {
    ...
    typography: { heading: '21px/300', body: '14px/400', caption: '11px/300' },
    spacing: { xs: '4px', sm: '8px', md: '16px', lg: '24px', xl: '36px' },
    components: { buttonHeight: '48px', inputHeight: '44px', cardPadding: '20px', borderRadius: '6px' },
    animation: { duration: '350ms', easing: 'cubic-bezier(0.4, 0, 0.2, 1)' }
  },
  5: {
    name: '清新自然',
    colors: {
    ...
    typography: { heading: '20px/600', body: '14px/400', caption: '12px/400' },
    spacing: { xs: '4px', sm: '8px', md: '16px', lg: '24px', xl: '32px' },
    components: { buttonHeight: '44px', inputHeight: '40px', cardPadding: '16px', borderRadius: '12px' },
    animation: { duration: '250ms', easing: 'cubic-bezier(0.4, 0, 0.2, 1)' }
  }
}
...
function exportDesignSpec() {
  ...
  a.download =
    '设计规格_' +
    proposals.find((p) => p.id === activeProposal.value)?.label +
    '_' +
