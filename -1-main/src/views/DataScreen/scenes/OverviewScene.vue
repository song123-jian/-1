  ...
  <div class="scene-overview">
    <div class="scene-overview__cards">
      <div class="scene-overview__card-group">
        <div class="scene-overview__group-label">
      ...
      <div class="scene-overview__left">
        <TechBorder color="#00d4ff">
          <div class="scene-overview__panel">
            <div class="scene-overview__panel-title">
              <span class="panel-dot"></span>
              销售趋势
            </div>
            <div ref="salesTrendRef" class="scene-overview__chart"></div>
          </div>
        </TechBorder>
      </div>
      <div class="scene-overview__center">
        <TechBorder color="#00ff88">
          <div class="scene-overview__panel">
            ...
            <div ref="regionRef" class="scene-overview__chart"></div>
          </div>
        </TechBorder>
      </div>
    ...
    <div class="scene-overview__bottom">
      <TechBorder color="#00ff88">
        <div class="scene-overview__panel">
          ...
          <div ref="inoutRef" class="scene-overview__chart"></div>
        </div>
      </TechBorder>
...
<script>
export default { name: 'OverviewScene' }
</script>
<script setup>
...
onUnmounted(() => {
  ;[salesTrendRef, regionRef, inoutRef].forEach((r) => {
...
function initRegion() {
  ...
  instance.setOption({
    ...
    xAxis: {
      type: 'category',
      data: region.labels,
      ...getBaseOption(dataScreenStore.currentTheme).categoryAxis,
      axisLabel: { ...getBaseOption(dataScreenStore.currentTheme).categoryAxis.axisLabel, rotate: 30 }
    },
    yAxis: { type: 'value', ...getBaseOption(dataScreenStore.currentTheme).valueAxis },
    series: [
      {
        ...
        data: region.data,
        barWidth: '50%',
        itemStyle: { color: (p) => colors[p.dataIndex % colors.length] + 'cc', borderRadius: [4, 4, 0, 0] }
      }
    ]
  })
}
...
.scene-overview__cards {
  display: grid;
  grid-template-columns: repeat(3, 1fr);
  gap: 16px;
}
.scene-overview__card-group {
  display: flex;
  flex-direction: column;
  gap: 8px;
}
.scene-overview__group-label {
  display: flex;
  align-items: center;
  gap: 6px;
  font-size: 13px;
...
.scene-overview__body {
  display: grid;
  grid-template-columns: 2fr 1.2fr 1fr;
  gap: 16px;
  min-height: 280px;
}
.scene-overview__bottom {
  display: grid;
  grid-template-columns: 1.2fr 2fr;
  gap: 16px;
  min-height: 280px;
}
.scene-overview__panel {
  padding: 0;
  height: 100%;
  display: flex;
  flex-direction: column;
}
.scene-overview__panel-title {
