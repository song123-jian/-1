  ...
  <div class="production-management-page">
    ...
    <div class="collapsible-stats">
      <div class="collapsible-stats-header" @click="showProdStatsExpanded = !showProdStatsExpanded">
        <span class="collapsible-stats-title"><Icon name="chart" :size="14" /> 详细统计</span>
    ...
    <div v-if="activeTab === 'orders'" class="prod-tab-content">
      ...
      <div class="gantt-timeline">
        <div class="gantt-header">
          <span class="gantt-title"><Icon name="activity" :size="14" /> 工单时间线</span>
          <div style="display:flex;gap:var(--space-2)">
            <button class="btn btn-ghost btn-sm" :class="{ active: !showGantt }" @click="showGantt = false">表格</button>
            <button class="btn btn-ghost btn-sm" :class="{ active: showGantt }" @click="showGantt = true">甘特图</button>
          </div>
        </div>
        <div v-if="showGantt" class="gantt-body">
          ...
          <div v-for="order in filteredOrders" :key="order.id" class="gantt-row">
            ...
            <div class="gantt-row-track">
              <div class="gantt-bar" :class="'status-' + order.status"
          ...
          <div v-if="filteredOrders.length === 0" style="text-align:center;padding:var(--space-6);color:var(--color-text-tertiary)">暂无工单数据</div>
        </div>
      </div>
            ...
            <table class="inv-table">
              ...
              <tbody>
                ...
                <tr v-for="(order, idx) in paginatedOrders" :key="order.id">
                  <td style="text-align:center;overflow-wrap:break-word;word-wrap:break-word">{{ (orderPage - 1) * orderPageSize + idx + 1 }}</td>
            ...
            <table class="inv-table">
              ...
              <tbody>
                ...
                <tr v-for="(bom, idx) in paginatedBomList" :key="bom.id">
                  <td style="text-align:center;overflow-wrap:break-word;word-wrap:break-word">{{ (bomPage - 1) * bomPageSize + idx + 1 }}</td>
...
// 筛选条件变化时重置页码
watch([orderSearch, orderStatusFilter, orderPriorityFilter], () => { orderPage.value = 1 })
...
// BOM筛选条件变化时重置页码
watch([bomSearch, bomStatusFilter, bomTypeFilter], () => { bomPage.value = 1 })
...
.gantt-bar.status-planned { background: var(--color-text-tertiary); }
.gantt-bar.status-released { background: var(--color-info); }
.gantt-bar.status-in_progress { background: var(--color-accent); }
.gantt-bar.status-quality_check { background: var(--color-warning); }
.gantt-bar.status-completed { background: var(--color-success); }
.gantt-bar.status-cancelled { background: var(--color-danger); }
</style>
