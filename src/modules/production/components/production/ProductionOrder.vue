...
const productionSmartPlaceholder =
  '粘贴生产工单或表格文本，系统将自动识别并提取BOM、数量、日期、车间、负责人和明细。'

function parseProductionSmartInfo(text) {
  const items = []
  ...
  const bomMatch = text.match(/(?:BOM|物料清单|产品)[:\s：]*([^\n\r]{2,40})/)
  if (bomMatch) pushItem('bomName', 'BOM', bomMatch[1].trim(), 72)

  const qtyMatch = text.match(/(?:数量|生产数量|计划数量)[:\s：]*([0-9.]+)/)
  if (qtyMatch) pushItem('quantity', '数量', Number(qtyMatch[1]), 85)

  const startMatch = text.match(/(?:开始日期|计划开始|开工日期)[:\s：]*([0-9]{4}[-/年][0-9]{1,2}[-/月][0-9]{1,2})/)
  if (startMatch) pushItem('plannedStartDate', '开始日期', startMatch[1].replace(/[年月]/g, '-').replace(/\//g, '-'), 80)

  const endMatch = text.match(/(?:结束日期|计划结束|完工日期)[:\s：]*([0-9]{4}[-/年][0-9]{1,2}[-/月][0-9]{1,2})/)
  if (endMatch) pushItem('plannedEndDate', '结束日期', endMatch[1].replace(/[年月]/g, '-').replace(/\//g, '-'), 80)

  const workshopMatch = text.match(/(?:车间|工位)[:\s：]*([^\n\r]{2,20})/)
  if (workshopMatch) pushItem('workshop', '车间', workshopMatch[1].trim(), 72)

  const operatorMatch = text.match(/(?:负责人|操作员)[:\s：]*([^\n\r]{2,20})/)
  if (operatorMatch) pushItem('operator', '负责人', operatorMatch[1].trim(), 72)
...
const {
  ...
  resetSmartRec
} = useSmartRecognizeBase(form, parseProductionSmartInfo, productionSmartPlaceholder)

function applySmartRecognizeToForm() {
  if (!smartRecResult.value) return
  smartRecResult.value.items?.forEach((item) => {
    if (Object.hasOwn(form, item.key) && item.value !== undefined && item.value !== null && item.value !== '') {
