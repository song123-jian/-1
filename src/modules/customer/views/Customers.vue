  ...
  <div class="customer-page">
    <div class="page-header">
      <div>
        ...
        <p class="page-header-subtitle">统一管理客户档案、联系人、标签和跟进状态</p>
      ...
      <div class="page-header-actions">
        ...
        <button v-if="canCreate" class="btn btn-primary" @click="openAddModal">新建客户</button>
        <div ref="moreMenuWrapRef" class="more-dropdown-wrap">
          <button
            type="button"
            class="btn btn-outline"
            :aria-expanded="showMoreMenu"
            :data-more-menu-open="showMoreMenu ? '1' : '0'"
          ...
          <div v-if="showMoreMenu" class="more-dropdown-menu" @click.stop>
            ...
            <button class="more-menu-item" @click="(handleBatchAdd(), (showMoreMenu = false))">
              <Icon name="list" :size="14" />
              批量添加
            </button>
            <button class="more-menu-item" @click="(handleExport(), (showMoreMenu = false))">
    ...
    <div class="customer-toolbar">
      ...
      <div class="customer-search-grid">
        ...
        <div class="search-field">
          ...
          <input v-model="advFilterPhone" type="text" class="search-input" placeholder="输入手机号码..." />
        </div>
        <div class="search-field">
      ...
      <div class="customer-filters">
        ...
        <select v-model="filterStatus" class="form-select filter-select">
          ...
          <option value="dormant">非活跃</option>
        </select>
...
import CustomerTable from '@/modules/customer/components/customers/CustomerTable.vue'
import CustomerList from '@/modules/customer/components/customers/CustomerList.vue'
import CustomerCardView from '@/modules/customer/components/customers/CustomerCardView.vue'
import CustomerCalendarView from '@/modules/customer/components/customers/CustomerCalendarView.vue'
import CustomerWeekView from '@/modules/customer/components/customers/CustomerWeekView.vue'
import CustomerDetailModal from '@/modules/customer/components/customers/CustomerDetailModal.vue'
...
const activeRingDash = computed(() => {
  const p = activePercent.value / 100
...
function saveCustomer() {
  ...
  if (!form.fullName.trim() && !form.contactName.trim() && !form.phone.trim()) {
    formErrors.value.push('请至少填写客户全称、联系人姓名或手机号码其中一项')
  }
  ...
  if (form.phone && !/^1[3-9]\d{9}$/.test(form.phone)) {
    formErrors.value.push('手机号码格式不正确，应为 11 位手机号')
  }
...
function handleBatchExport() {
  ...
  const statusText = (s) => (s === 'active' ? '活跃' : s === 'potential' ? '潜在' : '非活跃')
...
.tag-filter-item.active-success {
  background: var(--color-success-subtle);
  color: var(--color-success);
  border-color: var(--color-success);
}
...
.tag-filter-item.active-warning {
  background: var(--color-warning-subtle);
  color: var(--color-warning);
  border-color: var(--color-warning);
}
.tag-filter-item.active-danger {
  background: var(--color-danger-subtle);
  color: var(--color-danger);
  border-color: var(--color-danger);
}
