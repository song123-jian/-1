  ...
  <div class="customer-detail-page">
    ...
    <div v-if="customer">
      ...
      <div class="detail-top-bar">
        <div class="detail-avatar-lg" :style="{ background: levelColors[customer.level] || '#94a3b8' }">
          {{ (customer.fullName || customer.name || '?').charAt(0) }}
        </div>
        <div class="detail-top-info">
          ...
          <div class="detail-top-badges">
            ...
            <span v-for="tagId in customer.tags || []" :key="tagId" class="mini-tag" :style="_getTagStyle(tagId)">
              {{ _getTagName(tagId) }}
            </span>
          </div>
          <div class="detail-top-meta">
            {{ customer.customerNo }} · {{ customer.region || '未知区域' }} ·
            {{ customer.contactName || customer.contact || '未指定联系人' }}
          </div>
        </div>
        <div class="detail-top-kpis">
          ...
          <div class="kpi-card">
            ...
            <div
              ...
              :class="creditUtilization > 80 ? 'text-danger' : creditUtilization > 50 ? 'text-warning' : 'text-success'"
            >
              {{ creditUtilization }}%
            </div>
          </div>
      ...
      <div class="detail-grid-2col">
        <div class="detail-section">
          ...
          <div class="detail-fields">
            <div class="detail-field">
              ...
              <span class="mono">{{ customer.customerNo || '-' }}</span>
            </div>
            <div class="detail-field">
              <span class="df-label">全称</span>
              <span>{{ customer.fullName || '-' }}</span>
            </div>
            <div class="detail-field">
              <span class="df-label">简称</span>
              <span>{{ customer.shortName || '-' }}</span>
            </div>
            <div class="detail-field">
              <span class="df-label">联系人</span>
              <span>{{ customer.contactName || customer.contact || '-' }}</span>
            </div>
            <div class="detail-field">
              <span class="df-label">电话</span>
        ...
        <div class="detail-section">
          ...
          <div class="detail-fields">
            <div class="detail-field">
              <span class="df-label">客户等级</span>
              <span class="level-badge" :class="'level-' + customer.level">{{ levelLabel(customer.level) }}</span>
            </div>
            <div class="detail-field">
              <span class="df-label">决策权限</span>
              <span>{{ customer.decisionAuthority || '-' }}</span>
            </div>
            <div class="detail-field">
              <span class="df-label">关注点</span>
              <span>{{ customer.coreConcerns || '-' }}</span>
            </div>
            <div class="detail-field">
            ...
            <div class="detail-field">
              ...
              <span class="mono">¥{{ formatNumber(customer.creditLimit) }}</span>
            </div>
            <div class="detail-field">
              <span class="df-label">余额</span>
              <span class="mono">¥{{ formatNumber(customer.balance) }}</span>
            </div>
            <div class="detail-field">
      ...
      <div class="detail-section" style="margin-top: var(--space-4)">
        <div class="detail-tabs">
          <button
            ...
            :class="{ active: activeDocTab === tab.key }"
          ...
          <table v-if="customerQuotations.length > 0" class="detail-table">
            ...
            <tbody>
              <tr v-for="q in customerQuotations" :key="q.id">
                ...
                <td>
                  <span class="status-badge" :class="quoteStatusClass(q.status)">{{ quoteStatusLabel(q.status) }}</span>
                </td>
              </tr>
            </tbody>
          </table>
          ...
          <table v-if="customerDeliveries.length > 0" class="detail-table">
            ...
            <tbody>
              <tr v-for="d in customerDeliveries" :key="d.id">
                ...
                <td>
                  <span class="status-badge" :class="deliveryStatusClass(d.status)">
          ...
          <table v-if="customerCollections.length > 0" class="detail-table">
            ...
            <tbody>
              <tr v-for="col in customerCollections" :key="col.id">
                ...
                <td>
                  <span class="status-badge" :class="collectionStatusClass(col.status)">
                    {{ collectionStatusLabel(col.status) }}
                  </span>
                </td>
              </tr>
            </tbody>
...
const timeline = computed(() => {
  ...
  customerContracts.value.forEach((ct) => {
    events.push({
      date: ct.signDate || ct.createdAt,
      text: `签约 ${ct.contractNo} · ¥${formatNumber(ct.totalAmount)} · ${contractStatusLabel(ct.status)}`,
  ...
  customerDeliveries.value.forEach((d) => {
    events.push({
      date: d.date,
      text: `送货 ${d.deliveryNo} · ¥${formatNumber(d.totalAmount)} · ${deliveryStatusLabel(d.status)}`,
      dotClass: 'dot-delivery'
    })
  })
...
function goEdit() {
  router.push({ path: '/customers', query: { editId: customer.value.id } }).catch(() => {})
}
