    ...
    <div class="panel-card-body no-padding">
      <div class="table-toolbar">
        <div class="column-config-wrapper">
          <button class="btn btn-outline" @click="toggleColumnConfig($event)">
      ...
      <div class="table-container">
        <table class="customer-table">
          <thead>
            <tr>
              <th class="col-check">
                <div class="checkbox" :class="{ checked: isAllSelected }" @click="handleToggleSelectAll">
          ...
          <tbody>
            <tr
              v-for="(c, idx) in paginatedCustomers"
              :key="c.id"
              :style="{ borderLeftColor: levelColors[c.level] || '#94a3b8', animationDelay: idx * 30 + 'ms' }"
              class="row-with-border row-animated"
            >
              <td class="col-check">
                <div
                  class="checkbox"
                  :class="{ checked: selectedIds.includes(c.id) }"
                  @click="handleToggleSelect(c.id)"
                >
                  <Icon name="checkCircle" :size="14" />
                </div>
              </td>
              ...
              <td v-if="columnVisible.no" class="col-no">
                <strong class="text-accent">{{ c.customerNo || '-' }}</strong>
              </td>
              <td v-if="columnVisible.name" class="col-name cell-editable" @dblclick="startInlineEdit(c, 'fullName')">
                <div v-if="!isInlineEditing(c.id, 'fullName')" class="name-cell">
                  ...
                  <div v-if="c.tags && c.tags.length > 0" class="tag-row">
                    <span v-for="tagId in c.tags" :key="tagId" class="mini-tag" :style="_getTagStyle(tagId)">
                      {{ _getTagName(tagId) }}
                    </span>
                  </div>
                </div>
                <input
                  ...
                  class="inline-input"
                  @keydown="onInlineKeydown($event, c)"
                  @blur="confirmInlineEdit(c)"
                />
              </td>
              ...
              <td v-if="columnVisible.level" class="cell-editable" @dblclick="startInlineEdit(c, 'level')">
                <span v-if="!isInlineEditing(c.id, 'level')" class="level-badge" :class="'level-' + c.level">
                  {{ levelLabel(c.level) }}
                </span>
                <select
                  v-else
              ...
              <td
                v-if="columnVisible.decision"
                class="cell-editable"
                @dblclick="startInlineEdit(c, 'decisionAuthority')"
              >
              ...
              <td
                v-if="columnVisible.concerns"
                class="col-concerns cell-editable"
                @dblclick="startInlineEdit(c, 'coreConcerns')"
              >
              ...
              <td v-if="columnVisible.status" class="cell-editable" @dblclick="startInlineEdit(c, 'status')">
                <span v-if="!isInlineEditing(c.id, 'status')" class="status-badge" :class="'status-' + c.status">
              ...
              <td
                v-if="columnVisible.balance"
                class="col-balance"
                :class="{ 'anomaly-highlight': anomalyData[(currentPage - 1) * pageSize + idx]?.isAnomaly }"
              >
                <span class="mono">{{ formatNumber(c.balance || 0) }}</span>
                <span
                  v-if="trendData[(currentPage - 1) * pageSize + idx]"
                  :class="'trend-' + trendData[(currentPage - 1) * pageSize + idx].trend"
                >
                  {{
                    trendData[(currentPage - 1) * pageSize + idx].trend === 'up'
                      ? '↑'
                      : trendData[(currentPage - 1) * pageSize + idx].trend === 'down'
                        ? '↓'
                        : '→'
                  }}
                </span>
              ...
              <td v-if="columnVisible.docs" class="col-docs">
                <div class="docs-cell">
                  <span v-if="getCustomerDocCount(c) > 0" class="doc-badge" @click.stop="$emit('openDetail', c)">
                    {{ getCustomerDocCount(c) }}份
                  </span>
                  <span v-else class="doc-empty">-</span>
                </div>
              </td>
              <td class="col-actions">
                <button class="action-btn" title="编辑" @click="$emit('openEdit', c)">
                ...
                <button class="action-btn danger" title="删除" @click="$emit('handleDelete', c)">
...
<script>
export default { name: 'CustomerTable' }
</script>
<script setup>
