<template>
  <div class="customer-card-view">
    <div
      ...
      :key="c.id"
      class="customer-card"
      :class="{ 'dormant-card': c.status === 'dormant', 'selected-card': selectedIds.includes(c.id) }"
    ...
    >
      ...
      <div class="card-header">
        <div class="card-checkbox" @click.stop="$emit('toggleSelect', c.id)">
          <span class="checkbox" :class="{ checked: selectedIds.includes(c.id) }">
            {{ selectedIds.includes(c.id) ? '✓' : '' }}
          </span>
        </div>
        <div class="card-avatar" :style="{ background: levelColors[c.level] || '#94a3b8' }">
      ...
      <div class="card-body">
        <div class="card-field">
          ...
          <span>{{ c.contactName || c.contact || '-' }}</span>
        </div>
        <div class="card-field">
        ...
        <div class="card-field">
          ...
          <span class="mono">¥{{ formatNumber(c.creditLimit) }}</span>
        </div>
...
<script>
export default { name: 'CustomerCardView' }
</script>
<script setup>
...
.customer-card.dormant-card {
  opacity: 0.6;
}
.customer-card.selected-card {
