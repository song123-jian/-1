<template>
  <div class="contract-page">
      ...
      <div class="page-header-actions">
        ...
        <div class="more-actions-wrapper">
          ...
          <div v-if="showMoreActions" class="more-actions-dropdown">
            <button class="more-actions-item" @click="(handleExport(), (showMoreActions = false))">
            ...
            <button class="more-actions-item" @click="(openTemplateManager(), (showMoreActions = false))">
    ...
    <div v-if="showAdvFilter" class="contract-toolbar-adv">
      ...
      <input v-model.number="filterAmountMin" type="number" class="form-input filter-amount" placeholder="金额≥" />
      <input v-model.number="filterAmountMax" type="number" class="form-input filter-amount" placeholder="金额≤" />
      <button class="btn btn-outline filter-reset-btn" @click="resetFilters">清空筛选</button>
    </div>
    ...
    <div class="contract-stats-bar">
      ...
      <div class="kpi-card-sm" @click="filterStatus = ''">
        <div class="kpi-info">
          <span class="kpi-val mono">楼{{ formatNumber(contractStore.totalAmount) }}</span>
          <span class="kpi-label">
      ...
      <div class="kpi-card-sm" @click="filterStatus = 'signed'">
        <div class="kpi-info">
          <span class="kpi-val mono text-success">楼{{ formatNumber(contractStore.signedAmount) }}</span>
          <span class="kpi-label">
    ...
    <div v-if="showAnalytics" class="analytics-panel">
      <div class="analytics-kpis">
        ...
        <div class="kpi-card">
          <div class="kpi-value text-accent">楼{{ formatNumber(contractStore.signedAmount) }}</div>
        ...
        <div class="panel-card">
          ...
          <div class="panel-card-body">
            <div v-for="(c, i) in contractStore.customerTopList" :key="c.name" class="top-row">
              ...
              <span class="top-amount mono">楼{{ formatNumber(c.amount) }}</span>
            </div>
                  ...
                  <td class="cell-actions">
                    ...
                    <div class="action-more-wrapper">
                      ...
                      <div v-if="activeRowActions === c.id" class="action-more-dropdown">
                        ...
                        <button class="action-more-item" @click="(handleDuplicate(c), (activeRowActions = null))">
                          复制
                        </button>
                        <button
                          v-if="canDelete"
      ...
      <div v-if="currentView === 'funnel'" class="sales-funnel">
        <div v-for="(stage, idx) in funnelStages" :key="stage.key" class="funnel-stage">
          <div class="funnel-bar" :style="{ width: stage.width + '%', background: statusColors[stage.key] }">
            ...
            <span class="funnel-count">{{ stage.count }} 份</span>
            <span class="funnel-amount">楼{{ formatNumber(stage.amount) }}</span>
          </div>
...
function getFirstUnitPrice(c) {
  try {
    ...
    if (Array.isArray(products) && products.length > 0 && products[0].unitPrice) {
      return '楼' + parseFloat(products[0].unitPrice).toFixed(2)
    }
  } catch {
    /* ignore */
  }
...
function handleApprove(c) {
  ...
  if (confirm('确认审批通过合同 ' + c.contractNo + ' 吗？')) {
...
function handleSign(c) {
  ...
  if (confirm('确认签订合同 ' + c.contractNo + ' 吗？')) {
...
function handleArchive(c) {
  ...
  if (confirm('确认归档合同 ' + c.contractNo + ' 吗？')) {
...
function handleCancel(c) {
  ...
  if (confirm('确认作废合同 ' + c.contractNo + ' 吗？作废后不可恢复。')) {
...
function handleDelete(c) {
  if (confirm('确认删除合同 ' + c.contractNo + ' 吗？此操作不可恢复。')) {
...
function handleBatchDelete() {
  if (confirm('确认删除选中的 ' + selectedIds.value.length + ' 条合同吗？')) {
...
function parseContractTemplateFile(file) {
  ...
  setTimeout(() => {
    ...
    contractStore.addTemplate({
      ...
      alert('模板“' + name + '”已通过本地识别创建，请编辑确认详细信息')
    }
  }, 2000)
}
