...
const filteredContracts = computed(() => {
  ...
  if (filterAmountMin.value !== '' && Number(filterAmountMin.value) > 0) list = list.filter(c => (c.totalAmount || 0) >= Number(filterAmountMin.value) * 10000)
  if (filterAmountMax.value !== '' && Number(filterAmountMax.value) > 0) list = list.filter(c => (c.totalAmount || 0) <= Number(filterAmountMax.value) * 10000)
  return list
})
...
const calendarDays = computed(() => {
  ...
  for (let i = 1 - startPad; i <= lastDay.getDate(); i++) {
    ...
    days.push({ date: dateStr, day: d.getDate(), isCurrentMonth: d.getMonth() === m - 1, contracts, isToday: dateStr === todayStr() })
  }
  return days
})
...
.view-btn { padding: var(--space-2) var(--space-4); font-size: var(--font-size-base); border: none; background: var(--color-bg-secondary); color: var(--color-text-secondary); cursor: pointer; transition: all 0.15s; }
...
.search-input { width: 100%; height: 34px; padding: var(--space-2) var(--space-3) var(--space-2) var(--space-8); border: 1px solid var(--color-border); border-radius: var(--radius-md); font-size: 13px; background: var(--color-surface); color: var(--color-text-primary); outline: none; transition: border-color 0.2s; }
.search-input:focus { border-color: var(--color-accent); }
.filter-select { height: 34px; min-width: 110px; padding: var(--space-2) var(--space-3); font-size: 12px; border-radius: var(--radius-md); }
.filter-date-group { display: flex; align-items: center; gap: var(--space-1); }
.filter-date { height: 34px; width: 130px; padding: var(--space-2) var(--space-3); font-size: 12px; border-radius: var(--radius-md); }
...
.pagination-btn { padding: var(--space-1) var(--space-3); font-size: 12px; border: 1px solid var(--color-border); border-radius: 4px; background: var(--color-surface); cursor: pointer; }
.pagination-btn.active { background: var(--color-accent); color: #fff; border-color: var(--color-accent); }
.pagination-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.pagination-info { font-size: 12px; color: var(--color-text-tertiary); margin-left: var(--space-2); }
...
.kpi-card { background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-md); padding: var(--space-4); text-align: center; transition: transform 0.2s ease, box-shadow 0.2s ease; }
...
.kpi-label { font-size: 12px; color: var(--color-text-secondary); margin-top: var(--space-1); }
...
.top-row { display: flex; justify-content: space-between; padding: var(--space-1) 0; border-bottom: 1px solid var(--color-border); font-size: 13px; }
...
.btn { padding: var(--space-2) var(--space-4); border: 1px solid var(--color-border); border-radius: var(--radius-md); font-size: 13px; cursor: pointer; transition: all 0.15s; background: var(--color-surface); color: var(--color-text-primary); }
...
.panel-card-header { padding: var(--space-4) var(--space-5); font-weight: 600; font-size: 14px; border-bottom: 1px solid var(--color-border); }
.panel-card-body { padding: var(--space-4) var(--space-5); }
