  ...
  <div class="quotation-page">
    ...
    <div class="quotation-stats-bar">
      <div class="stats-ring-section" @click="handleRingClick" style="cursor:pointer" title="点击筛选已接受报价">
        ...
        <div class="stats-ring-text">
          <span class="stats-ring-percent" :style="{ color: conversionColor }">{{ quotationStore.conversionRate }}%</span>
    ...
    <template v-else>
        ...
        <div class="panel-card-body no-padding">
          <div class="table-container">
            <table class="data-table">
              ...
              <tbody>
                <tr v-if="pagedQuotations.length === 0"><td :colspan="3 + visibleColCount" class="empty-state"><div class="empty-state-icon"><Icon name="empty" :size="24" /></div>暂无报价数据</td></tr>
      ...
      <div v-if="currentView === 'list'" class="panel-card">
        <div class="panel-card-body">
          <div v-if="filteredQuotations.length === 0" class="empty-state"><div class="empty-state-icon"><Icon name="empty" :size="24" /></div>暂无报价数据</div>
    ...
    <div v-if="currentView === 'funnel'" class="panel-card" style="margin-bottom: var(--space-4);">
      <div class="panel-card-header">
        <span class="panel-card-title"><Icon name="trendingUp" :size="14" /> 销售漏斗</span>
      </div>
      <div class="panel-card-body">
...
const viewModes = [
  ...
  { key: 'funnel', icon: 'trendingUp', label: '漏斗' }
]
...
const funnelStages = computed(() => {
  ...
  return [
    { key: 'draft', label: '草稿', count: draft, height: Math.max(20, (draft / maxCount) * 160), color: 'var(--color-text-tertiary)', rate: Math.round((draft / total) * 100) },
    { key: 'pending', label: '待审批', count: pending, height: Math.max(20, (pending / maxCount) * 160), color: 'var(--color-warning)', rate: draft > 0 ? Math.round((pending / draft) * 100) : 0 },
    { key: 'approved', label: '已审批', count: approved, height: Math.max(20, (approved / maxCount) * 160), color: 'var(--color-info)', rate: pending > 0 ? Math.round((approved / pending) * 100) : 0 },
    { key: 'accepted', label: '已接受', count: accepted, height: Math.max(20, (accepted / maxCount) * 160), color: 'var(--color-success)', rate: approved > 0 ? Math.round((accepted / approved) * 100) : 0 },
  ]
})
...
.page-header-subtitle { font-size: var(--font-size-sm); color: var(--color-text-secondary); margin: var(--space-1) 0 0; }
...
.view-btn { padding: var(--space-2) var(--space-4); font-size: var(--font-size-sm); border: none; background: var(--color-bg-secondary); color: var(--color-text-secondary); cursor: pointer; transition: all 0.15s; }
.view-btn + .view-btn { border-left: 1px solid var(--color-border); }
.view-btn.active { background: var(--color-accent); color: #fff; }
.quotation-toolbar { display: flex; gap: var(--space-2); margin-bottom: var(--space-4); flex-wrap: wrap; align-items: center; }
...
.search-input { width: 100%; padding: var(--space-2) var(--space-3) var(--space-2) var(--space-8); border: 1px solid var(--color-border); border-radius: var(--radius-md); font-size: var(--font-size-sm); background: var(--color-surface); color: var(--color-text-primary); }
...
.quotation-stats-bar { display: flex; gap: var(--space-5); padding: var(--space-4); background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-lg); margin-bottom: var(--space-4); font-size: var(--font-size-base); flex-wrap: wrap; align-items: center; }
...
.stat-money-icon { color: var(--color-text-tertiary); font-size: var(--font-size-xs); }
...
.th-icon { font-size: var(--font-size-sm); margin-right: var(--space-1); vertical-align: middle; line-height: 1; }
...
.pagination-btn { padding: var(--space-1) var(--space-3); font-size: var(--font-size-xs); border: 1px solid var(--color-border); border-radius: 4px; background: var(--color-surface); cursor: pointer; }
.pagination-btn.active { background: var(--color-accent); color: #fff; border-color: var(--color-accent); }
.pagination-btn:disabled { opacity: 0.5; cursor: not-allowed; }
.pagination-info { font-size: var(--font-size-sm); color: var(--color-text-tertiary); margin-left: var(--space-2); }
...
.list-item:hover { background: var(--color-bg-secondary); transform: translateX(2px); }
.list-item-check { flex-shrink: 0; }
.list-item-avatar { width: 40px; height: 40px; border-radius: var(--radius-md); display: flex; align-items: center; justify-content: center; color: #fff; font-weight: 700; font-size: var(--font-size-sm); flex-shrink: 0; box-shadow: 0 2px 8px rgba(0, 0, 0, 0.1); transition: transform 0.2s ease; }
.list-item:hover .list-item-avatar { transform: scale(1.08); }
.list-item-main { flex: 1; min-width: 0; }
.list-item-row1 { display: flex; align-items: center; gap: var(--space-2); margin-bottom: var(--space-1); }
.list-item-name { font-size: var(--font-size-base); }
.list-item-row2 { display: flex; gap: var(--space-3); font-size: var(--font-size-sm); color: var(--color-text-secondary); margin-bottom: var(--space-1); }
.list-item-row3 { display: flex; gap: var(--space-3); font-size: var(--font-size-sm); }
...
.quote-card { background: var(--color-surface); border: 1px solid var(--color-border); border-radius: var(--radius-md); overflow: hidden; transition: all 0.25s ease; animation: cardFadeIn 0.4s ease-out both; }
...
.quote-card-header { display: flex; justify-content: space-between; align-items: center; padding: var(--space-3) var(--space-4); border-bottom: 1px solid var(--color-border); }
.quote-card-body { padding: var(--space-3) var(--space-4); }
.quote-card-field { display: flex; justify-content: space-between; padding: var(--space-1) 0; font-size: var(--font-size-base); }
.field-label { color: var(--color-text-secondary); font-size: var(--font-size-sm); display: flex; align-items: center; gap: var(--space-1); white-space: nowrap; }
.quote-card-footer { display: flex; gap: var(--space-1); padding: var(--space-2) var(--space-4); border-top: 1px solid var(--color-border); background: var(--color-bg-secondary); }
.btn { padding: var(--space-2) var(--space-4); border: 1px solid var(--color-border); border-radius: var(--radius-md); font-size: var(--font-size-sm); cursor: pointer; transition: all 0.15s; background: var(--color-surface); color: var(--color-text-primary); }
...
.sort-icon { font-size: var(--font-size-xs); margin-left: var(--space-1); }
...
.panel-card-header { padding: var(--space-4) var(--space-5); font-weight: 600; font-size: var(--font-size-base); border-bottom: 1px solid var(--color-border); flex-shrink: 0; }
.panel-card-body { padding: var(--space-4) var(--space-5); flex: 1; }
...
.sales-funnel { display: flex; align-items: flex-end; gap: var(--space-5); padding: var(--space-4) 0; justify-content: center; min-height: 240px; }
.funnel-stage { display: flex; flex-direction: column; align-items: center; gap: var(--space-2); flex: 1; max-width: 160px; }
.funnel-bar { width: 100%; border-radius: var(--radius-md) var(--radius-md) 0 0; display: flex; align-items: flex-start; justify-content: center; padding-top: var(--space-2); transition: height 0.4s ease; min-width: 60px; }
.funnel-bar-label { font-weight: 700; font-family: var(--font-mono); font-size: var(--font-size-lg); color: #fff; text-shadow: 0 1px 3px rgba(0,0,0,0.3); }
.funnel-stage-name { font-size: var(--font-size-sm); font-weight: 600; color: var(--color-text-secondary); }
.funnel-stage-rate { font-size: var(--font-size-xs); color: var(--color-text-tertiary); font-family: var(--font-mono); }
