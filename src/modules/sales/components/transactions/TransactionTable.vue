<template>
  <div class="table-container">
    ...
    <div v-if="totalPages > 1" class="pagination-bar">
      <button class="pagination-btn" :disabled="currentPage <= 1" @click="currentPage = 1">芦</button>
      ...
      <button class="pagination-btn" :disabled="currentPage >= totalPages" @click="currentPage = totalPages">禄</button>
...
function runSpacingEval() {
  nextTick(() => {
    ...
    const suggestions = []
    if (balanceScore < 80)
      suggestions.push({ icon: '', text: '部分列宽与建议值偏差较大，建议通过 colgroup 精确控制各列宽度比例。' })
    if (alignScore < 100)
      suggestions.push({
        icon: '',
        text: '部分列的文本对齐方式不够统一，数字列建议右对齐，标签列居中，文本列左对齐。'
      })
    if (densityScore < 85)
      suggestions.push({ icon: '', text: '行高偏小导致内容密度过高，建议增加单元格内边距以提升可读性。' })
    if (hierarchyScore < 85)
      suggestions.push({ icon: '', text: '视觉层级不够清晰，建议结合 table-layout: fixed 与 colgroup 保持列宽稳定。' })
    details.forEach((d) => {
      if (d.statusClass !== 'status-optimal') {
        suggestions.push({
          icon: '',
          text: `“${d.label}”列当前 ${d.currentWidth}px，建议调整为 ${d.optimalWidth}px，当前状态：${d.statusText}。`
        })
      }
    })
    if (suggestions.length === 0) {
      suggestions.push({
        icon: '',
        text: '表格排版已达到最佳状态，各列间距均衡、对齐一致、密度适宜。'
      })
    }
