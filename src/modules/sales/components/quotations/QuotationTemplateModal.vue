    ...
    <div v-if="showModal" class="modal-overlay" @click.self="handleClose">
      <div class="modal-dialog">
        <div class="modal-header">
          <h3><Icon name="list" :size="14" /> 报价模板管理</h3>
        ...
        <div class="modal-body">
          <div class="template-upload-area">
            ...
            <div v-if="aiParsing" class="ai-parsing-hint"><Icon name="zap" :size="14" /> AI正在识别中...</div>
          </div>
          <div style="max-height:400px;overflow-y:auto">
            <div v-if="quotationStore.templates.length === 0" style="text-align:center;padding:40px;color:var(--color-text-tertiary)">
              <div style="font-size:36px;margin-bottom:12px"><Icon name="list" :size="14" /></div>暂无报价模板<br><span style="font-size:12px">在报价编辑界面点击"另存为模板"来创建</span>
            </div>
...
.upload-text { font-size: var(--font-size-base); font-weight: 600; color: var(--color-text-primary); margin-bottom: var(--space-1); }
.upload-hint { font-size: var(--font-size-sm); color: var(--color-text-tertiary); }
.ai-parsing-hint { text-align: center; padding: var(--space-2); color: var(--color-accent); font-size: var(--font-size-sm); margin-top: var(--space-2); }
</style>
