    ...
    <div class="panel-card-body">
      <div class="calendar-nav">
        ...
        <button
          ...
          @click="calYear = new Date().getFullYear(); calMonth = new Date().getMonth() + 1"
        >
      ...
      <div class="calendar-grid">
        ...
        >
          ...
          <div class="calendar-events">
            <div
              v-for="q in day.quotations"
              :key="q.id"
              class="calendar-event"
              :class="'cal-status-' + q.status"
              :title="q.quoteNo + ' ' + q.customerName"
              @click="emit('edit', q)"
            >
    ...
    <div class="panel-card-body">
      ...
      <div class="week-grid">
        <div v-for="day in weekDays" :key="day.date" class="week-column" :class="{ 'week-today': day.isToday }">
          ...
          <div class="week-column-body">
            <div v-if="day.quotations.length === 0" class="week-empty">
            ...
            >
              <div class="week-event-header">
                <span class="week-event-id">
                  <Icon name="checkCircle" :size="14" />
                  {{ q.quoteNo }}
                </span>
                <span class="week-event-status" :class="'status-' + q.status">
                  {{ statusLabels[q.status] || q.status }}
                </span>
              </div>
              <div class="week-event-customer">
                <Icon name="building" :size="14" />
                {{ q.customerName }}
              </div>
              <div class="week-event-amount">
                <Icon name="dollar" :size="14" />
                ¥{{ (q.totalAmount || 0).toLocaleString('zh-CN') }}
              </div>
              <div v-if="q.margin" class="week-event-margin">
      ...
      <div class="week-summary">
        <span>
          ...
          本周报价:
          <strong>{{ weekDays.reduce((s, d) => s + d.quotations.length, 0) }}</strong>
          条
        </span>
        <span>
          <Icon name="dollar" :size="14" />
          ...
          <strong>
            ¥{{
              weekDays
                .reduce((s, d) => s + d.quotations.reduce((a, q) => a + (q.totalAmount || 0), 0), 0)
        ...
        <span>
          ...
          <strong>
            ¥{{
              Math.round(
                weekDays.reduce((s, d) => s + d.quotations.reduce((a, q) => a + (q.totalAmount || 0), 0), 0) / 7
              ).toLocaleString('zh-CN')
            }}
          </strong>
        </span>
...
<script>
export default { name: 'QuotationCalendarView' }
</script>
<script setup>
...
function weekPrev() {
  weekStartDate.value = new Date(weekStartDate.value.getTime() - 7 * 86400000)
}
function weekNext() {
  weekStartDate.value = new Date(weekStartDate.value.getTime() + 7 * 86400000)
}
...
.week-event-status.status-draft {
  background: rgba(100, 116, 139, 0.15);
  color: #64748b;
}
.week-event-status.status-pending {
  background: rgba(245, 158, 11, 0.15);
  color: #d97706;
}
.week-event-status.status-approved {
  background: rgba(59, 130, 246, 0.15);
  color: #3b82f6;
}
.week-event-status.status-sent {
  background: rgba(6, 182, 212, 0.15);
  color: #06b6d4;
}
.week-event-status.status-accepted {
  background: rgba(34, 197, 94, 0.15);
  color: #22c55e;
}
.week-event-status.status-rejected {
  background: rgba(239, 68, 68, 0.15);
  color: #ef4444;
}
.week-event-status.status-expired {
