    ...
    <div v-if="showModal" class="quote-letter-overlay" @click.self="handleClose">
      <div class="quote-letter-container">
        <div class="quote-letter-toolbar">
          <span class="quote-letter-toolbar-title">
            <Icon name="file" :size="14" />
            正式报价函预览
          </span>
          <div class="quote-letter-toolbar-actions">
            <button class="btn btn-secondary btn-sm" @click="exportQuoteLetterPDF">
              <Icon name="download" :size="14" />
              导出PDF
            </button>
            <button class="btn btn-secondary btn-sm" @click="exportQuoteLetterWord">
              <Icon name="download" :size="14" />
              导出Word
            </button>
            <button class="btn btn-secondary btn-sm" @click="printQuoteLetter">
        ...
        <div v-if="quotation" class="quote-letter-content">
          ...
          <div class="ql-info-grid">
            <div class="ql-info-row">
              <span class="ql-info-label">致：</span>
              <span class="ql-info-value">{{ quotation.customerFullName || quotation.customerName }}</span>
            </div>
            <div class="ql-info-row">
              <span class="ql-info-label">发件方：</span>
              <span class="ql-info-value">{{ quotation.senderCompany || '苏州冠久新材料科技有限公司' }}</span>
            </div>
            <div class="ql-info-row">
              <span class="ql-info-label">联系人：</span>
              <span class="ql-info-value">{{ quotation.custContact || '-' }}</span>
            </div>
            <div class="ql-info-row">
              <span class="ql-info-label">联系人：</span>
              <span class="ql-info-value">{{ quotation.senderContact || '[我司业务对接人]' }}</span>
            </div>
            <div class="ql-info-row">
              <span class="ql-info-label">联系电话：</span>
              <span class="ql-info-value">{{ quotation.custPhone || '-' }}</span>
            </div>
            <div class="ql-info-row">
              <span class="ql-info-label">联系电话：</span>
              <span class="ql-info-value">{{ quotation.senderPhone || '-' }}</span>
            </div>
            <div class="ql-info-row">
              <span class="ql-info-label">电子邮箱：</span>
              <span class="ql-info-value">{{ quotation.custEmail || '-' }}</span>
            </div>
            <div class="ql-info-row">
              <span class="ql-info-label">电子邮箱：</span>
              <span class="ql-info-value">{{ quotation.senderEmail || '-' }}</span>
            </div>
          </div>
          <hr class="ql-divider-thin" />
          <p class="ql-indent">尊敬的{{ quotation.custContact || quotation.customerName }}：</p>
          <p class="ql-indent">
            首先，衷心感谢贵司及您对我司的信任与长期支持！针对贵司近期提出的材料采购需求，我司经过认真核算，现提供以下正式报价方案：
          </p>
          ...
          <table class="ql-table">
            ...
            <tbody>
              ...
              <tr style="font-weight: bold; background: #f8f8f8">
                ...
                <td style="text-align: right; overflow-wrap: break-word; word-wrap: break-word">
                  {{ (((quotation.subtotal || 0) * (quotation.taxRate || 13)) / 100).toFixed(2) }}
                </td>
                <td></td>
              </tr>
              <tr style="font-weight: bold; background: #f0f0f0">
                <td colspan="3" style="text-align: right">含税合计</td>
                <td style="text-align: right; color: #c00; overflow-wrap: break-word; word-wrap: break-word">
          ...
          <div class="ql-terms">
            <ol>
              ...
              <li>
                <strong>报价有效期：</strong>
                自{{ formatDateCN(quotation.date) }}起至{{ formatDateCN(quotation.expiryDate) }}止，共计{{
                  expiryDays
                }}天。
              </li>
              <li>
                <strong>付款方式：</strong>
                {{
                  quotation.termPayment || '默认采用"款到发货"结算方式，具体付款条件可根据订单金额另行友好协商确定。'
                }}
              </li>
              <li>
                <strong>交货周期：</strong>
                {{ quotation.termDelivery || '在收到贵司全额货款后5个工作日内完成生产并发货。' }}
              </li>
              <li>
                <strong>交货地点：</strong>
                {{ quotation.termDeliveryAddr || '[请填写具体收货地址]' }}
              </li>
              <li>
                <strong>质量标准：</strong>
                {{
                  quotation.termQuality || '产品质量严格符合国家相关行业标准及双方确认的技术要求，随货提供质检报告。'
                }}
              </li>
              <li>
                <strong>价格调整机制：</strong>
                {{
                  quotation.termPriceAdj ||
          ...
          <div class="ql-closing">
            <p class="ql-indent">
              感谢贵公司长期以来的信任与支持，我司将一如既往地为贵司提供稳定可靠的产品品质与专业高效的技术服务！期待与贵司的愉快合作！
            </p>
          ...
          <div class="ql-signature">
            <div class="ql-signature-block">
              <div>
                <strong>{{ quotation.senderCompany || '苏州冠久新材料科技有限公司' }}</strong>
              </div>
            ...
            <div class="ql-signature-block">
              <div>
                授权代表签字：
                <span class="ql-signature-line"></span>
              </div>
              <div>
                联系人：
                <span class="ql-signature-line"></span>
...
<script>
export default { name: 'QuotationPreview' }
</script>
<script setup>
...
function exportQuoteLetterWord() {
  ...
  if (!content) return
  const html =
    '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:w="urn:schemas-microsoft-com:office:word" xmlns="http://www.w3.org/TR/REC-html40"><head><meta charset="utf-8"><title>报价函</title></head><body>' +
    content.innerHTML +
    '</body></html>'
...
.quote-letter-toolbar-title {
  font-size: var(--font-size-base);
  font-weight: 600;
  color: var(--color-text-primary);
}
.quote-letter-toolbar-actions {
  display: flex;
  gap: var(--space-2);
}
