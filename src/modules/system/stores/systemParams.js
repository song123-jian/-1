...
export const useSystemParamsStore = defineStore('systemParams', () => {
  ...
  const paramGroups = computed(() => [
    ...
    { key: 'backup', label: '备份设置', icon: 'save' },
    { key: 'notification', label: '通知设置', icon: 'mail' },
    { key: 'performance', label: '性能设置', icon: 'zap' }
  ])

  function _persistParams() {
    persist(PARAMS_KEY, params.value)
  }
  function _persistLogs() {
    persist(LOGS_KEY, logs.value)
  }
  ...
  function resetGroup(group) {
    ...
    keys.forEach((k) => {
      params.value[k] = defaultParams[k]
    })
    _persistParams()
    _addLog('重置分组', group)
  }
