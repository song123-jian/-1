  ...
  <div class="docsettings-page">
    <div class="page-header">
      ...
      <div class="page-header-actions">
        <button class="btn btn-primary" @click="openQualModal"><Icon name="add" :size="14" /> 新增资质</button>
        <button class="btn btn-ghost" @click="checkCompliance"><Icon name="search" :size="14" /> 合规检查</button>
        <button class="btn btn-ghost" @click="exportQuals"><Icon name="upload" :size="14" /> 导出清单</button>
      </div>
    </div>
    ...
    <div v-if="activeTab === 'config'">
      <div class="panel-card" style="margin-bottom:var(--space-6)">
        ...
        <div class="panel-card-body">
          ...
          <div class="form-group">
            <label class="form-label"><input type="checkbox" v-model="editSettings.autoFreeze"> 到期自动冻结交易 <span style="font-size:var(--font-size-xs);color:var(--color-danger)"><Icon name="warning" :size="14" /> 关闭此选项需管理员权限</span></label>
          </div>
          <div class="form-group">
          ...
          <div class="form-group" style="margin-top:var(--space-3)">
            ...
            <div style="display:flex;gap:var(--space-2)">
              <button class="btn btn-ghost btn-sm" @click="applyPreset('strict')">[锁定] 严格模式</button>
              <button class="btn btn-ghost btn-sm" @click="applyPreset('standard')"><Icon name="list" :size="14" /> 标准模式</button>
              <button class="btn btn-ghost btn-sm" @click="applyPreset('loose')">[解锁] 宽松模式</button>
            </div>
          </div>
          <button class="btn btn-primary" style="margin-top:var(--space-4)" @click="saveSettings"><Icon name="save" :size="14" /> 保存设置</button>
        </div>
      </div>
    ...
    <div v-if="activeTab === 'quals'">
      <div class="stats-row" style="margin-bottom:var(--space-4)">
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-accent-subtle);color:var(--color-accent)"><Icon name="list" :size="14" /></div><div class="stat-card-value">{{ dsStore.qualTotal }}</div><div class="stat-card-label">资质总数</div></div>
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-success-subtle);color:var(--color-success)"><Icon name="check" :size="14" /></div><div class="stat-card-value">{{ dsStore.qualActive }}</div><div class="stat-card-label">有效资质</div></div>
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-warning-subtle);color:var(--color-warning)"><Icon name="bell" :size="14" /></div><div class="stat-card-value">{{ dsStore.qualExpiring }}</div><div class="stat-card-label">即将到期</div></div>
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-danger-subtle);color:var(--color-danger)"><Icon name="close" :size="14" /></div><div class="stat-card-value">{{ dsStore.qualExpired }}</div><div class="stat-card-label">已过期</div></div>
      </div>
        ...
        <div class="panel-card-body no-padding">
          <div class="table-container">
            <table class="data-table">
              ...
              <tbody>
                <tr v-if="filteredQuals.length === 0"><td colspan="9" class="empty-state"><Icon name="list" :size="14" /> 暂无资质证照，点击'新增资质'开始添加</td></tr>
                <tr v-for="q in filteredQuals" :key="q.id">
                  ...
                  <td>
                    <button class="btn btn-ghost btn-sm" @click="viewQual(q)"><Icon name="eye" :size="14" /> 查看</button>
                    <button class="btn btn-ghost btn-sm" @click="editQual(q)"><Icon name="edit" :size="14" /> 编辑</button>
                    <button v-if="q.status === 'expired' || q.status === 'expiring_soon'" class="btn btn-ghost btn-sm" @click="renewQual(q)"><Icon name="refresh" :size="14" /> 续期</button>
                    <button v-if="q.status === 'pending_review'" class="btn btn-ghost btn-sm" style="color:var(--color-success)" @click="approveQual(q.id)">[√] 审核</button>
                    <button class="btn btn-ghost btn-sm" style="color:var(--color-danger)" @click="revokeQual(q.id)"><Icon name="close" :size="14" /> 作废</button>
                  </td>
                </tr>
              </tbody>
            </table>
    ...
    <div v-if="activeTab === 'invoice'">
      <div class="page-header" style="margin-bottom:var(--space-3)">
        <div></div>
        <div class="page-header-actions"><button class="btn btn-primary" @click="openInvoiceModal"><Icon name="add" :size="14" /> 新增开票资料</button></div>
      </div>
      <div class="panel-card">
        <div class="panel-card-body no-padding">
          <div class="table-container">
            <table class="data-table">
              ...
              <tbody>
                <tr v-if="dsStore.invoiceProfiles.length === 0"><td colspan="9" class="empty-state"><Icon name="file" :size="14" /> 暂无开票资料，点击'新增开票资料'开始添加</td></tr>
                <tr v-for="inv in dsStore.invoiceProfiles" :key="inv.id">
                  <td>{{ inv.companyName }}</td><td>{{ inv.taxNo }}</td><td>{{ inv.bank }}</td><td style="font-family:var(--font-mono);font-size:var(--font-size-xs);overflow-wrap:break-word;word-wrap:break-word">{{ inv.bankAccount }}</td>
                  ...
                  <td>
                    <button class="btn btn-ghost btn-sm" @click="editInvoice(inv)"><Icon name="edit" :size="14" /> 编辑</button>
                    <button v-if="!inv.isDefault" class="btn btn-ghost btn-sm" @click="setDefaultInvoice(inv.id)">设为默认</button>
                    <button class="btn btn-ghost btn-sm" style="color:var(--color-danger)" @click="deleteInvoice(inv.id)"><Icon name="close" :size="14" /> 删除</button>
                  </td>
                </tr>
              </tbody>
            </table>
    ...
    <div v-if="activeTab === 'compliance'">
      <div class="stats-row" style="margin-bottom:var(--space-4)">
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-success-subtle);color:var(--color-success)"><Icon name="shield" :size="14" /></div><div class="stat-card-value">{{ dsStore.complianceScore }}</div><div class="stat-card-label">合规评分</div></div>
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-accent-subtle);color:var(--color-accent)"><Icon name="list" :size="14" /></div><div class="stat-card-value">{{ dsStore.qualTotal }}</div><div class="stat-card-label">资质总数</div></div>
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-warning-subtle);color:var(--color-warning)"><Icon name="warning" :size="14" /></div><div class="stat-card-value">{{ dsStore.riskCount }}</div><div class="stat-card-label">风险项</div></div>
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-info-subtle);color:var(--color-info)"><Icon name="chart" :size="14" /></div><div class="stat-card-value">{{ dsStore.complianceRate }}</div><div class="stat-card-label">合规率</div></div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-4)">
        <div class="panel-card">
          <div class="panel-card-header"><span class="panel-card-title"><Icon name="calendar" :size="14" /> 资质到期时间线</span></div>
          <div class="panel-card-body"><canvas ref="timelineChartRef" style="width:100%;height:260px"></canvas></div>
        </div>
        <div class="panel-card">
          <div class="panel-card-header"><span class="panel-card-title"><Icon name="chart" :size="14" /> 资质类型分布</span></div>
        ...
        <div class="panel-card-body" style="max-height:400px;overflow-y:auto">
          ...
          <div v-for="r in riskItems" :key="r.id" style="padding:var(--space-3);border-bottom:1px solid var(--color-border);display:flex;justify-content:space-between;align-items:center">
            ...
            <div style="display:flex;align-items:center;gap:var(--space-2)">
              ...
              <button class="btn btn-ghost btn-sm" @click="renewQual(r)"><Icon name="refresh" :size="14" /> 续期</button>
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="activeTab === 'logs'">
      <div class="filter-bar" style="margin-bottom:var(--space-3);flex-wrap:wrap;gap:var(--space-2)">
        ...
        <button class="btn btn-ghost btn-sm" @click="exportLogs"><Icon name="upload" :size="14" /> 导出日志</button>
      </div>
      <div class="panel-card">
            ...
            <table class="data-table">
              ...
              <tbody>
                ...
                <tr v-for="log in filteredLogs.slice(0, 50)" :key="log.id">
                  <td>{{ log.time }}</td><td>{{ log.user }}</td><td>{{ log.action }}</td><td style="max-width:300px;overflow-wrap:break-word;word-wrap:break-word">{{ log.detail }}</td><td>{{ log.ip }}</td>
                </tr>
              </tbody>
            </table>
          </div>
    ...
    <div v-if="showQualModal" class="modal-overlay" @click.self="showQualModal = false">
      <div class="modal-dialog" @click.stop>
        <div class="modal-header"><span class="modal-title">{{ editingQual ? '编辑资质' : '新增资质' }}</span><button class="modal-close" @click="showQualModal = false"><Icon name="close" :size="14" /></button></div>
        <div class="modal-body">
          ...
          <div class="form-group"><label class="form-label">备注</label><textarea class="form-input" v-model="qualForm.remark" rows="2" placeholder="备注信息"></textarea></div>
        </div>
        <div class="modal-footer"><button class="btn btn-secondary" @click="showQualModal = false">取消</button><button class="btn btn-primary" @click="submitQual">{{ editingQual ? '保存修改' : '确认添加' }}</button></div>
      </div>
    </div>

    <div v-if="showInvoiceModal" class="modal-overlay" @click.self="showInvoiceModal = false">
      <div class="modal-dialog" @click.stop>
        <div class="modal-header"><span class="modal-title">{{ editingInvoice ? '编辑开票资料' : '新增开票资料' }}</span><button class="modal-close" @click="showInvoiceModal = false"><Icon name="close" :size="14" /></button></div>
        <div class="modal-body">
        ...
        <div class="modal-footer"><button class="btn btn-secondary" @click="showInvoiceModal = false">取消</button><button class="btn btn-primary" @click="submitInvoice">{{ editingInvoice ? '保存修改' : '确认添加' }}</button></div>
      </div>
    </div>
  </div>
</template>
...
const qualForm = reactive({ name: '', type: 'business_license', certNo: '', issuer: '', issueDate: '', expiryDate: '', remark: '' })
...
function openQualModal() {
  ...
  Object.assign(qualForm, { name: '', type: 'business_license', certNo: '', issuer: '', issueDate: '', expiryDate: '', remark: '' })
  showQualModal.value = true
}

function editQual(q) {
  editingQual.value = q
  Object.assign(qualForm, { name: q.name, type: q.type, certNo: q.certNo, issuer: q.issuer, issueDate: q.issueDate, expiryDate: q.expiryDate, remark: q.remark || '' })
  showQualModal.value = true
}
...
function viewQual(q) { alert('资质详情: ' + q.name) }
...
function editInvoice(inv) {
  ...
  Object.assign(invoiceForm, { companyName: inv.companyName, taxNo: inv.taxNo, bank: inv.bank, bankAccount: inv.bankAccount, address: inv.address, phone: inv.phone, invoiceType: inv.invoiceType, isDefault: inv.isDefault })
