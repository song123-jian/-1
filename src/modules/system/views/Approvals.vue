  ...
  <div class="approval-page">
    <div class="page-header">
      <div>
        ...
        <p class="page-header-subtitle">配置审批流转规则，支持金额、角色和节点路由</p>
      </div>
    ...
    <div class="panel-card" style="margin-bottom: var(--space-4)">
      <div class="panel-card-header">
        <span class="panel-card-title">
          <Icon name="clock" :size="14" />
          待我审批 ({{ workflowStore.pendingTasks.length }})
        </span>
        <button class="btn btn-ghost btn-sm" @click="refreshPendingTasks">
          <Icon name="refresh" :size="14" />
          刷新
        </button>
      </div>
      <div class="panel-card-body no-padding">
        <div v-if="workflowStore.pendingTasks.length > 0" class="table-container">
          <table class="data-table">
            ...
            <tbody>
              <tr v-for="task in workflowStore.pendingTasks" :key="task.id">
                <td>
                  <span class="status-badge accent">{{ task.templateName }}</span>
                </td>
    ...
    <div class="panel-card" style="margin-bottom: var(--space-4)">
      <div class="panel-card-header">
        <span class="panel-card-title">
          <Icon name="checkCircle" :size="14" />
          已审批 ({{ workflowStore.completedTasks.length }})
        </span>
      </div>
      <div class="panel-card-body no-padding">
        <div v-if="workflowStore.completedTasks.length > 0" class="table-container">
          <table class="data-table">
            <thead>
              <tr>
                <th>流程类型</th>
            ...
            <tbody>
              <tr v-for="task in workflowStore.completedTasks.slice(0, 10)" :key="task.id">
                ...
                <td>
                  <span
                    class="status-badge"
                    :class="task.status === 'completed' ? 'success' : task.status === 'rejected' ? 'danger' : 'neutral'"
                  >
    ...
    <div style="display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-6); margin-bottom: var(--space-4)">
      <div class="panel-card">
        <div class="panel-card-header">
          ...
          <button class="btn btn-ghost btn-sm" @click="refreshFlowChart">
            <Icon name="refresh" :size="14" />
            刷新
          </button>
        </div>
        <div class="panel-card-body">
          <canvas
            ref="flowCanvas"
            style="width: 100%; height: 300px; border: 1px dashed var(--color-border); border-radius: var(--radius-md)"
          ></canvas>
        </div>
      </div>

      <div class="panel-card">
        <div class="panel-card-header"><span class="panel-card-title">新增审批规则</span></div>
        <div class="panel-card-body">
          <div class="form-group">
            <label class="form-label">规则模板</label>
            <select v-model="editorData.template" class="form-select" @change="applyTemplate">
          ...
          <div class="form-row form-row-2">
            <div class="form-group">
              ...
              <select v-model="editorData.module" class="form-select">
                <option v-for="m in approvalStore.moduleOptions" :key="m" :value="m">{{ m }}</option>
              </select>
            </div>
            <div class="form-group">
              <label class="form-label">条件类型</label>
              <select v-model="editorData.condition" class="form-select">
          ...
          <div class="form-row form-row-2">
            <div class="form-group">
              ...
              <input v-model.number="editorData.threshold" type="number" class="form-input" min="0" />
            </div>
            <div class="form-group">
              <label class="form-label">审批人角色</label>
              <select v-model="editorData.approver" class="form-select">
          ...
          <div class="form-row form-row-3">
            <div class="form-group">
              ...
              <select v-model="editorData.approvalType" class="form-select">
            ...
            <div class="form-group">
              ...
              <input v-model.number="editorData.timeout" type="number" class="form-input" min="1" max="720" />
            </div>
            <div class="form-group">
              <label class="form-label">下一位审批人</label>
              <select v-model="editorData.nextApprover" class="form-select">
      ...
      <div class="panel-card-header">
        ...
        <div style="display: flex; gap: var(--space-2); align-items: center; flex-wrap: wrap">
          <input
            ...
            class="form-input"
            placeholder="搜索规则..."
            style="width: 160px; font-size: var(--font-size-xs)"
          />
          <button v-if="canApprove" class="btn btn-ghost btn-sm" @click="batchOp('enable')">
            <Icon name="checkCircle" :size="14" />
            批量启用
          </button>
          <button v-if="canApprove" class="btn btn-ghost btn-sm" @click="batchOp('disable')">批量禁用</button>
          <button v-if="canApprove" class="btn btn-danger btn-sm" @click="batchOp('delete')">
          ...
          <table class="data-table">
            <thead>
              <tr>
                ...
                <th style="cursor: pointer" @click="sortBy('module')">
                  模块
                  <Icon name="filter" :size="14" />
                </th>
                <th style="cursor: pointer" @click="sortBy('condition')">
                  触发条件
                  <Icon name="filter" :size="14" />
                </th>
                <th>审批人</th>
            ...
            <tbody>
              ...
              <tr v-for="(rule, idx) in filteredRules" :key="rule.id">
                <td><input v-model="selectedIds" type="checkbox" :value="rule.id" /></td>
                ...
                <td>
                  <div style="display: flex; gap: 2px; flex-wrap: wrap">
                    <button
                      ...
                      title="委托审批"
                      @click="openDelegateModal(rule)"
                    >
                      委托
                    </button>
                    <button v-if="canApprove" class="btn btn-ghost btn-sm" title="撤回审批" @click="doRecall(rule.id)">
                      撤回
                    </button>
                    <button
                      v-if="canApprove"
                      class="btn btn-ghost btn-sm"
                      title="超时升级"
                      :disabled="!rule.nextApprover"
                      @click="doEscalate(rule)"
                    >
                      升级
                    </button>
                    <button
                      ...
                      class="btn btn-ghost btn-sm"
                      title="超时检查"
                      @click="checkTimeoutStatus(rule)"
                    >
                      [检查]
                    </button>
                    <button v-if="canApprove" class="btn btn-danger btn-sm" @click="deleteRule(rule.id)">
                      <Icon name="delete" :size="14" />
                    </button>
                  </div>
                </td>
    ...
    <div class="panel-card" style="margin-top: var(--space-4)">
      <div class="panel-card-header">
        ...
        <div style="display: flex; gap: var(--space-2); align-items: center">
          <select v-model="logFilter" class="form-select" style="font-size: var(--font-size-xs); width: auto">
          ...
          <button class="btn btn-ghost btn-sm" title="清空日志" @click="approvalStore.clearLogs()">清空</button>
        </div>
      </div>
      <div class="panel-card-body no-padding">
          ...
          <table class="data-table">
            ...
            <tbody>
              <tr v-if="filteredLogs.length === 0">
                <td colspan="4" style="text-align: center; color: var(--color-text-tertiary); padding: var(--space-4)">
                  暂无审批日志
                </td>
              </tr>
              <tr v-for="log in filteredLogs" :key="log.id">
                <td style="font-size: var(--font-size-xs); overflow-wrap: break-word; word-wrap: break-word">
                  {{ formatTime(log.createdAt) }}
                </td>
                <td>
                  <span class="status-badge" :class="logActionClass(log.action)">{{ logActionLabel(log.action) }}</span>
                </td>
                <td style="font-size: var(--font-size-xs); overflow-wrap: break-word; word-wrap: break-word">
                  {{ log.description }}
                </td>
    ...
    <div v-if="showPasswordModal" class="modal-overlay" @click.self="showPasswordModal = false">
      <div class="modal-dialog" @click.stop>
        <div class="modal-header">
          <span class="modal-title">审批确认</span>
          <button class="modal-close" @click="showPasswordModal = false"><Icon name="close" :size="14" /></button>
        </div>
        ...
        <div class="modal-footer">
          ...
          <button class="btn btn-primary" @click="confirmPassword">确认</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showDelegateModal" class="modal-overlay" @click.self="showDelegateModal = false">
      <div class="modal-dialog" @click.stop>
        <div class="modal-header">
          <span class="modal-title">审批委托</span>
          <button class="modal-close" @click="showDelegateModal = false"><Icon name="close" :size="14" /></button>
        </div>
        <div class="modal-body">
          <div
            style="
          ...
          >
            当前审批人:
            <strong>{{ delegateTarget?.approver }}</strong>
            | 模块:
            <strong>{{ delegateTarget?.module }}</strong>
          </div>
          <div class="form-group">
            ...
            <select v-model="delegateTo" class="form-select">
              ...
              <option
                v-for="a in approvalStore.approverOptions.filter((x) => x !== delegateTarget?.approver)"
                :key="a"
                :value="a"
              >
                {{ a }}
        ...
        <div class="modal-footer">
          <button class="btn btn-secondary" @click="showDelegateModal = false">取消</button>
          <button class="btn btn-primary" @click="doDelegate">确认委托</button>
        </div>
      </div>
    </div>
    ...
    <div v-if="showTimeoutModal" class="modal-overlay" @click.self="showTimeoutModal = false">
      <div class="modal-dialog" @click.stop>
        <div class="modal-header">
          <span class="modal-title">超时检查结果</span>
          <button class="modal-close" @click="showTimeoutModal = false"><Icon name="close" :size="14" /></button>
        </div>
        <div class="modal-body">
          <div
            v-if="timeoutResult.timedOut"
            style="padding: var(--space-3); background: var(--color-danger-subtle); border-radius: var(--radius-md)"
          >
            ...
            <div style="font-size: var(--font-size-sm); color: var(--color-danger)">
              超时: {{ timeoutResult.hoursOverdue }} 小时
            </div>
            <div v-if="timeoutResult.rule?.nextApprover" style="margin-top: var(--space-2)">
              <button
                class="btn btn-danger btn-sm"
                @click="(doEscalate(timeoutResult.rule), (showTimeoutModal = false))"
              >
                立即升级到 {{ timeoutResult.rule.nextApprover }}
              </button>
            </div>
          </div>
          <div
            v-else
            style="padding: var(--space-3); background: var(--color-success-subtle); border-radius: var(--radius-md)"
          >
            ...
            <div style="font-size: var(--font-size-sm); color: var(--color-success)">
              剩余: {{ timeoutResult.hoursRemaining }} 小时
            </div>
          </div>
        </div>
        <div class="modal-footer">
          <button class="btn btn-secondary" @click="showTimeoutModal = false">关闭</button>
        </div>
      </div>
    </div>
  </div>
...
function applyTemplate() {
  ...
  if (t) {
    ...
    editorData.nextApprover = t.nextApprover || ''
  }
}
...
function validateRule() {
  ...
  if (editorData.condition !== 'all' && editorData.threshold <= 0) errors.push('阈值必须大于 0')
  if (editorData.timeout < 1) errors.push('超时时间至少 1 小时')
  const isDuplicate = approvalStore.rules.some(
    (r) => r.module === editorData.module && r.condition === editorData.condition
  )
...
function batchOp(action) {
  ...
  if (action === 'delete' && !confirm('确认批量删除选中的规则吗？')) return
...
function doRecall(ruleId) {
  if (confirm('确认撤回该审批规则吗？撤回后规则将被禁用。')) {
    approvalStore.recallApproval(ruleId)
  }
}

function doEscalate(rule) {
  if (!rule.nextApprover) {
    alert('该规则未配置下一位审批人，无法升级')
    return
  }
  if (confirm('确认将审批升级到 ' + rule.nextApprover + ' 吗？')) {
...
function renderFlowCanvas() {
  ...
  const w = rect.width,
    h = rect.height
  const bgColor =
    getComputedStyle(document.documentElement).getPropertyValue('--color-bg-secondary').trim() || '#1e293b'
  ctx.fillStyle = bgColor
  ctx.fillRect(0, 0, w, h)
  const rules = approvalStore.rules
  if (rules.length === 0) {
    ctx.fillStyle = '#94a3b8'
    ctx.font = `14px ${CJK_SANS_FONT_STACK}`
  ...
  ctx.font = `bold 12px ${CJK_SANS_FONT_STACK}`
  ctx.textAlign = 'center'
  ...
  for (let i = 0; i < rules.length; i++) {
    ...
    if (ctx.roundRect) {
      ...
      ctx.moveTo(x + 8, y)
      ctx.arcTo(x + nodeW, y, x + nodeW, y + nodeH, 8)
      ctx.arcTo(x + nodeW, y + nodeH, x, y + nodeH, 8)
      ctx.arcTo(x, y + nodeH, x, y, 8)
      ctx.arcTo(x, y, x + nodeW, y, 8)
      ctx.closePath()
    }
    ctx.fill()
    ctx.fillStyle = '#fff'
    ctx.font = `bold 11px ${CJK_SANS_FONT_STACK}`
    ctx.textAlign = 'center'
    ctx.fillText(rules[i].module || '', x + nodeW / 2, y + 18)
    ctx.font = `9px ${CJK_SANS_FONT_STACK}`
    ctx.fillText(rules[i].approver || '', x + nodeW / 2, y + 33)
    ctx.fillText(rules[i].enabled ? '已启用' : '已禁用', x + nodeW / 2, y + 44)
    if (i === 0) {
      ctx.strokeStyle = '#475569'
      ctx.lineWidth = 1
      ctx.beginPath()
      ctx.moveTo(w / 2, startY - 2)
      ctx.lineTo(x + nodeW / 2, y)
      ctx.stroke()
    }
    if (i > 0) {
