  ...
  <div class="settings-params-page">
    <div class="page-header">
      ...
      <div class="page-header-actions">
        <button class="btn btn-primary" @click="saveParams"><Icon name="save" :size="14" /> 保存参数</button>
      </div>
    </div>
    ...
    <div class="panel-card">
      ...
      <div class="panel-card-body">
        <div class="form-group">
          ...
          <div style="font-size:var(--font-size-xs);color:var(--color-text-tertiary);margin-top:var(--space-1)">加权平均：按所有批次加权计算均价，适合价格波动大的场景；FIFO：先入库的先出库，适合有保质期的物料</div>
        </div>
        <div class="form-group">
          <label class="form-label">会话超时(小时)</label>
          <input type="number" class="form-input" v-model.number="form.sessionTimeout" min="1" max="72">
        </div>
        <div class="form-group">
...
function saveParams() {
  if (form.taxRate < 0 || form.taxRate > 100) { alert('税率必须在0-100之间'); return }
  if (form.sessionTimeout < 1 || form.sessionTimeout > 72) { alert('会话超时必须在1-72小时之间'); return }
