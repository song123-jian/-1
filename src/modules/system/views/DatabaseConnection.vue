  ...
  <div class="db-connection-page">
    <div class="page-header">
      <div>
        ...
        <p class="page-header-subtitle">连接 Supabase 云数据库，开启团队数据共享与同步</p>
      </div>
...
function subscribeAllTables() {
  ...
  const storeMap = {
    ...
    suppliers: { store: supplierStore, dataKey: 'suppliers' }
  }
...
async function handlePullAll() {
  ...
  try {
    ...
    if (data.suppliers) supplierStore.replaceData(validateDataArray(data.suppliers, 'suppliers'))
  } finally {
    syncing.value = false
  }
}
...
async function handlePullOne(resourceName, storeRef) {
  ...
  const replaceMap = {
    ...
    suppliers: () => supplierStore.replaceData(data)
  }
...
CREATE TABLE suppliers (
  ...
  "deliveryScore" NUMERIC DEFAULT 70,
  "qualityScore" NUMERIC DEFAULT 70,
  "priceScore" NUMERIC DEFAULT 70,
  "serviceScore" NUMERIC DEFAULT 70,
...
CREATE POLICY "Auth users full access" ON suppliers FOR ALL USING (auth.role() = 'authenticated') WITH CHECK (auth.role() = 'authenticated');
