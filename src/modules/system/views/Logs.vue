  ...
  <div class="log-page">
    <div class="page-header">
      <div>
        ...
        <p class="page-header-subtitle">查看系统操作轨迹，便于审计、排查和回溯</p>
      </div>
      <div class="page-header-actions">
        <div style="position: relative; display: inline-block">
          ...
          >
            <div
              style="padding: var(--space-2) var(--space-3); cursor: pointer; font-size: var(--font-size-sm)"
              @click="(exportLogs('csv'), (showExportMenu = false))"
            >
              CSV
            </div>
            <div
              style="padding: var(--space-2) var(--space-3); cursor: pointer; font-size: var(--font-size-sm)"
              @click="(exportLogs('excel'), (showExportMenu = false))"
            >
              Excel
            </div>
            <div
              style="padding: var(--space-2) var(--space-3); cursor: pointer; font-size: var(--font-size-sm)"
              @click="(exportLogs('pdf'), (showExportMenu = false))"
            >
              PDF
            </div>
          </div>
    ...
    <div class="filter-bar" style="margin-bottom: var(--space-3)">
      ...
      />
      <select v-model="filters.module" class="form-select" style="width: auto; min-width: 120px">
        <option value="">全部模块</option>
        <option v-for="(label, key) in logStore.moduleLabels" :key="key" :value="key">{{ label }}</option>
      </select>
      <input v-model="filters.dateFrom" type="date" class="form-input" style="width: 140px" />
      <span style="color: var(--color-text-tertiary); align-self: center">至</span>
      <input v-model="filters.dateTo" type="date" class="form-input" style="width: 140px" />
      <select v-model="logUser" class="form-select">
      ...
      <div class="panel-card-body no-padding">
            ...
            <tbody>
              <tr v-if="filteredLogs.length === 0">
                <td colspan="7" class="empty-state">
                  <div class="empty-state-icon"><Icon name="log" :size="14" /></div>
                  暂无操作日志
                </td>
              </tr>
              <tr v-for="(l, idx) in pagedLogs" :key="l.time + l.action">
                ...
                <td style="max-width: 300px; overflow-wrap: break-word; word-wrap: break-word" :title="l.detail">
                  {{ l.detail || '-' }}
                </td>
              </tr>
            </tbody>
        ...
        >
          <div style="display: flex; align-items: center; gap: var(--space-2)">
            ...
            <select v-model="perPage" class="form-select" style="width: 80px; font-size: var(--font-size-xs)">
          ...
          <div style="display: flex; gap: var(--space-1); align-items: center">
            <button
              ...
              style="padding: 4px 8px; font-size: 12px; min-width: 28px"
              @click="currentPage = p"
            >
              {{ p }}
            </button>
...
function renderTrendChart() {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    ctx.beginPath()
    if (ctx.roundRect) {
      ctx.roundRect(x, h - 20 - barH, barW, barH, [3, 3, 0, 0])
    } else {
...
function renderModuleChart() {
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    ctx.beginPath()
    if (ctx.roundRect) {
      ctx.roundRect(90, y, barW, barH, 3)
    } else {
...
function exportLogs(format) {
  ...
  if (format === 'csv') {
    ...
    for (const l of list) {
      csv +=
        [l.time, l.user, l.action, logStore.moduleLabels[l.module] || l.module, l.detail]
    ...
    let xml =
      '<?xml version="1.0"?><?mso-application progid="Excel.Sheet"?><Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet" xmlns:ss="urn:schemas-microsoft-com:office:spreadsheet"><Worksheet ss:Name="操作日志"><Table><Row><Cell><Data ss:Type="String">时间</Data></Cell><Cell><Data ss:Type="String">用户</Data></Cell><Cell><Data ss:Type="String">操作</Data></Cell><Cell><Data ss:Type="String">模块</Data></Cell><Cell><Data ss:Type="String">详情</Data></Cell><Cell><Data ss:Type="String">敏感</Data></Cell></Cell></Row>'
    for (const l of list) {
      ...
      xml +=
        ...
        (l.action || '') +
        '</Data></Cell><Cell><Data ss:Type="String">' +
        (logStore.moduleLabels[l.module] || l.module || '') +
        '</Data></Cell><Cell><Data ss:Type="String">' +
        (l.detail || '') +
        '</Data></Cell><Cell><Data ss:Type="String">' +
        (isSensitive ? '<Icon name="warning" :size="14" />' : '') +
        '</Data></Cell></Row>'
    }
    ...
    let body =
      ...
      ' | 共 ' +
      list.length +
      ' 条</p><table><thead><tr><th>时间</th><th>用户</th><th>操作</th><th>模块</th><th>详情</th><th>敏感</th></tr></thead><tbody>'
    for (const l of list) {
      const isSensitive = isSensitiveAction(l.action)
      body +=
        '<tr><td>' +
        ...
        escapeHtml(l.action || '') +
        '</td><td>' +
        escapeHtml(logStore.moduleLabels[l.module] || l.module || '') +
        '</td><td>' +
        escapeHtml(l.detail || '') +
        '</td><td class="' +
