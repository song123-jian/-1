  ...
  <div>
    <div class="page-header" style="margin-bottom:var(--space-3)">
      ...
      <div class="page-header-actions">
        <button class="btn btn-primary" @click="openUserModal"><Icon name="add" :size="14" /> 新增用户</button>
      </div>
    </div>
    <div class="filter-bar" style="margin-bottom:var(--space-3);flex-wrap:wrap;gap:var(--space-2)">
      <input type="text" class="form-input" v-model="userSearch" placeholder="搜索用户名/姓名..." style="min-width:160px">
      <select class="form-select" v-model="userRoleFilter">
        <option value="">全部角色</option>
        <option value="admin">管理员</option><option value="gm">总经理</option><option value="sales_manager">销售主管</option>
        <option value="sales">销售员</option><option value="warehouse_manager">仓库主管</option><option value="warehouse_staff">仓管员</option>
        <option value="finance">财务</option><option value="viewer">查看者</option>
      </select>
      ...
      <div class="panel-card-body no-padding">
        <div class="table-container">
          <table class="data-table">
            <thead><tr><th>用户名</th><th>姓名</th><th>角色</th><th>状态</th><th>最后登录</th><th>操作</th></tr></thead>
            <tbody>
              ...
              <tr v-for="user in filteredUsers" :key="user.id">
                ...
                <td><span class="status-badge" :class="user.status === 'active' ? 'success' : 'neutral'">{{ user.status === 'active' ? '启用' : '禁用' }}</span></td>
                <td>{{ user.lastLogin || '-' }}</td>
                <td>
                  <button class="btn btn-ghost btn-sm" @click="editUser(user)">编辑</button>
                  <button class="btn btn-ghost btn-sm" @click="toggleUserStatus(user)">{{ user.status === 'active' ? '禁用' : '启用' }}</button>
    ...
    <div class="panel-card" style="margin-top:var(--space-4)">
      <div class="panel-card-header"><span class="panel-card-title"><Icon name="shield" :size="14" /> 角色权限矩阵</span></div>
      <div class="panel-card-body" style="overflow-x:auto">
        <table class="data-table" style="font-size:var(--font-size-xs)">
          <thead>
            <tr>
              <th style="position:sticky;left:0;background:var(--color-surface-elevated);z-index:1">权限项</th>
              <th v-for="role in roleList" :key="role.key">{{ role.label }}</th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="perm in permissionList" :key="perm.key">
              <td style="position:sticky;left:0;background:var(--color-surface-elevated);z-index:1;font-weight:600;overflow-wrap:break-word;word-wrap:break-word">{{ perm.label }}</td>
              <td v-for="role in roleList" :key="role.key" style="text-align:center">
                <span v-if="rolePermissions[role.key]?.includes(perm.key)" style="color:var(--color-success)"><Icon name="checkCircle" :size="14" /></span>
    ...
    <div v-if="showUserModal" class="modal-overlay" @click.self="showUserModal = false">
      <div class="modal-dialog" @click.stop>
        <div class="modal-header"><span class="modal-title">{{ editingUser ? '编辑用户' : '新增用户' }}</span><button class="modal-close" @click="showUserModal = false"><Icon name="close" :size="14" /></button></div>
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <select class="form-select" v-model="userForm.role">
              <option>管理员</option><option>总经理</option><option>销售主管</option><option>销售员</option><option>仓库主管</option><option>仓管员</option><option>财务</option><option>查看者</option>
            </select>
          </div>
        </div>
...
const roleList = [
  { key: 'admin', label: '管理员' }, { key: 'gm', label: '总经理' },
  { key: 'sales_manager', label: '销售主管' }, { key: 'sales', label: '销售员' },
  { key: 'warehouse_manager', label: '仓库主管' }, { key: 'warehouse_staff', label: '仓管员' },
  { key: 'finance', label: '财务' }, { key: 'viewer', label: '查看者' }
]

const permissionList = [
  { key: 'create_quotation', label: '创建报价' }, { key: 'approve_quotation', label: '审批报价' },
  { key: 'delete_quotation', label: '删除报价' }, { key: 'create_contract', label: '创建合同' },
  { key: 'sign_contract', label: '签订合同' }, { key: 'create_inbound', label: '创建入库' },
  { key: 'confirm_inbound', label: '确认入库' }, { key: 'create_outbound', label: '创建出库' },
  { key: 'approve_outbound', label: '审批出库' }, { key: 'create_reconciliation', label: '创建对账' },
  { key: 'confirm_reconciliation', label: '确认对账' }, { key: 'create_delivery', label: '创建送货' },
  { key: 'view_cost', label: '查看成本' }, { key: 'edit_cost', label: '编辑成本' }
]

const rolePermissions = {
  admin: permissionList.map(p => p.key),
  gm: ['create_quotation','approve_quotation','delete_quotation','create_contract','sign_contract','create_inbound','confirm_inbound','create_outbound','approve_outbound','create_reconciliation','confirm_reconciliation','create_delivery','view_cost','edit_cost'],
...
const filteredUsers = computed(() => {
  ...
  if (userSearch.value) {
    ...
    users = users.filter(u => (u.username || '').toLowerCase().includes(q) || (u.realName || '').toLowerCase().includes(q))
  }
  return users
})
...
.role-badge { display: inline-block; padding: var(--space-1) var(--space-2); background: var(--color-info-subtle); color: var(--color-info); border-radius: var(--radius-full); font-size: var(--font-size-xs); }
.form-input, .form-select { padding: var(--space-2) var(--space-2); border: 1px solid var(--color-border); border-radius: var(--radius-md); font-size: var(--font-size-sm); background: var(--color-surface); color: var(--color-text-primary); }
