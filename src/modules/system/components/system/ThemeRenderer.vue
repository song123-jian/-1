    ...
    <div v-for="group in section.groups" :key="group.title" class="tr-group">
      <div class="panel-card" :class="{ 'tr-group--no-header-toggle': !group.headerToggle }">
        <div class="panel-card-header">
          ...
          <div v-if="group.headerActions" class="panel-card-actions">
            <button
              v-for="a in group.headerActions"
              :key="a.action"
          ...
          <label v-if="group.headerToggle" class="toggle-switch">
            <input
              type="checkbox"
              :checked="getValue(group.headerToggle.key)"
              @change="setValue(group.headerToggle.key, $event.target.checked)"
            />
          ...
          <template v-for="(field, idx) in visibleFields(group.fields)" :key="field.key || field.type + idx">
            ...
            <template v-else-if="field.type === 'range'">
              <div class="form-group">
                <label class="form-label">
                  {{ field.label }}
                  <span class="value-badge">{{ formatRangeValue(getValue(field.key), field) }}</span>
                </label>
                <input
                  type="range"
                  ...
                  :step="field.step || 1"
                  class="range-slider"
                  @input="setValue(field.key, parseFloat($event.target.value))"
                />
            ...
            <template v-else-if="field.type === 'livePreview'">
              <div class="live-preview-box">
                ...
                <div
                  class="preview-subtle"
                  :style="{ borderLeftColor: settings.accentColor, background: 'var(--color-surface-elevated)' }"
                >
                  主色强调信息
                </div>
              </div>
            ...
            <template v-else-if="field.type === 'contrastDisplay'">
              <div class="form-group">
                ...
                <div class="contrast-row">
                  <span class="contrast-badge" :class="{ pass: whiteContrastRatio >= 4.5 }">
                    白底对比度: {{ whiteContrast }}
                  </span>
                  <span class="contrast-badge" :class="{ pass: blackContrastRatio >= 4.5 }">
                    黑底对比度: {{ blackContrast }}
                  </span>
                </div>
              </div>
            ...
            <template v-else-if="field.type === 'colorSwatches'">
              <div class="form-group">
                ...
                <div class="swatch-grid">
                  <div
                    v-for="s in field.swatches"
                    :key="s.color"
                    class="color-swatch"
                    :style="{ background: s.color }"
                    :title="s.name"
                    @click="setValue(field.targetKey, s.color)"
                  ></div>
                </div>
              </div>
            </template>
            ...
            <template v-else-if="field.type === 'accentPreview'">
              <div class="live-preview-box">
                ...
                <div class="preview-tags">
                  ...
                  <span
                    class="preview-tag"
                    :style="{ background: settings.accentColor + '15', color: settings.accentColor }"
                  >
                    状态标签
                  </span>
                </div>
            ...
            <template v-else-if="field.type === 'typographyPreview'">
              <div class="typography-preview">
                <h1
                  class="tp-h1"
                  :style="{
                    fontSize: `calc(var(--font-size-2xl) * ${settings.fontScale / 100})`,
                    marginBottom: `calc(var(--space-3) * ${settings.spacingScale / 100})`
                  }"
                >
                  一级标题示例
                </h1>
                <h2
                  class="tp-h2"
                  :style="{
                    fontSize: `calc(var(--font-size-xl) * ${settings.fontScale / 100})`,
                    marginBottom: `calc(var(--space-3) * ${settings.spacingScale / 100})`
                  }"
                >
                  二级标题示例
                ...
                <p
                  class="tp-body"
                  :style="{
                    fontSize: `calc(var(--font-size-sm) * ${settings.fontScale / 100})`,
                    lineHeight: settings.lineHeight,
                    marginBottom: `calc(var(--space-3) * ${settings.spacingScale / 100})`
                  }"
                >
                  正文文本示例，用于展示排版间距和字体缩放效果。正文文本示例，用于展示排版间距和字体缩放效果。
                </p>
                <div
                  class="tp-card"
                  :style="{
                    padding: `calc(${settings.layoutPadding}rem * ${settings.spacingScale / 100})`,
                    borderRadius: `calc(${settings.componentRadius}rem)`,
                    marginBottom: `calc(var(--space-3) * ${settings.spacingScale / 100})`
                  }"
                >
                  <div
                    class="tp-card-title"
                    :style="{ fontSize: `calc(var(--font-size-sm) * ${settings.fontScale / 100})` }"
                  >
                    卡片标题
                  </div>
                  <div
                    class="tp-card-desc"
                    :style="{ fontSize: `calc(var(--font-size-xs) * ${settings.fontScale / 100})` }"
                  >
                    卡片描述文本信息
                  </div>
                </div>
            ...
            <template v-else-if="field.type === 'splashGrid'">
              <div class="form-group">
                ...
                <div class="splash-grid">
                  <button
                    v-for="s in field.options"
                    :key="s.name"
                    class="splash-style-btn"
                    :class="{ active: settings.splash === s.name }"
                    @click="setValue(field.key, s.name)"
                  >
                    <div
            ...
            <template v-else-if="field.type === 'presetGrid'">
              <div class="preset-grid">
                <div
                  v-for="preset in field.options"
                  :key="preset.name"
                  class="preset-card"
                  :class="{ active: store.currentTheme === preset.name }"
                  @click="store.setTheme(preset.name)"
                >
            ...
            <template v-else-if="field.type === 'comparisonTable'">
              <div class="table-container">
                <table class="comparison-table">
                  <thead>
                    <tr>
                      <th v-for="col in field.columns" :key="col.key" :class="{ 'tc-center': col.format === 'stars' }">
                        {{ col.label }}
                      </th>
                    </tr>
                  </thead>
                  <tbody>
                    <tr v-for="row in field.rows" :key="row.name">
                      <td v-for="col in field.columns" :key="col.key" :class="{ 'tc-center': col.format === 'stars' }">
                        ...
                        <span v-if="col.format === 'stars'" class="star-rating">
                          {{ '★'.repeat(row[col.key]) }}{{ '☆'.repeat(5 - row[col.key]) }}
                        </span>
            ...
            <template v-else-if="field.type === 'colorSystem'">
              <div class="color-system-grid">
                ...
                <div class="color-system-card">
                  ...
                  <div class="color-system-swatches">
                    <div
                      v-for="c in ['#22c55e', '#f59e0b', '#ef4444', '#3b82f6']"
                      :key="c"
                      class="color-system-swatch"
                      :style="{ background: c }"
                    ></div>
                ...
                <div class="color-system-card">
                  ...
                  <div class="color-system-swatches">
                    <div
                      v-for="s in accentSwatches.slice(0, 8)"
                      :key="s.color"
                      class="color-system-swatch"
                      :style="{ background: s.color }"
                    ></div>
            ...
            <template v-else-if="field.type === 'customPreset'">
              <div class="custom-preset-row">
                <div class="form-group" style="margin-bottom: 0; flex: 1; min-width: 200px">
                  ...
                  <input v-model="customPresetName" type="text" class="form-input" placeholder="输入预设名称" />
                </div>
                <button class="btn btn-primary" @click="saveCustomPreset">保存当前样式</button>
                <div class="form-group" style="margin-bottom: 0; flex: 1; min-width: 200px">
                  <label class="form-label">加载预设</label>
                  <select v-model="loadPresetSelect" class="form-select" style="width: 100%">
...
<script>
export default { name: 'ThemeRenderer' }
</script>
<script setup>
...
function setValue(key, value) {
  if (!key) return
  if (typeof props.store.patchSetting === 'function') {
    props.store.patchSetting(key, value)
    return
  }
...
const spCardStyle = computed(() => ({
  ...
  boxShadow: props.settings.shadowEnabled
    ? `0 4px ${(props.settings.shadowIntensity / 100) * 20}px rgba(0,0,0,${(props.settings.shadowIntensity / 100) * 0.5})`
    : 'none'
}))
