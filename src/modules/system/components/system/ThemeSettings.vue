...
function resetThemeToDefault() {
  ...
  layoutPaddingRaw.value = 16
  layoutGapRaw.value = 16
  componentRadiusRaw.value = 8
}
...
// 监听themeSettings深层变化，同步到DOM和localStorage
watch(themeSettings, () => {
  themeStore.applyToDocument()
  themeStore.persist()
}, { deep: true })
...
// ============ Store同步 ============
watch(() => themeStore.themeSettings.brightness, (val) => { themeBrightness.value = val })
watch(() => themeStore.themeSettings.accentColor, (val) => { themeAccentColor.value = val })
watch(() => themeStore.themeSettings.accentHover, (val) => { themeAccentHover.value = val })
watch(() => themeStore.themeSettings.fontScale, (val) => { fontScale.value = val })
watch(() => themeStore.themeSettings.lineHeight, (val) => { lineHeightRaw.value = Math.round(val * 10) })
watch(() => themeStore.themeSettings.spacingScale, (val) => { spacingScale.value = val })
watch(() => themeStore.themeSettings.layoutPadding, (val) => { layoutPaddingRaw.value = Math.round(val * 16) })
watch(() => themeStore.themeSettings.layoutGap, (val) => { layoutGapRaw.value = Math.round(val * 16) })
watch(() => themeStore.themeSettings.componentRadius, (val) => { componentRadiusRaw.value = Math.round(val * 16) })
