    ...
    <div class="panel-card-body">
      <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:var(--space-3);margin-bottom:var(--space-4)">
        <div v-for="item in autoSaveItems" :key="item.name" style="background:var(--color-surface-elevated);padding:var(--space-3);border-radius:var(--radius-md)">
          ...
          <div style="font-size:var(--font-size-sm);font-weight:600;margin-top:2px" :style="{ color: item.status === '已保存' ? 'var(--color-success)' : 'var(--color-warning)' }">
            {{ item.status }}
          </div>
      ...
      <div style="margin-top:var(--space-4);display:flex;gap:var(--space-3)">
        <button class="btn btn-secondary btn-sm" @click="handleRecoverLatest">
          <Icon name="refresh" :size="14" /> 从最新快照恢复
        </button>
      </div>
    </div>
  </div>
