<template>
  <div>
    ...
    <DataBackup @showImport="showImportDialog = true" />
    <DataClear @exportBackup="handleExportBackup" />
    <BackupRecords />
    <AutoSavePanel />
    <OpHistory />
    <SystemHealth />
      ...
      <div class="modal-dialog" @click.stop>
        ...
        <div class="modal-body">
          <div style="background:var(--color-warning-subtle);color:var(--color-warning);padding:var(--space-3);border-radius:var(--radius-md);margin-bottom:var(--space-4);font-size:var(--font-size-sm)">
            <Icon name="warning" :size="14" /> 导入将覆盖当前所有数据，请确保已备份！
          </div>
          <div class="form-group">
            <label class="form-label">选择JSON文件</label>
            <input type="file" style="width:100%;color:var(--color-text-secondary)" @change="handleImportDialogFile">
          </div>
        </div>
