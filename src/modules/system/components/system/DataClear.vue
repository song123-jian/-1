  ...
  <div class="panel-card" style="margin-bottom: var(--space-4)">
    ...
    <div class="panel-card-body">
      ...
      <div
        ...
        "
      >
        清除范围：客户、报价、库存、出入库、送货、回款、对账、采购、供应商、物料表、项目、营业执照、开票资料、盘点、成本核算、审批规则、待办事项、通知、操作日志、客户交互记录、标签等全部业务数据。
      </div>
...
async function handleClear() {
  if (!confirm('确认清除全部数据？此操作不可恢复。')) {
    console.info('[DataClear] 用户取消第一次确认')
    return
  }
  if (!confirm('再次确认：真的要删除所有数据吗？')) {
    console.info('[DataClear] 用户取消第二次确认')
    return
  }

  isClearing.value = true
  console.info('[DataClear] 开始清除全部数据')

  const localStorageBefore = localStorage.length
  localStorage.clear()
  console.info(`[DataClear] localStorage 已清除，原键数量: ${localStorageBefore}`)

  const sessionStorageBefore = safeSessionStorage ? safeSessionStorage.length : 0
  if (safeSessionStorage) {
    safeSessionStorage.clear()
  }
  console.info(`[DataClear] sessionStorage 已清除，原键数量: ${sessionStorageBefore}`)

  try {
    await new Promise((resolve) => {
      const request = indexedDB.deleteDatabase('dp-erp-storage')
      request.onsuccess = () => {
        console.info('[DataClear] IndexedDB 删除成功')
        resolve()
      }
      request.onerror = () => {
      ...
      setTimeout(() => {
        console.info('[DataClear] IndexedDB 删除超时，继续执行')
        resolve()
      }, 1000)
    })
  } catch (error) {
    console.warn('[DataClear] 清除 IndexedDB 异常:', error)
  }
  ...
  console.info(`[DataClear] 已重建 ${allInitFlags.length} 组初始化标记`)
