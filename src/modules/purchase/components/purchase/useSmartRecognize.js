...
export function useSmartRecognize(form) {
  function parsePurchaseInfo(text) {
    ...
    for (const pattern of CommonPatterns.contactName) {
      ...
      if (match) { pushItem(makeItem('contactName', '联系人', match[1], 70)); break }
    }

    const phoneMatch = text.match(CommonPatterns.phone)
    if (phoneMatch) pushItem(makeItem('phone', '电话', phoneMatch[0], 90))

    const dateMatch = text.match(/(?:到货日|交货日|预计到货|交货期|交付日)[:\s：]*(\d{4}[-/年]\d{1,2}[-/月]\d{1,2}[日]?)/)
    if (dateMatch) {
  ...
  return useSmartRecognizeBase(form, parsePurchaseInfo, '粘贴采购信息或表格数据（支持Excel复制），AI将自动识别并提取关键字段和明细行...')
}
