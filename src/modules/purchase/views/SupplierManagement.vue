  ...
  <div class="supplier-management-page">
    ...
    <div class="page-header">
      <div class="page-header-row">
        <div>
          ...
          <p class="page-header-subtitle">统一维护供应商档案、评分、合作状态和重复数据</p>
        </div>
        <div class="page-header-actions">
          ...
          >
            ...
            发现 {{ duplicateGroups.length }} 组重复数据
          </button>
        ...
        <div class="overview-ring-body">
          <svg width="80" height="80" viewBox="0 0 80 80" class="overview-ring-svg">
            ...
            <circle
              ...
              r="30"
              fill="none"
              :stroke="activeRateColor"
              stroke-width="5"
              stroke-linecap="round"
              :stroke-dasharray="activeRateDash"
              stroke-dashoffset="0"
        ...
        <div class="overview-rating-body">
          ...
          <div class="overview-rating-stars">
            <Icon
              ...
              name="star"
              :size="16"
              :style="{ color: i <= Math.round(avgCompositeScore / 20) ? '#f59e0b' : 'var(--color-border)' }"
            />
          </div>
          <div class="overview-rating-dims">
            <div class="rating-dim">
              <span class="rating-dim-dot" style="background: #3b82f6"></span>
              交货 {{ avgDelivery.toFixed(0) }}
            </div>
            <div class="rating-dim">
              <span class="rating-dim-dot" style="background: #22c55e"></span>
              质量 {{ avgQuality.toFixed(0) }}
            </div>
            <div class="rating-dim">
              <span class="rating-dim-dot" style="background: #f59e0b"></span>
              价格 {{ avgPrice.toFixed(0) }}
            </div>
            <div class="rating-dim">
              <span class="rating-dim-dot" style="background: #a855f7"></span>
              服务 {{ avgService.toFixed(0) }}
            </div>
          </div>
        </div>
      </div>
    ...
    <div v-if="expiringQualifications.length > 0" class="panel-card alert-panel">
      ...
      <div class="panel-card-body">
        <div
          v-for="item in expiringQualifications"
          :key="item.id"
          class="alert-qual-item"
          :class="{ 'alert-qual-urgent': item.daysLeft <= 30 }"
        >
          ...
          <span class="alert-qual-badge" :class="item.daysLeft <= 30 ? 'badge-danger' : 'badge-warning'">
            剩余 {{ item.daysLeft }} 天
          </span>
        </div>
      </div>
    ...
    <div class="panel-card">
      <div class="panel-card-header">
        ...
        <div style="display: flex; align-items: center; gap: var(--space-2)">
          ...
          <div class="view-toggle">
            <button
              class="btn btn-sm"
              :class="currentView === 'table' ? 'btn-outline active' : 'btn-ghost'"
            ...
            <button
              class="btn btn-sm"
              :class="currentView === 'card' ? 'btn-outline active' : 'btn-ghost'"
      ...
      <div v-if="currentView === 'table'" class="table-container">
        <table v-if="paginatedList.length > 0" class="data-table">
          <thead>
            <tr>
            ...
            <tr v-for="(item, index) in paginatedList" :key="item.id">
              ...
              <td>
                <div class="action-cell">
                  <button class="action-btn" title="查看详情" @click="openDetail(item)">
                    <Icon name="eye" :size="14" />
                  </button>
                  <button class="action-btn" title="编辑" @click="openEditModal(item)">
                    <Icon name="edit" :size="14" />
                  </button>
                  <button
                    class="action-btn"
                    :title="item.status === 'blacklist' ? '移出黑名单' : '加入黑名单'"
                    @click="handleToggleBlacklist(item)"
                  >
                    <Icon :name="item.status === 'blacklist' ? 'checkCircle' : 'shield'" :size="14" />
                  </button>
                  <button class="action-btn danger" title="删除" @click="openDeleteConfirm(item)">
          ...
          <div class="supplier-card-body">
            ...
            <div class="supplier-card-scores">
              <div class="mini-score">
                <span class="mini-score-dot" style="background: #3b82f6"></span>
                交货 {{ item.deliveryScore }}
              </div>
              <div class="mini-score">
                <span class="mini-score-dot" style="background: #22c55e"></span>
                质量 {{ item.qualityScore }}
              </div>
              <div class="mini-score">
                <span class="mini-score-dot" style="background: #f59e0b"></span>
                价格 {{ item.priceScore }}
              </div>
              <div class="mini-score">
                <span class="mini-score-dot" style="background: #a855f7"></span>
                服务 {{ item.serviceScore }}
              </div>
            </div>
          </div>
        </div>
    ...
    <Teleport to="body">
      <div v-if="showDetailModal" class="modal-overlay" @click.self="showDetailModal = false">
        <div class="modal-dialog modal-lg">
          ...
          <div v-if="detailSupplier" class="modal-body">
            <div class="detail-grid">
              ...
              <div class="detail-item">
                ...
                <span class="detail-value">
                  <span class="tag-badge">{{ detailSupplier.category }}</span>
                </span>
              </div>
              <div class="detail-item">
              ...
              <div class="detail-item">
                ...
                <span class="detail-value">
                  <span class="status-badge" :class="statusClass(detailSupplier.status)">
                ...
                <table class="data-table">
                  ...
                  <tbody>
                    <tr v-for="po in purchaseHistory" :key="po.id">
                      ...
                      <td>
                        <span class="status-badge" :class="poStatusClass(po.status)">
      ...
      <div v-if="showDeleteConfirm" class="modal-overlay" @click.self="showDeleteConfirm = false">
        <div class="modal-dialog" style="max-width: 420px">
          ...
          <div class="modal-footer">
            ...
            <button class="btn btn-danger" :disabled="!!deleteWarning" @click="handleDelete">确认删除</button>
          </div>
        </div>
      </div>
    </Teleport>
        ...
        <div class="modal-dialog" style="max-width: 600px">
          ...
          <div class="modal-body">
            <p style="margin-bottom: var(--space-3); color: var(--color-text-secondary)">
              检测到 {{ duplicateGroups.length }} 组相似供应商：
            </p>
...
<script>
export default { name: 'SupplierManagement' }
</script>
<script setup>
...
import {
  usePurchaseStore,
  STATUS_LABELS as PO_STATUS_LABELS,
  STATUS_COLORS as PO_STATUS_COLORS
} from '@/modules/purchase/stores/purchase'
...
const categoryStats = computed(() => {
  const map = {}
  supplierStore.suppliers.forEach((s) => {
...
const avgDelivery = computed(() => {
  const list = supplierStore.suppliers
  return list.length ? list.reduce((s, x) => s + (x.deliveryScore || 0), 0) / list.length : 0
})
const avgQuality = computed(() => {
  const list = supplierStore.suppliers
  return list.length ? list.reduce((s, x) => s + (x.qualityScore || 0), 0) / list.length : 0
})
const avgPrice = computed(() => {
  const list = supplierStore.suppliers
  return list.length ? list.reduce((s, x) => s + (x.priceScore || 0), 0) / list.length : 0
})
const avgService = computed(() => {
  const list = supplierStore.suppliers
  return list.length ? list.reduce((s, x) => s + (x.serviceScore || 0), 0) / list.length : 0
})
const avgCompositeScore = computed(
  () => avgDelivery.value * 0.3 + avgQuality.value * 0.35 + avgPrice.value * 0.2 + avgService.value * 0.15
)
...
function compositeScore(item) {
  return (
    (item.deliveryScore || 0) * 0.3 +
    (item.qualityScore || 0) * 0.35 +
    (item.priceScore || 0) * 0.2 +
    (item.serviceScore || 0) * 0.15
  )
}
...
.alert-qual-item:hover {
  background: var(--color-surface-hover);
}
.alert-qual-item:last-child {
  border-bottom: none;
}
.alert-qual-name {
  font-weight: 600;
