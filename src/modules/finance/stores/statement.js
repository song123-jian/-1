...
export const useStatementStore = defineStore('statement', () => {
  ...
  const pendingCount = computed(() => statements.value.filter((s) => s.status === 'pending').length)

  const confirmedCount = computed(() => statements.value.filter((s) => s.status === 'confirmed').length)

  const paidCount = computed(() => statements.value.filter((s) => s.status === 'paid').length)
  ...
  const totalAmount = computed(() =>
    statements.value.filter((s) => s.status !== 'voided').reduce((sum, s) => sum + (s.totalAmount || 0), 0)
  )

  const totalPaid = computed(() =>
    statements.value.filter((s) => s.status !== 'voided').reduce((sum, s) => sum + (s.paidAmount || 0), 0)
  )
  ...
  function updateStatement(id, updates) {
    ...
    if (idx !== -1) {
      ...
      if (updates.totalAmount !== undefined) {
        statements.value[idx].balance = (updates.totalAmount || 0) - (statements.value[idx].paidAmount || 0)
      }
      safeSetJSON(STORAGE_KEY, statements.value)
    }
  }
        ...
        if (balance > 0) {
          ...
          if (daysSinceConfirm > termDays) {
            alerts.push({
              ...
              priority: 'high',
              message:
                s.statementNo + ' 付款逾期 ' + (daysSinceConfirm - termDays) + ' 天，未结 ¥' + balance.toFixed(2),
              statementNo: s.statementNo,
  ...
  function analyzeDiscrepancies(statementId, transactions) {
    ...
    const matchedTxns = (transactions || []).filter(
      (t) => t.customerId === stmt.buyerId && t.date >= periodStart && t.date <= periodEnd && t.type !== 'collection'
    )
    ...
    statements.value = [
      {
        ...
        taxAmount: 3646.5,
        totalAmount: 31696.5,
        totalChinese: numberToChinese(31696.5),
        paidAmount: 10000,
        balance: 21696.5,
        paymentMethod: '银行转账',
        paymentTerm: '月结30天',
      ...
      {
        id: 'fs7',
        statementNo: 'DZ-20241220-001',
        type: '月度对账单',
        period: '2024-12',
        reconDate: '2024-12-20',
        preparer: 'admin',
        reviewer: '',
        contactPhone: '',
        buyerId: 'c7',
        buyerName: '上海贸易有限公司浦东分公司',
        buyerAddress: '',
        buyerContact: '',
        buyerPhone: '',
        buyerEmail: '',
        ...
        taxAmount: 2333.5,
        totalAmount: 20283.5,
        totalChinese: numberToChinese(20283.5),
        paidAmount: 0,
        balance: 20283.5,
        paymentMethod: '银行转账',
        paymentTerm: '月结30天',
      ...
      {
        id: 'fs8',
        statementNo: 'DZ-20241225-001',
        type: '月度对账单',
        period: '2024-12',
        reconDate: '2024-12-25',
        preparer: 'admin',
        reviewer: '',
        contactPhone: '',
        buyerId: 'c9',
        buyerName: '深圳智能制造股份公司',
        buyerAddress: '',
        buyerContact: '',
        buyerPhone: '',
        buyerEmail: '',
        ...
        taxAmount: 3510,
        totalAmount: 30510,
        totalChinese: numberToChinese(30510),
        paidAmount: 0,
        balance: 30510,
        paymentMethod: '银行承兑',
        paymentTerm: '月结60天',
