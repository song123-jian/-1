  ...
  <div class="statement-page">
    <div class="page-header">
      <div>
        ...
        <p class="page-header-subtitle">统一生成、核对和确认对账单，减少反复查账和手工对比</p>
      </div>
      <div class="page-header-actions">
        ...
        <button class="btn btn-outline" :disabled="selectedIds.length === 0" @click="handleBatchPrint">打印所选</button>
        <button
          v-if="canDelete"
          class="btn btn-outline"
        ...
        <button v-if="canCreate" class="btn btn-primary" @click="openEditor()">新建对账单</button>
      </div>
    </div>
    ...
    >
      <div class="panel-card-header">
          <span class="panel-card-title">异常提醒</span>
      </div>
...
function handleConfirm(id) {
  showConfirmDialog('确认操作', '确认此对账单吗？', () => {
    statementStore.confirmStatement(id)
  })
}

function handleVoid(id) {
  showConfirmDialog('确认操作', '确认作废此对账单吗？', () => {
...
function handleReopen(id) {
  showConfirmDialog('确认操作', '确认重新打开此对账单吗？', () => {
    statementStore.reopenStatement(id)
  })
}

function handleDelete(id) {
  showConfirmDialog('确认删除', '确认删除此对账单吗？删除后不可恢复。', () => {
  ...
  printWin.document.write(`
    ...
    <html lang="zh-CN">
      <head>
        ...
        <style>
          body {
            font-family: ${CJK_SANS_FONT_STACK};
            padding: 24px;
            color: #333;
            font-size: 12px;
          }
      ...
      <body>
        ...
        <div class="section">
          ...
          <table>
            <tr>
              <td><strong>对账单号：</strong>${escapeHtml(stmt.statementNo || '')}</td>
              <td><strong>期间：</strong>${escapeHtml(formatPeriod(stmt.period))}</td>
            </tr>
            <tr>
              <td><strong>采购方：</strong>${escapeHtml(stmt.buyerName || '-')}</td>
              <td><strong>供应商：</strong>${escapeHtml(stmt.sellerName || '-')}</td>
            </tr>
          </table>
        </div>
        <div class="section">
        ...
        <div class="section">
          ...
          <div class="amounts">
            <div><strong>小计：</strong>¥${(stmt.subtotal || 0).toFixed(2)}</div>
            <div><strong>税率：</strong>${stmt.taxRate || 0}%</div>
            <div><strong>税额：</strong>¥${(stmt.taxAmount || 0).toFixed(2)}</div>
            <div class="amount-total"><strong>合计：</strong>¥${(stmt.totalAmount || 0).toFixed(2)}</div>
          </div>
        </div>
...
function handleBatchDelete() {
  ...
  showConfirmDialog('批量删除', `确定删除选中的 ${selectedIds.value.length} 条对账单吗？`, () => {
