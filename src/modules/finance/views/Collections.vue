  ...
  <div class="collection-page">
    ...
    <div class="fund-pipeline">
      <div class="pipeline-stage">
        ...
        <span class="pipeline-stage-value">¥{{ formatMoney(collectionStore.totalCollected + collectionStore.totalPending) }}</span>
      </div>
      <div class="pipeline-arrow">→</div>
      <div class="pipeline-stage">
        <span class="pipeline-stage-label">累计回款</span>
        <span class="pipeline-stage-value" style="color:var(--color-success)">¥{{ formatMoney(collectionStore.totalCollected) }}</span>
      </div>
      <div class="pipeline-arrow">→</div>
      <div class="pipeline-stage">
        <span class="pipeline-stage-label">待回款</span>
        <span class="pipeline-stage-value" style="color:var(--color-warning)">¥{{ formatMoney(collectionStore.totalPending) }}</span>
      </div>
      <div class="pipeline-arrow">→</div>
      <div class="pipeline-stage pipeline-stage-danger">
        <span class="pipeline-stage-label">逾期金额</span>
        <span class="pipeline-stage-value" style="color:var(--color-danger)">¥{{ formatMoney(collectionStore.totalOverdue) }}</span>
      </div>
    </div>
    ...
    <div class="stats-row stats-grid-4">
      <div class="stat-card" style="animation-delay:0ms">
        ...
        <div class="stat-card-value" style="color:var(--color-success)">¥{{ formatMoney(collectionStore.totalCollected) }}</div>
      </div>
      <div class="stat-card" style="animation-delay:60ms">
        <div class="stat-card-header">
          <span class="stat-card-label">待回款</span>
          <div class="stat-dot-halo"><span class="alert-dot-pulse" style="background:var(--color-warning)"></span></div>
        </div>
        <div class="stat-card-value" style="color:var(--color-warning)">¥{{ formatMoney(collectionStore.totalPending) }}</div>
      </div>
      ...
      <div class="stat-card" style="animation-delay:180ms">
        <div class="stat-card-header">
          ...
          <div class="stat-dot-halo"><span class="alert-dot-pulse" style="background:var(--color-danger)"></span></div>
        </div>
        <div class="stat-card-value" style="color:var(--color-danger)">¥{{ formatMoney(collectionStore.totalOverdue) }}</div>
      </div>
    </div>
    ...
    <div class="overview-row">
      ...
      <div class="overview-card" style="animation-delay:60ms">
        ...
        <div class="overview-card-body">
          <div class="method-bar-row" v-for="m in methodDistribution" :key="m.key">
      ...
      <div class="overview-card" style="animation-delay:120ms">
        <div class="overview-card-title">近7日回款趋势</div>
        <div class="overview-card-body">
          <div class="trend-chart">
            <div class="trend-bar-wrapper" v-for="(d, i) in last7DaysTrend" :key="i">
    ...
    <div v-if="alertCollections.length > 0" class="alert-panel">
      <div class="alert-panel-header">
        <div class="alert-dot-halo"><span class="alert-dot-pulse" style="background:var(--color-danger)"></span></div>
            ...
            <tbody>
              <tr v-if="filteredCollections.length === 0">
                <td colspan="10" class="empty-state">
                  <div class="empty-state-icon"><Icon name="empty" :size="28" /></div>暂无回款记录
                </td>
              </tr>
              <tr
              ...
              >
                <td style="width:50px;text-align:center;overflow-wrap:break-word;word-wrap:break-word">{{ i + 1 }}</td>
