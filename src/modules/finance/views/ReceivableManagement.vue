  ...
  <div class="receivable-page">
    <div class="page-header">
      <div>
        ...
        <p class="page-header-subtitle">统一跟踪应收账款、回款进度和账龄风险</p>
      </div>
    ...
    <div class="fund-pipeline">
      <div class="pipeline-node">
        <div class="pipeline-node-amount">{{ formatMoney(receivableStore.totalAmount || 0) }}</div>
        <div class="pipeline-node-label">应收总额</div>
      </div>
...
import ReceiptFormModal from '@/modules/finance/components/finance/ReceiptFormModal.vue'
...
const agingMatrixData = computed(() => {
  ...
  for (const item of items) {
    ...
    const customer = item.customerName || item.customer || '未知'
    if (!customerMap[customer]) {
      customerMap[customer] = { customer, d30: 0, d60: 0, d90: 0, d90plus: 0 }
    }
    ...
    if (days <= 30) customerMap[customer].d30 += amount
    else if (days <= 60) customerMap[customer].d60 += amount
    else if (days <= 90) customerMap[customer].d90 += amount
    else customerMap[customer].d90plus += amount
  }
