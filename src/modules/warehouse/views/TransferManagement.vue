<template>
  <div class="transfer-page">
    ...
    <div class="flow-board">
      <template v-for="(step, idx) in flowBoardSteps" :key="step.status">
        <div
          class="flow-board-node"
          :class="{ active: statusFilter === step.status }"
          @click="toggleFlowBoardNode(step.status)"
        >
          <span class="flow-board-dot" :class="step.dotClass"></span>
          <span class="flow-board-label">{{ step.label }}</span>
          <span class="flow-board-count">{{ step.count }}</span>
        </div>
        <span v-if="idx < flowBoardSteps.length - 1" class="flow-board-arrow">
    ...
    <div class="collapsible-stats">
      <div class="collapsible-stats-header" @click="showTransferStatsExpanded = !showTransferStatsExpanded">
        <span class="collapsible-stats-title">
          <Icon name="barChart" :size="14" />
          统计与概览
        </span>
        <span class="collapsible-stats-toggle" :class="{ expanded: showTransferStatsExpanded }">
          <Icon name="chevronDown" :size="12" />
        </span>
      </div>
      <div v-show="showTransferStatsExpanded" class="collapsible-stats-body">
        <!-- 统计卡片 -->
        <div class="stats-grid stats-grid-4">
          ...
          <div class="stat-card" style="animation-delay: 120ms">
            ...
            <div class="stat-card-value">
              <span class="stat-dot-halo" style="background: var(--color-info)"></span>
              {{ transferStore.inTransitCount }}
            </div>
          </div>
        ...
        <div v-if="pendingAlerts.length > 0" class="panel-card transfer-alert-panel">
          ...
          <div class="panel-card-body">
            ...
            >
              ...
              <span class="transfer-alert-route">
                {{ a.fromWarehouseName }}
          ...
          <tbody>
            <tr v-if="pagedOrders.length === 0">
              <td colspan="11" class="empty-state">
                <div class="empty-state-icon"><Icon name="shuffle" :size="24" /></div>
                暂无调拨单
              </td>
            </tr>
    ...
    <!-- 新增/编辑弹窗 -->
    <TransferFormModal v-if="showFormModal" @close="showFormModal = false" @created="handleCreated" />

    <!-- 预览弹窗 -->
    <TransferPreview v-if="previewOrder" :order="previewOrder" @close="previewOrder = null" />
...
<script>
export default { name: 'TransferManagement' }
</script>
<script setup>
import { ref, computed, onMounted, watch } from 'vue'
import { useTransferStore } from '@/modules/warehouse/stores/transfer'
import { formatMoney } from '@/utils/format'
import TransferFormModal from '@/modules/warehouse/components/inventory/TransferFormModal.vue'
import TransferPreview from '@/modules/warehouse/components/inventory/TransferPreview.vue'
...
const showTransferStatsExpanded = ref(false)
...
/* ====== 流程看板 ====== */
const flowBoardSteps = computed(() => {
  const orders = transferStore.transferOrders || []
  const countByStatus = (status) => orders.filter((o) => o.status === status).length
  return [
    { status: 'draft', label: STATUS_LABELS.draft, count: countByStatus('draft'), dotClass: 'draft' },
    { status: 'pending', label: STATUS_LABELS.pending, count: countByStatus('pending'), dotClass: 'pending' },
    { status: 'approved', label: STATUS_LABELS.approved, count: countByStatus('approved'), dotClass: 'progress' },
    { status: 'in_transit', label: STATUS_LABELS.in_transit, count: countByStatus('in_transit'), dotClass: 'progress' },
    { status: 'completed', label: STATUS_LABELS.completed, count: countByStatus('completed'), dotClass: 'completed' }
  ]
})

function toggleFlowBoardNode(status) {
...
const filteredOrders = computed(() => {
  ...
  if (searchText.value) {
    ...
    list = list.filter(
      (o) => (o.orderNo || '').toLowerCase().includes(kw) || (o.notes || '').toLowerCase().includes(kw)
    )
  }
  return list
})
...
const pendingAlerts = computed(() => {
  return (transferStore.transferOrders || [])
    ...
    .sort((a, b) => {
      const dateA = a.expectedDate ? new Date(a.expectedDate) : new Date('9999-12-31')
      const dateB = b.expectedDate ? new Date(b.expectedDate) : new Date('9999-12-31')
      return dateA - dateB
    })
    .slice(0, 5)
})
...
.transfer-alert-item:last-child {
  border-bottom: none;
}
