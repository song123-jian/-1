...
const showLocationStatsExpanded = ref(false)
...
function parseLocationInfo(text) {
  ...
  const codeMatch = text.match(/(?:库位编码|库位号|编号)[:\s：]*([A-Za-z0-9_-]{4,30})/)
  if (codeMatch) pushItem('locationCode', '库位编码', codeMatch[1], 90)

  const warehouseMatch = text.match(/(?:仓库名称|仓库)[:\s：]*([^\n\r]{2,30})/)
  if (warehouseMatch) pushItem('warehouseName', '仓库名称', warehouseMatch[1].trim(), 85)

  const areaMatch = text.match(/(?:库区名称|库区|区域)[:\s：]*([^\n\r]{2,20})/)
  if (areaMatch) pushItem('areaName', '库区名称', areaMatch[1].trim(), 80)

  const managerMatch = text.match(/(?:库管员|管理员|负责人)[:\s：]*([^\n\r]{2,10})/)
  if (managerMatch) pushItem('manager', '库管员', managerMatch[1].trim(), 80)

  const phoneMatch = text.match(/(?:联系电话|电话|手机)[:\s：]*((?:1[3-9]\d{9})|(?:\d[\d-]{6,18}\d))/)
  if (phoneMatch) pushItem('managerPhone', '联系电话', phoneMatch[1].trim(), 85)

  const notesMatch = text.match(/(?:澶囨敞|璇存槑|瑙ｉ噴)[:\s：]*([^\n\r]{3,120})/)
  if (notesMatch) pushItem('notes', '澶囨敞', notesMatch[1].trim(), 60)
...
const {
  ...
  resetSmartRec
} = useSmartRecognizeBase(formData.value, parseLocationInfo, '粘贴库位信息或表格文本，系统将自动识别并提取库位、仓库、区域、管理员和电话。')
...
function openEditModal(loc) {
  ...
  Object.assign(formData.value, loc)
  formErrors.value = []
  resetSmartRec()
  showModal.value = true
}
