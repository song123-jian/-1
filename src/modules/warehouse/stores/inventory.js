...
const SUPPLIER_KEY = STORAGE_PREFIX + 'inventorySuppliers'
...
export const useInventoryStore = defineStore('inventory', () => {
  ...
  const lowStockCount = computed(() => enrichedInventory.value.filter((i) => i.alertStatus === 'low').length)
  const exhaustedCount = computed(() => enrichedInventory.value.filter((i) => i.alertStatus === 'exhausted').length)
  const overStockCount = computed(() => enrichedInventory.value.filter((i) => i.alertStatus === 'over').length)
  const normalStockCount = computed(() => enrichedInventory.value.filter((i) => i.alertStatus === 'ok').length)
  const totalStockWeight = computed(() => enrichedInventory.value.reduce((s, i) => s + i.stock, 0))
  const totalStockValue = computed(() => enrichedInventory.value.reduce((s, i) => s + i.totalValue, 0))
  ...
  function lookupSupplier(idOrCode) {
    ...
    return (
      suppliers.value.find((s) => s.id === idOrCode || s.supplierCode === idOrCode || s.shortName === idOrCode) || null
    )
  }
  ...
  function updateInventoryItem(id, updates) {
    ...
    if (idx !== -1) {
      ...
      inventory.value[idx].totalValue =
        (parseFloat(inventory.value[idx].quantity) || 0) * (parseFloat(inventory.value[idx].unitCost) || 0)
      inventory.value[idx].status =
        computeAlertStatus(inventory.value[idx]) === 'ok' ? 'normal' : computeAlertStatus(inventory.value[idx])
      persist()
    }
  }
  ...
  function adjustStock(itemCode, delta, direction, reason, itemId) {
    ...
    inventory.value[invIdx].totalValue =
      inventory.value[invIdx].quantity * (parseFloat(inventory.value[invIdx].unitCost) || 0)
    inventory.value[invIdx].status =
      computeAlertStatus(inventory.value[invIdx]) === 'ok' ? 'normal' : computeAlertStatus(inventory.value[invIdx])
    persist()
    return true
  }
  ...
  function submitInboundOrder(data) {
    ...
    const order = {
      ...
      qualityStatus:
        items.length > 0
          ? items.every((it) => it.qualityStatus === 'passed')
            ? 'passed'
            : items.some((it) => it.qualityStatus === 'failed')
              ? 'failed'
              : 'pending'
          : 'pending',
      createdAt: new Date().toISOString(),
  ...
  function confirmInbound(orderId) {
    ...
    for (const item of items) {
      ...
      if (invIdx !== -1) {
        ...
        inventory.value[invIdx].totalValue =
          inventory.value[invIdx].quantity * (parseFloat(inventory.value[invIdx].unitCost) || 0)
        inventory.value[invIdx].lastInboundDate = order.date || toLocalDateStr()
        inventory.value[invIdx].status =
          computeAlertStatus(inventory.value[invIdx]) === 'ok' ? 'normal' : computeAlertStatus(inventory.value[invIdx])
      } else {
        newItems.push(item)
    ...
    if (newItems.length > 0) {
      for (const ni of newItems) {
        ...
        inventory.value.push({
          ...
          color: ni.color || '',
          brand: '',
          lastInboundDate: order.date || toLocalDateStr(),
          createdAt: new Date().toISOString()
        })
      }
    }
  ...
  function copyInboundOrder(id) {
    ...
    persistOrders()
    addAuditLog('create', 'inbound', '复制入库单: ' + newOrder.orderNo, {
      orderNo: newOrder.orderNo,
      sourceOrderNo: src.orderNo
    })
    return newOrder
  }
  ...
  function reverseOutboundOrder(orderId) {
    ...
    const reverseOrder = {
      ...
      grade: order.grade || '',
      color: order.color || '',
      outDate: toLocalDateStr(),
      date: toLocalDateStr(),
      outQty: -order.outQty,
      totalQuantity: -order.outQty,
      unitPrice: order.unitPrice || 0,
      outAmount: -(order.outAmount || 0),
      batchNo: order.batchNo || '',
      referenceId: order.outboundNo,
      outStatus: 'confirmed',
      status: 'confirmed',
  ...
  function initSeedData() {
    ...
    warehouseOrders.value = [
      {
        id: 'w1',
        orderNo: 'RK20260519001',
        type: 'purchase',
        referenceId: 'PO-2024-0078',
        counterpartyId: 's1',
        counterpartyName: '江苏钢铁集团',
        supplierCode: 'SUP-001',
        warehouseId: 'main',
        date: '2026-05-19',
        totalQuantity: 320,
      ...
      {
        id: 'w2',
        orderNo: 'CK20241212001',
        type: 'sales',
        outType: 'sales',
        outboundNo: 'CK20241212001',
        referenceId: 'QT20241201001',
        counterpartyId: 'c2',
        counterpartyName: '北京科技发展集团',
      ...
      {
        id: 'w3',
        orderNo: 'RK20260518001',
        type: 'customer_return',
        counterpartyId: 'c4',
        counterpartyName: '深圳智能制造有限公司',
        warehouseId: 'main',
        date: '2026-05-18',
        totalQuantity: 80,
        status: 'inspecting',
      ...
      {
        id: 'w4',
        orderNo: 'RK20260519002',
        type: 'purchase',
        referenceId: 'PO-2026-0032',
        counterpartyId: 's3',
        counterpartyName: '广东有色金属有限公司',
        supplierCode: 'SUP-003',
        warehouseId: 'main',
        date: '2026-05-19',
        totalQuantity: 500,
  ...
  function mergeInboundOrders(items) {
    ...
    const existingInbound = warehouseOrders.value.filter(
      (o) => inboundTypes.includes(o.type) && !o.outType && !o.outboundNo
    )
  ...
  function mergeOutboundOrders(items) {
    if (!Array.isArray(items)) return
    const inboundTypes = ['purchase', 'return', 'transfer', 'customer_return', 'production_return', 'surplus', 'gift']
    const inboundOnly = warehouseOrders.value.filter(
      (o) => inboundTypes.includes(o.type) && !o.outType && !o.outboundNo
    )
    const existingOutbound = warehouseOrders.value.filter(
      (o) => !inboundTypes.includes(o.type) || o.outType || o.outboundNo
    )
  ...
  function reverseInboundOrder(orderId) {
    ...
    for (const item of items) {
      ...
      const invIdx = inventory.value.findIndex((i) => i.code === item.code)
      if (invIdx !== -1) {
        inventory.value[invIdx].quantity = Math.max(
          0,
          (parseFloat(inventory.value[invIdx].quantity) || 0) - (item.qty || 0)
        )
        inventory.value[invIdx].totalValue =
          inventory.value[invIdx].quantity * (parseFloat(inventory.value[invIdx].unitCost) || 0)
        inventory.value[invIdx].status =
