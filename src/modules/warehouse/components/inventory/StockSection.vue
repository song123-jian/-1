  ...
  <div class="stock-page">
    ...
    <div class="compact-metrics">
      ...
      <div class="compact-metric">
        ...
        <span class="compact-metric-value" :style="{ color: healthScore >= 80 ? 'var(--color-success)' : healthScore >= 50 ? 'var(--color-warning)' : 'var(--color-danger)' }">{{ healthScore }}%</span>
      </div>
    </div>
    ...
    <div class="collapsible-stats">
      <div class="collapsible-stats-header" @click="showStockStatsExpanded = !showStockStatsExpanded">
        <span class="collapsible-stats-title"><Icon name="chart" :size="14" /> 详细统计与概览</span>
      ...
      <div class="inv-stats-row">
        ...
        <div class="inv-stat-card" style="animation-delay:120ms">
          ...
          <div>
            <div class="inv-stat-value"><span class="stat-dot-halo" style="background:var(--color-danger)"></span>{{ inventoryStore.exhaustedCount }}</div>
        ...
        <div class="inv-stat-card" style="animation-delay:180ms">
          ...
          <div>
            <div class="inv-stat-value"><span class="stat-dot-halo" style="background:var(--color-warning)"></span>{{ inventoryStore.lowStockCount }}</div>
        ...
        <div class="overview-card overview-ring-card">
          ...
          <div class="overview-ring-body">
            <svg width="72" height="72" viewBox="0 0 72 72" class="overview-ring-svg">
              ...
              <circle cx="36" cy="36" r="26" fill="none" :stroke="healthScoreColor" stroke-width="5" stroke-linecap="round"
                :stroke-dasharray="healthScoreDash" stroke-dashoffset="0" transform="rotate(-90 36 36)" class="overview-ring-progress" />
            </svg>
          ...
          <div class="alert-bars">
            <div class="alert-bar-item">
              ...
              <div class="alert-bar-track">
                <div class="alert-bar-fill" style="background:var(--color-danger)" :style="{ width: alertPercent('exhausted') + '%' }"></div>
              </div>
            ...
            <div class="alert-bar-item">
              ...
              <div class="alert-bar-track">
                <div class="alert-bar-fill" style="background:var(--color-warning)" :style="{ width: alertPercent('low') + '%' }"></div>
              </div>
            ...
            <div class="alert-bar-item">
              ...
              <div class="alert-bar-track">
                <div class="alert-bar-fill" style="background:var(--color-info)" :style="{ width: alertPercent('over') + '%' }"></div>
              </div>
            ...
            <div class="alert-bar-item">
              ...
              <div class="alert-bar-track">
                <div class="alert-bar-fill" style="background:var(--color-success)" :style="{ width: alertPercent('ok') + '%' }"></div>
              </div>
      ...
      <div v-if="criticalAlerts.length > 0" class="panel-card stock-alert-panel">
        <div class="panel-card-header">
          <span class="panel-card-title" style="color:var(--color-danger)"><span class="alert-dot-pulse-danger"></span> 库存预警</span>
        </div>
        <div class="panel-card-body">
          <div v-for="(a, idx) in criticalAlerts" :key="a.id" class="stock-alert-item" :style="{ animationDelay: idx * 60 + 'ms' }">
            <span class="stock-alert-badge" :class="'alert-' + a.alertStatus">{{ inventoryStore.ALERT_STATUS_MAP[a.alertStatus] }}</span>
            <span class="stock-alert-name">{{ a.name }}</span>
            <span class="stock-alert-code">{{ a.code }}</span>
            <span class="stock-alert-stock">库存: <strong :style="{ color: alertColor(a.alertStatus) }">{{ a.stock.toFixed(1) }}</strong>kg</span>
    ...
    <div class="inv-toolbar">
            ...
            <table class="inv-table">
              ...
              <tbody>
                <tr v-if="paginatedInventory.length === 0">
                  <td colspan="13" class="empty-cell"><div class="empty-state-icon"><Icon name="package" :size="24" /></div>暂无库存数据</td>
                </tr>
                <tr v-for="(item, idx) in paginatedInventory" :key="item.id" :style="{ animationDelay: idx * 20 + 'ms' }">
                  ...
                  <td style="width:50px;text-align:center;overflow-wrap:break-word;word-wrap:break-word">{{ (stockPage - 1) * stockPageSize + idx + 1 }}</td>
      ...
      <div v-if="stockView === 'list'" class="panel-card" style="margin-top:var(--space-3)">
        ...
        <div class="panel-card-body no-padding">
          <div v-for="(item, idx) in paginatedInventory" :key="item.id" class="stock-list-item" :style="{ animationDelay: idx * 50 + 'ms' }" @click="emit('edit-item', item)">
          ...
          <div v-if="filteredInventory.length === 0" class="empty-cell" style="padding:var(--space-8)"><div class="empty-state-icon"><Icon name="package" :size="24" /></div>暂无库存数据</div>
        </div>
      </div>
      ...
      <div v-if="stockView === 'card'" style="margin-top:var(--space-3)">
        ...
        <div class="stock-card-grid">
          <div v-for="(item, idx) in paginatedInventory" :key="item.id" class="stock-card-item" :style="{ animationDelay: idx * 60 + 'ms' }" @click="emit('edit-item', item)">
  ...
  for (let i = 0; i < 6; i++) {
    ...
    for (let j = 0; j < 7; j++) {
      ...
      for (const di of dayItems.slice(0, 2)) {
        html += '<div style="font-size:10px;color:var(--color-text-tertiary);;overflow-wrap:break-word;word-wrap:break-word">' + escapeHtml(di.name) + '</div>'
      }
