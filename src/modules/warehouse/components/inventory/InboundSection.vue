  ...
  <div class="inbound-page">
    <div class="inbound-header">
      <div class="inbound-header-info">
        ...
        <div class="inbound-header-actions">
          ...
          <div class="view-toggle">
            <button class="view-toggle-btn" :class="{ active: inboundView === 'table' }" @click="inboundView = 'table'" title="表格视图">
              <Icon name="chart" :size="14" />
              <span>表格</span>
            </button>
            <button class="view-toggle-btn" :class="{ active: inboundView === 'list' }" @click="inboundView = 'list'" title="列表视图">
              <Icon name="list" :size="14" />
              <span>列表</span>
            </button>
            <button class="view-toggle-btn" :class="{ active: inboundView === 'card' }" @click="inboundView = 'card'" title="卡片视图">
              <Icon name="card" :size="14" />
              <span>卡片</span>
            </button>
            <button class="view-toggle-btn" :class="{ active: inboundView === 'calendar' }" @click="inboundView = 'calendar'" title="日历视图">
      ...
      <div class="flow-board">
        <div class="flow-board-node" :class="{ active: inboundStatusFilter === 'pending' }" @click="inboundStatusFilter = inboundStatusFilter === 'pending' ? '' : 'pending'">
        ...
        <div class="flow-board-node" :class="{ active: inboundStatusFilter === 'inspecting' }" @click="inboundStatusFilter = inboundStatusFilter === 'inspecting' ? '' : 'inspecting'">
          <span class="flow-board-dot progress"></span>
          <div>
            <div class="flow-board-count">{{ inventoryStore.inboundOrders.filter(o => o.status === 'inspecting').length }}</div>
            <div class="flow-board-label">质检中</div>
          </div>
        </div>
        <span class="flow-board-arrow">→</span>
        <div class="flow-board-node" :class="{ active: inboundStatusFilter === 'confirmed' }" @click="inboundStatusFilter = inboundStatusFilter === 'confirmed' ? '' : 'confirmed'">
          <span class="flow-board-dot completed"></span>
          <div>
            <div class="flow-board-count">{{ inventoryStore.inboundOrders.filter(o => o.status === 'confirmed').length }}</div>
                ...
                <table class="inv-table">
                  ...
                  <tbody>
                    <tr v-if="paginatedInbound.length === 0">
                      <td colspan="11" class="empty-cell"><div class="empty-state-icon"><Icon name="upload" :size="24" /></div>暂无入库记录</td>
                    </tr>
                    <tr v-for="(order, idx) in paginatedInbound" :key="order.id" :class="{ 'row-confirmed': order.status === 'confirmed' }" @dblclick="handleDoubleClickInbound(order)">
                      <td><input type="checkbox" :value="order.id" v-model="inboundSelectedIds" /></td>
                      <td style="width:50px;text-align:center;overflow-wrap:break-word;word-wrap:break-word">{{ (inboundPage - 1) * inboundPageSize + idx + 1 }}</td>
  ...
  for (let i = 0; i < 6; i++) {
    ...
    for (let j = 0; j < 7; j++) {
      ...
      for (const di of dayItems.slice(0, 2)) {
        html += '<div style="font-size:10px;color:var(--color-text-tertiary);;overflow-wrap:break-word;word-wrap:break-word">' + escapeHtml(di.orderNo) + '</div>'
      }
...
function handlePrintInbound(order) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    totalQty += (it.qty || 0)
    totalAmt += amt
    itemRows += '<tr><td>' + (i + 1) + '</td><td>' + escapeHtml(it.code || '') + '</td><td>' + escapeHtml(it.name || '') + '</td><td>' + escapeHtml(it.grade || '') + '</td><td>' + escapeHtml(it.color || '') + '</td><td>' + (it.qty || 0).toFixed(2) + '</td><td>' + (it.cost || 0).toFixed(2) + '</td><td style="font-weight:bold;overflow-wrap:break-word;word-wrap:break-word">' + amt.toFixed(2) + '</td><td>' + escapeHtml(it.batch || '') + '</td></tr>'
  }
  const printWindow = window.open('', '_blank', 'width=800,height=600')
  printWindow.document.write('<!DOCTYPE html><html><head><meta charset="UTF-8"><title>入库单 - ' + escapeHtml(order.orderNo || '') + '</title><style>body{font-family:"SimSun","Microsoft YaHei",serif;font-size:12pt;line-height:1.8;color:#000;margin:0;padding:20mm}table{width:100%;border-collapse:collapse;margin:var(--space-2) 0}th{border:1px solid #333;padding:var(--space-2) var(--space-2);text-align:center;font-size:10.5pt; overflow-wrap: break-word; word-wrap: break-word}td{border:1px solid #333;padding:var(--space-2) var(--space-2);text-align:center;font-size:10.5pt; overflow-wrap: break-word; word-wrap: break-word}th{background:#f0f0f0;font-weight:bold}.title{text-align:center;font-size:20pt;font-weight:bold;letter-spacing:6px;margin-bottom:var(--space-1)}.sub{text-align:center;font-size:10pt;color:#555;margin-bottom:var(--space-5)}.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:var(--space-1) var(--space-5);font-size:10.5pt;margin:var(--space-2) 0}.sign{margin-top:var(--space-10);display:flex;justify-content:space-between;font-size:10.5pt}.sign-line{display:inline-block;width:140px;border-bottom:1px solid #333}.total-row td{font-weight:bold;background:#f8f8f8; overflow-wrap: break-word; word-wrap: break-word}@page{size:A4;margin:15mm}</style></head><body>')
  printWindow.document.write('<div class="title">苏州冠久新材料科技有限公司</div><div class="title" style="font-size:16pt">入 库 单</div><div class="sub">入库单号: ' + escapeHtml(order.orderNo || '') + '</div>')
  printWindow.document.write('<div class="info-grid"><div>入库类型: ' + escapeHtml(typeLabel) + '</div><div>入库日期: ' + escapeHtml(order.date || '-') + '</div><div>供应商: ' + escapeHtml(order.counterpartyName || '-') + '</div><div>供应商编码: ' + escapeHtml(order.supplierCode || '-') + '</div><div>状态: ' + (inventoryStore.INBOUND_STATUS_LABELS[order.status] || escapeHtml(order.status || '')) + '</div><div>备注: ' + escapeHtml(order.notes || '-') + '</div></div>')
  printWindow.document.write('<table><thead><tr><th>序号</th><th>编号</th><th>物料名称</th><th>牌号</th><th>颜色</th><th>数量(kg)</th><th>单价(元/kg)</th><th>金额(元)</th><th>批次号</th></tr></thead><tbody>')
...
function handleDownloadImportTemplate() {
  const headers = ['入库类型', '入库日期', '供应商编码', '编号', '物料名称', '牌号', '颜色', '数量(kg)', '单价(元/kg)', '批次号', '备注']
...
/* 流程看板 */
.flow-board { display: flex; align-items: center; gap: var(--space-3); margin-bottom: var(--space-4); padding: var(--space-3) var(--space-4); background: var(--color-bg-primary); border: 1px solid var(--color-border); border-radius: var(--radius-lg)}
.flow-board-node { display: flex; align-items: center; gap: var(--space-2); padding: var(--space-2) var(--space-3); border-radius: var(--radius-md); cursor: pointer; transition: all 0.2s}
.flow-board-node:hover { background: var(--color-bg-tertiary)}
.flow-board-node.active { background: var(--color-accent-subtle)}
.flow-board-dot { width: 10px; height: 10px; border-radius: 50%; flex-shrink: 0}
.flow-board-dot.pending { background: var(--color-warning)}
.flow-board-dot.progress { background: var(--color-info)}
.flow-board-dot.completed { background: var(--color-success)}
.flow-board-count { font-size: var(--font-size-lg); font-weight: 700; font-family: var(--font-mono); line-height: 1}
.flow-board-label { font-size: var(--font-size-xs); color: var(--color-text-secondary)}
.flow-board-arrow { color: var(--color-text-tertiary); font-size: var(--font-size-lg)}

/* 折叠统计区 */
.collapsible-stats { margin-bottom: var(--space-3); border: 1px solid var(--color-border); border-radius: var(--radius-lg); overflow: hidden}
.collapsible-stats-header { display: flex; align-items: center; justify-content: space-between; padding: var(--space-2) var(--space-4); background: var(--color-bg-secondary); cursor: pointer; user-select: none; transition: background 0.2s}
.collapsible-stats-header:hover { background: var(--color-bg-tertiary)}
.collapsible-stats-title { font-size: var(--font-size-sm); font-weight: 600; color: var(--color-text-secondary); display: flex; align-items: center; gap: var(--space-2)}
.collapsible-stats-toggle { font-size: var(--font-size-xs); color: var(--color-text-tertiary); transition: transform 0.3s ease}
.collapsible-stats-toggle.expanded { transform: rotate(180deg)}
.collapsible-stats-body { padding: var(--space-4); border-top: 1px solid var(--color-border)}
...
.trend-bars { display: flex; align-items: flex-end; gap: var(--space-2); height: 80px; padding-top: var(--space-1)}
.trend-bar-col { flex: 1; display: flex; flex-direction: column; align-items: center; gap: var(--space-1); height: 100%}
...
.alert-dot-pulse { display: inline-block; width: 8px; height: 8px; border-radius: 50%; background: var(--color-warning); animation: alertDotPulse 1.5s ease-in-out infinite; margin-right: var(--space-1)}
...
.inbound-alert-badge { padding: var(--space-1) var(--space-2); border-radius: var(--radius-full); font-size: var(--font-size-xs); font-weight: 600}
...
.inv-toolbar { display: flex; justify-content: flex-start; align-items: center; gap: var(--space-3); margin-bottom: var(--space-3); flex-wrap: wrap}
.inv-search { position: relative; min-width: 200px; max-width: 360px}
.inv-search .search-icon { position: absolute; left: 10px; top: 50%; transform: translateY(-50%); color: var(--color-text-tertiary)}
.inv-search .search-input { width: 100%; padding-left: var(--space-8)}
...
.inbound-tab-bar .btn { border: none; border-bottom: 2px solid transparent; margin-bottom: -var(--space-1); border-radius: var(--radius-md) var(--radius-md) 0 0; padding: var(--space-2) var(--space-3); font-family: var(--font-family); font-size: var(--font-size-base)}
...
.inv-table th{padding: var(--space-2) var(--space-3); text-align: left; border-bottom: 1px solid var(--color-border); overflow-wrap: break-word; word-wrap: break-word}
.inv-table td {padding: var(--space-2) var(--space-3); text-align: left; border-bottom: 1px solid var(--color-border); overflow-wrap: break-word; word-wrap: break-word}
.inv-table th { background: var(--color-bg-secondary); font-weight: 600; color: var(--color-text-secondary); position: sticky; top: 0}
.inv-table-sm th{padding: var(--space-1) var(--space-2); font-size: var(--font-size-xs); overflow-wrap: break-word; word-wrap: break-word}
.inv-table-sm td {padding: var(--space-1) var(--space-2); font-size: var(--font-size-xs); overflow-wrap: break-word; word-wrap: break-word}
.inv-table tr:hover td {background: var(--color-bg-tertiary); overflow-wrap: break-word; word-wrap: break-word}
...
.cell-actions {}
.cell-actions .btn { margin-right: var(--space-1)}
...
.row-confirmed td {background: rgba(16, 185, 129, 0.04); overflow-wrap: break-word; word-wrap: break-word}

.status-badge { display: inline-block; padding: var(--space-1) var(--space-2); border-radius: 9999px; font-size: var(--font-size-xs); font-weight: 600}
...
.form-input-sm { padding: var(--space-1) var(--space-2); font-size: var(--font-size-xs)}
...
.form-error { font-size: var(--font-size-sm); color: var(--color-danger); padding: var(--space-1) 0; display: flex; align-items: center; gap: var(--space-1)}
...
.cell-material-select .data-select-trigger { min-height: 28px; padding: var(--space-1) var(--space-6) var(--space-1) var(--space-2); font-size: var(--font-size-xs)}
...
.cal-table th{border: 1px solid var(--color-border); padding: var(--space-1) var(--space-2); vertical-align: top; min-height: 60px; height: 80px; overflow-wrap: break-word; word-wrap: break-word}
.cal-table td {border: 1px solid var(--color-border); padding: var(--space-1) var(--space-2); vertical-align: top; min-height: 60px; height: 80px; overflow-wrap: break-word; word-wrap: break-word}
...
.recycle-bin-table th{padding: var(--space-2) var(--space-3); border-bottom: 1px solid var(--color-border); text-align: left; overflow-wrap: break-word; word-wrap: break-word}
.recycle-bin-table td {padding: var(--space-2) var(--space-3); border-bottom: 1px solid var(--color-border); text-align: left; overflow-wrap: break-word; word-wrap: break-word}
