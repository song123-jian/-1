  ...
  <div class="inbound-page">
    <div class="inbound-header">
      ...
      <div class="inbound-header-actions">
        <button class="btn btn-ghost btn-sm" @click="openRecycleBinModal">
        ...
        <div class="view-toggle">
          <button
            ...
            :class="{ active: inboundView === 'table' }"
            title="表格视图"
            @click="inboundView = 'table'"
          >
          ...
          <button
            class="view-toggle-btn"
            :class="{ active: inboundView === 'list' }"
            title="列表视图"
            @click="inboundView = 'list'"
          >
          ...
          <button
            class="view-toggle-btn"
            :class="{ active: inboundView === 'card' }"
            title="卡片视图"
            @click="inboundView = 'card'"
          >
          ...
          <button
            class="view-toggle-btn"
            :class="{ active: inboundView === 'calendar' }"
            title="日历视图"
            @click="inboundView = 'calendar'"
          >
            <Icon name="calendar" :size="14" />
            <span>日历</span>
          </button>
        </div>
        <button class="btn btn-ghost btn-sm" @click="handleDownloadImportTemplate">
          <Icon name="upload" :size="14" />
          导入模板
        </button>
    ...
    <div class="flow-board">
      <div
        ...
        @click="inboundStatusFilter = inboundStatusFilter === 'pending' ? '' : 'pending'"
      >
        <span class="flow-board-dot pending"></span>
        <div>
          <div class="flow-board-count">{{ inventoryStore.pendingInboundCount }}</div>
      ...
      <div
        class="flow-board-node"
        :class="{ active: inboundStatusFilter === 'inspecting' }"
        @click="inboundStatusFilter = inboundStatusFilter === 'inspecting' ? '' : 'inspecting'"
      >
        <span class="flow-board-dot progress"></span>
        <div>
          <div class="flow-board-count">
            {{ inventoryStore.inboundOrders.filter((o) => o.status === 'inspecting').length }}
          </div>
          <div class="flow-board-label">质检中</div>
        </div>
      </div>
      <span class="flow-board-arrow">→</span>
      <div
        class="flow-board-node"
        :class="{ active: inboundStatusFilter === 'confirmed' }"
        @click="inboundStatusFilter = inboundStatusFilter === 'confirmed' ? '' : 'confirmed'"
      >
        <span class="flow-board-dot completed"></span>
        <div>
          <div class="flow-board-count">
            {{ inventoryStore.inboundOrders.filter((o) => o.status === 'confirmed').length }}
          </div>
          <div class="flow-board-label">已入库</div>
        </div>
      </div>
    </div>
    ...
    <div class="collapsible-stats">
      <div class="collapsible-stats-header" @click="showInboundStatsExpanded = !showInboundStatsExpanded">
        <span class="collapsible-stats-title">
          <Icon name="chart" :size="14" />
          统计与概览
        </span>
        <span class="collapsible-stats-toggle" :class="{ expanded: showInboundStatsExpanded }">▼</span>
      </div>
      <div v-show="showInboundStatsExpanded" class="collapsible-stats-body">
            ...
            <div class="overview-ring-body">
              <svg width="72" height="72" viewBox="0 0 72 72" class="overview-ring-svg">
                ...
                <circle
                  ...
                  r="26"
                  fill="none"
                  :stroke="confirmRateColor"
                  stroke-width="5"
                  stroke-linecap="round"
                  :stroke-dasharray="confirmRateDash"
                  stroke-dashoffset="0"
        ...
        <div v-if="pendingAlerts.length > 0" class="panel-card inbound-alert-panel">
          ...
          <div class="panel-card-body">
            <div
              ...
              :key="a.id"
              class="inbound-alert-item"
              :style="{ animationDelay: idx * 60 + 'ms' }"
            >
              <span class="inbound-alert-badge" :class="'status-' + a.status">
                {{ inventoryStore.INBOUND_STATUS_LABELS[a.status] }}
              </span>
    ...
    <div v-if="inboundSelectedIds.length > 0" class="inbound-bulk-bar">
      ...
      <button
        v-if="inboundSelectedPendingCount > 0 && canInspectInbound"
        class="btn btn-secondary btn-sm"
        @click="handleBatchApproveInbound"
      >
        批量审批({{ inboundSelectedPendingCount }})
      </button>
      <button
        v-if="inboundSelectedInspectingCount > 0 && canConfirmInbound"
        class="btn btn-primary btn-sm"
        @click="handleBatchConfirmInbound"
      >
        批量确认入库({{ inboundSelectedInspectingCount }})
      </button>
      <button v-if="canExportInbound" class="btn btn-ghost btn-sm" @click="handleExportSelectedInbound">
        导出选中
      </button>
    </div>

    <!-- 筛选+搜索栏 -->
    <div class="inv-toolbar" style="margin-bottom: var(--space-3)">
      <div class="inv-filters">
        ...
        <select v-model="inboundStatusFilter" class="form-select filter-select">
          ...
          <option v-for="(label, key) in inventoryStore.INBOUND_STATUS_LABELS" :key="key" :value="key">
            {{ label }}
          </option>
      ...
      <div class="inv-search" style="min-width: 200px; max-width: 360px">
    ...
    <div class="inbound-tab-bar">
      <button class="btn btn-sm" :class="{ active: inboundTab === 'main' }" @click="inboundTab = 'main'">
        入库单主表
      </button>
      <button class="btn btn-sm" :class="{ active: inboundTab === 'detail' }" @click="inboundTab = 'detail'">
        入库明细表
      </button>
    </div>
    ...
    <div v-if="inboundView === 'table'">
      <div v-if="inboundTab === 'main'">
                ...
                <thead>
                  <tr>
                    <th style="width: 40px">
                      <input
                        type="checkbox"
                        :checked="inboundSelectAll"
                        @change="toggleInboundSelectAll($event.target.checked)"
                      />
                    </th>
                ...
                <tbody>
                  <tr v-if="paginatedInbound.length === 0">
                    <td colspan="11" class="empty-cell">
                      <div class="empty-state-icon"><Icon name="upload" :size="24" /></div>
                      暂无入库记录
                    </td>
                  </tr>
                  <tr
                    v-for="(order, idx) in paginatedInbound"
                    :key="order.id"
                    :class="{ 'row-confirmed': order.status === 'confirmed' }"
                    @dblclick="handleDoubleClickInbound(order)"
                  >
                    <td><input v-model="inboundSelectedIds" type="checkbox" :value="order.id" /></td>
                    <td style="width: 50px; text-align: center; overflow-wrap: break-word; word-wrap: break-word">
                      {{ (inboundPage - 1) * inboundPageSize + idx + 1 }}
                    </td>
                    <td v-if="inboundColumnVisible.orderNo" class="cell-mono">
                    ...
                    <td v-if="inboundColumnVisible.totalWeight" class="cell-mono">
                      {{ calcInboundWeight(order).toFixed(2) }}
                    </td>
                    <td v-if="inboundColumnVisible.totalAmount" class="cell-mono">
                      {{ formatNumber(calcInboundAmount(order)) }}
                    </td>
                    <td v-if="inboundColumnVisible.status">
                      <span class="status-badge" :class="'status-' + order.status">
                        {{ inventoryStore.INBOUND_STATUS_LABELS[order.status] || order.status }}
                      </span>
                    </td>
                    <td class="cell-actions">
                      <template v-if="order.status === 'draft'">
                        <button
                          ...
                          @click="handleChangeInboundStatus(order.id, 'pending')"
                        >
                          提交审核
                        </button>
                      ...
                      <template v-else-if="order.status === 'pending'">
                        <button
                          v-if="canInspectInbound"
                          class="btn btn-ghost btn-sm"
                          style="color: var(--color-info)"
                          @click="handleChangeInboundStatus(order.id, 'inspecting')"
                        >
                          审批
                        </button>
                      </template>
                      <template v-else-if="order.status === 'inspecting'">
                        <button
                          ...
                          @click="handleConfirmInbound(order.id)"
                        >
                          确认入库
                        </button>
                      </template>
                      ...
                      <template v-if="order.status === 'confirmed'">
                        ...
                        <button
                          ...
                          @click="handleReverseInbound(order.id)"
                        >
                          冲销
                        </button>
                      </template>
                      <button
                        v-if="(order.status === 'draft' || order.status === 'pending') && canDeleteInbound"
                        class="btn btn-ghost btn-sm"
                        style="color: var(--color-danger)"
                        @click="handleDeleteInbound(order.id)"
                      >
                        删除
                      </button>
                    </td>
        ...
        >
          ...
          <div style="display: flex; align-items: center; gap: var(--space-1)">
            ...
            <button
              v-for="p in inboundVisiblePages"
              :key="p"
              class="btn btn-ghost btn-sm"
              :class="{ 'btn-primary': p === inboundPage }"
              style="min-width: 28px"
              @click="inboundPage = p"
            >
              {{ p }}
            </button>
            <button class="btn btn-ghost btn-sm" :disabled="inboundPage >= inboundTotalPages" @click="inboundPage++">
              &rsaquo;
            </button>
            <button
              class="btn btn-ghost btn-sm"
              :disabled="inboundPage >= inboundTotalPages"
              @click="inboundPage = inboundTotalPages"
            >
              &raquo;
            </button>
          </div>
          <div style="display: flex; align-items: center; gap: var(--space-2)">
            <button v-if="canExportInbound" class="btn btn-ghost btn-sm" @click="handleExport">
              <Icon name="upload" :size="14" />
              导出Excel
            </button>
            <button
              v-if="canDeleteInbound && inboundSelectedIds.length > 0"
              class="btn btn-ghost btn-sm"
              style="color: var(--color-danger)"
              @click="handleBatchDeleteInbound"
            >
              <Icon name="delete" :size="14" />
              批量删除({{ inboundSelectedIds.length }})
            </button>
          </div>
        </div>
      </div>
      ...
      <div v-if="inboundTab === 'detail'">
              ...
              <table class="inv-table">
                ...
                <tbody>
                  ...
                  <tr v-for="d in paginatedInboundDetails" :key="d.id">
                    ...
                    <td class="cell-mono">
                      <strong style="color: var(--color-accent)">{{ d.orderNo }}</strong>
                    </td>
                    ...
                    <td class="cell-actions">
                      <button
                        class="btn btn-ghost btn-sm"
                        @click="viewInboundDetail(inventoryStore.inboundOrders.find((o) => o.id === d.orderId))"
                      >
                        详情
                      </button>
                    </td>
        ...
        <div v-if="inboundDetailTotalPages > 1" class="pagination-bar">
          <span class="page-info">
            第 {{ inboundDetailPage }} / {{ inboundDetailTotalPages }} 页，共 {{ filteredInboundDetails.length }} 条
          </span>
          <div class="page-btns">
            <button class="btn btn-ghost btn-sm" :disabled="inboundDetailPage <= 1" @click="inboundDetailPage = 1">
              &laquo;
            </button>
            <button class="btn btn-ghost btn-sm" :disabled="inboundDetailPage <= 1" @click="inboundDetailPage--">
              &lsaquo;
            </button>
            <button
              v-for="p in inboundDetailVisiblePages"
              :key="p"
              class="btn btn-ghost btn-sm"
              :class="{ 'btn-primary': p === inboundDetailPage }"
              style="min-width: 28px"
              @click="inboundDetailPage = p"
            >
              {{ p }}
            </button>
            <button
              class="btn btn-ghost btn-sm"
              :disabled="inboundDetailPage >= inboundDetailTotalPages"
              @click="inboundDetailPage++"
            >
              &rsaquo;
            </button>
            <button
              class="btn btn-ghost btn-sm"
              :disabled="inboundDetailPage >= inboundDetailTotalPages"
              @click="inboundDetailPage = inboundDetailTotalPages"
            >
              &raquo;
            </button>
          </div>
    ...
    <div v-else-if="inboundView === 'list'">
      <div class="panel-card">
        ...
        <div class="panel-card-body">
          <div v-if="filteredInboundOrders.length === 0" class="empty-cell">
            <div class="empty-state-icon"><Icon name="upload" :size="24" /></div>
            暂无入库记录
          </div>
          <div
      ...
      <div v-if="inboundTotalPages > 1" class="pagination-bar">
        <span class="page-info">
          第 {{ inboundPage }} / {{ inboundTotalPages }} 页，共 {{ filteredInboundOrders.length }} 条
        </span>
        <div class="page-btns">
          <button class="btn btn-ghost btn-sm" :disabled="inboundPage <= 1" @click="inboundPage = 1">&laquo;</button>
          <button class="btn btn-ghost btn-sm" :disabled="inboundPage <= 1" @click="inboundPage--">&lsaquo;</button>
        ...
        >
          ...
          <div class="inbound-card-item-body">
            ...
            <div>
              ...
              {{ calcInboundWeight(order).toFixed(2) }} kg
            </div>
            <div>
    ...
    <div v-else-if="inboundView === 'calendar'">
      <div class="panel-card">
        ...
        <div v-safe-html="inboundCalHtml" class="panel-card-body no-padding"></div>
      </div>
    </div>
  </div>
      ...
      <div class="wizard-modal wizard-modal-lg">
        ...
        <div class="wizard-body">
          ...
          <div class="form-grid form-grid-3">
            ...
            <div class="form-group">
              ...
              <div class="supplier-input-group">
                <DataSelect
                  v-model="inboundForm.counterpartyId"
                  module="supplier"
                  variant="active"
                  value-field="id"
                  label-field="name"
                ...
                <input
                  ...
                  class="form-input"
                  placeholder="供应商编码"
                  @change="onInboundSupplierCodeChange"
                />
              </div>
            </div>
            <div class="form-group">
          ...
          <div style="margin-top: var(--space-4)">
            ...
            <div class="inbound-items-table">
              <table class="inv-table inv-table-sm">
                ...
                <tbody>
                  <tr v-for="(item, idx) in inboundFormItems" :key="item.id || idx">
                    <td class="cell-mono">{{ idx + 1 }}</td>
                    <td>
                      <input
                        v-model="item.barcode"
                        type="text"
                        class="form-input form-input-sm inbound-barcode-input"
                        placeholder="扫码/输入"
                        @change="onInboundItemBarcodeChange(idx)"
                      />
                    </td>
                    <td class="cell-material-select">
                      <DataSelect
                        v-model="item.code"
                        module="inventory"
                        value-field="code"
                        label-field="name"
                        placeholder="选择物料"
                        @change="(e) => onMaterialChange(idx, e)"
                      />
                    </td>
                    <td>
                      <input
                    ...
                    <td>
                      <input v-model="item.name" type="text" class="form-input form-input-sm" placeholder="名称" />
                    </td>
                    <td>
                      <input v-model="item.grade" type="text" class="form-input form-input-sm" placeholder="牌号" />
                    </td>
                    <td>
                      <input v-model="item.color" type="text" class="form-input form-input-sm" placeholder="颜色" />
                    </td>
                    <td>
                      <input
              ...
              >
                ...
                <button class="btn btn-ghost btn-sm" @click="focusBarcodeInput">
                  <Icon name="mobile" :size="14" />
                  扫码录入
                </button>
                <button class="btn btn-ghost btn-sm" @click="toggleContinuousScan">
                  <Icon name="search" :size="14" />
                  连续扫码({{ continuousScanMode ? '开' : '关' }})
                </button>
              </div>
              <div class="inbound-summary">
                <span>
                  总重量：
                  <strong>{{ inboundItemsTotalWeight.toFixed(2) }}</strong>
                  kg
                </span>
                <span>
                  总金额：
                  <strong>{{ inboundItemsTotalAmount.toFixed(2) }}</strong>
                  元
                </span>
              </div>
            </div>
          ...
          <div v-if="inboundErrors.length > 0" class="form-errors">
            <div v-for="(err, idx) in inboundErrors" :key="idx" class="form-error">
        ...
        <div class="wizard-footer">
          ...
          <button class="btn btn-secondary" @click="handleSaveInboundDraft">
            <Icon name="save" :size="14" />
            保存草稿
          </button>
          <button class="btn btn-primary" @click="handleSubmitInbound">
            <Icon name="check" :size="14" />
            提交入库
          </button>
        </div>
    ...
    <div v-if="showInboundDetail" class="wizard-overlay" @click.self="showInboundDetail = false">
      <div class="wizard-modal wizard-modal-lg">
        <div class="wizard-header">
          ...
          <button class="btn btn-ghost btn-sm" @click="showInboundDetail = false">
            <Icon name="close" :size="14" />
          </button>
        </div>
        <div v-if="detailOrder" class="wizard-body">
          <div class="detail-info-grid">
            <div class="detail-field">
              <span class="detail-label">入库单号</span>
              <span class="detail-value cell-mono">{{ detailOrder.orderNo }}</span>
            </div>
            <div class="detail-field">
              <span class="detail-label">入库日期</span>
              <span class="detail-value">{{ detailOrder.date }}</span>
            </div>
            <div class="detail-field">
              <span class="detail-label">入库类型</span>
              <span class="detail-value">{{ inboundTypeLabel(detailOrder.type) }}</span>
            </div>
            <div class="detail-field">
              <span class="detail-label">供应商</span>
              <span class="detail-value">{{ detailOrder.counterpartyName || '-' }}</span>
            </div>
            <div class="detail-field">
              <span class="detail-label">供应商编码</span>
              <span class="detail-value cell-mono">{{ detailOrder.supplierCode || '-' }}</span>
            </div>
            <div class="detail-field">
              <span class="detail-label">状态</span>
              <span class="detail-value">
                <span class="status-badge" :class="'status-' + detailOrder.status">
                  {{ inventoryStore.INBOUND_STATUS_LABELS[detailOrder.status] }}
                </span>
              </span>
            </div>
            <div class="detail-field">
              <span class="detail-label">总重量</span>
              <span class="detail-value">{{ calcInboundWeight(detailOrder).toFixed(2) }} kg</span>
            </div>
            <div class="detail-field">
              <span class="detail-label">总金额</span>
              <span class="detail-value">¥{{ formatNumber(calcInboundAmount(detailOrder)) }}</span>
            </div>
          </div>
          <div style="margin-top: var(--space-4)">
          ...
          <div style="margin-top: var(--space-4)">
            ...
            <div style="display: flex; gap: var(--space-2); align-items: center; flex-wrap: wrap">
              <button class="btn btn-ghost btn-sm" disabled title="功能开发中">
              ...
              <tr v-for="item in inventoryStore.recycleBin" :key="item.id">
                ...
                <td class="cell-actions">
                  <button
                    class="btn btn-ghost btn-sm"
                    style="color: var(--color-success)"
                    @click="handleRestoreFromRecycleBin(item.id)"
                  >
                    恢复
                  </button>
                  <button
                    class="btn btn-ghost btn-sm"
                    style="color: var(--color-danger)"
                    @click="handlePermanentDeleteFromRecycleBin(item.id)"
                  >
                    永久删除
                  </button>
                </td>
...
<script>
export default { name: 'InboundSection' }
</script>
<script setup>
...
const inboundSmartPlaceholder =
  '粘贴入库信息或表格文本，系统将自动识别并提取单号、日期、仓库、供应商、金额和明细。'
...
function parseInboundSmartInfo(text) {
  const items = []
  ...
  const dateMatch = text.match(/(?:入库日期|日期|到货日期)[:\s：]*([0-9]{4}[-/年][0-9]{1,2}[-/月][0-9]{1,2})/)
  if (dateMatch) {
    pushItem('date', '入库日期', dateMatch[1].replace(/[年月]/g, '-').replace(/\//g, '-'), 82)
  }

  const supplierMatch = text.match(/(?:供应商|往来单位|客户)[:\s：]*([^\n\r]{2,30})/)
  if (supplierMatch) pushItem('counterpartyName', '供应商', supplierMatch[1].trim(), 75)

  const codeMatch = text.match(/(?:供应商编码|编码|编号)[:\s：]*([A-Za-z0-9_-]{2,30})/)
  if (codeMatch) pushItem('supplierCode', '供应商编码', codeMatch[1].trim(), 85)

  const warehouseMatch = text.match(/(?:仓库|目标仓库)[:\s：]*([^\n\r]{2,20})/)
  if (warehouseMatch) pushItem('warehouseId', '仓库', warehouseMatch[1].trim(), 72)

  const notesMatch = text.match(/(?:备注|说明)[:\s：]*([^\n\r]{3,120})/)
  if (notesMatch) pushItem('notes', '备注', notesMatch[1].trim(), 60)
  ...
  const tableRows = parseTableText(text, [
    ...
    { key: 'grade', label: '牌号', type: 'string' },
    { key: 'color', label: '颜色', type: 'string' },
    { key: 'qty', label: '数量', type: 'number' },
    { key: 'cost', label: '单价', type: 'number' },
...
const {
  ...
  resetSmartRec
} = useSmartRecognizeBase(inboundForm, parseInboundSmartInfo, inboundSmartPlaceholder)
...
const inboundDetailTotalPages = computed(() =>
  Math.max(1, Math.ceil(filteredInboundDetails.value.length / inboundDetailPageSize))
)
...
function applySmartRecognizeToForm() {
  ...
  smartRecResult.value.items?.forEach((item) => {
    if (Object.hasOwn(inboundForm, item.key) && item.value !== undefined && item.value !== null && item.value !== '') {
      inboundForm[item.key] = item.value
    }
  })
  if (smartRecResult.value.tableRows && smartRecResult.value.tableRows.length > 0) {
    inboundFormItems.value = smartRecResult.value.tableRows.map((row) => ({
      barcode: row.barcode || '',
      code: row.code || '',
      name: row.name || '',
      grade: row.grade || '',
...
async function handleExportSelectedInbound() {
  const selected = inventoryStore.inboundOrders.filter((o) => inboundSelectedIds.value.includes(o.id))
  if (selected.length === 0) {
    inboundErrors.value = ['请选择要导出的入库单']
    return
  }
  const data = selected.map((o) => ({
    入库单号: o.orderNo,
    入库日期: o.date,
    入库类型: inboundTypeLabel(o.type),
    供应商编码: o.supplierCode || '',
    供应商名称: o.counterpartyName || '',
    总重量: calcInboundWeight(o).toFixed(2),
    总金额: calcInboundAmount(o).toFixed(2),
...
function handlePrintInbound(order) {
  ...
  for (let i = 0; i < items.length; i++) {
    ...
    itemRows +=
      ...
      '</td><td>' +
      (it.cost || 0).toFixed(2) +
      '</td><td style="font-weight:bold;overflow-wrap:break-word;word-wrap:break-word">' +
      amt.toFixed(2) +
      '</td><td>' +
      escapeHtml(it.batch || '') +
      '</td></tr>'
  ...
  printWindow.document.write(
    '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>入库单 - ' +
      escapeHtml(order.orderNo || '') +
      '</title><style>body{font-family:"HarmonyOS Sans SC","PingFang SC","Microsoft YaHei","Noto Sans SC",sans-serif;font-size:12pt;line-height:1.8;color:#000;margin:0;padding:20mm}table{width:100%;border-collapse:collapse;margin:var(--space-2) 0}th{border:1px solid #333;padding:var(--space-2) var(--space-2);text-align:center;font-size:10.5pt; overflow-wrap: break-word; word-wrap: break-word}td{border:1px solid #333;padding:var(--space-2) var(--space-2);text-align:center;font-size:10.5pt; overflow-wrap: break-word; word-wrap: break-word}th{background:#f0f0f0;font-weight:bold}.title{text-align:center;font-size:20pt;font-weight:bold;letter-spacing:6px;margin-bottom:var(--space-1)}.sub{text-align:center;font-size:10pt;color:#555;margin-bottom:var(--space-5)}.info-grid{display:grid;grid-template-columns:1fr 1fr;gap:var(--space-1) var(--space-5);font-size:10.5pt;margin:var(--space-2) 0}.sign{margin-top:var(--space-10);display:flex;justify-content:space-between;font-size:10.5pt}.sign-line{display:inline-block;width:140px;border-bottom:1px solid #333}.total-row td{font-weight:bold;background:#f8f8f8; overflow-wrap: break-word; word-wrap: break-word}@page{size:A4;margin:15mm}</style></head><body>'
  )
  printWindow.document.write(
    '<div class="title">苏州冠久新材料科技有限公司</div><div class="title" style="font-size:16pt">入 库 单</div><div class="sub">入库单号: ' +
      escapeHtml(order.orderNo || '') +
      '</div>'
  )
  printWindow.document.write(
    '<div class="info-grid"><div>入库类型: ' +
      escapeHtml(typeLabel) +
      '</div><div>入库日期: ' +
      escapeHtml(order.date || '-') +
      '</div><div>供应商: ' +
      escapeHtml(order.counterpartyName || '-') +
      '</div><div>供应商编码: ' +
      escapeHtml(order.supplierCode || '-') +
      '</div><div>状态: ' +
      (inventoryStore.INBOUND_STATUS_LABELS[order.status] || escapeHtml(order.status || '')) +
      '</div><div>备注: ' +
      escapeHtml(order.notes || '-') +
      '</div></div>'
  )
  printWindow.document.write(
    '<table><thead><tr><th>序号</th><th>编号</th><th>物料名称</th><th>牌号</th><th>颜色</th><th>数量(kg)</th><th>单价(元/kg)</th><th>金额(元)</th><th>批次号</th></tr></thead><tbody>'
  )
  printWindow.document.write(itemRows)
  printWindow.document.write(
    '<tr class="total-row"><td colspan="5">合计</td><td>' +
      totalQty.toFixed(2) +
      '</td><td></td><td>' +
      totalAmt.toFixed(2) +
      '</td><td></td></tr>'
  ...
  printWindow.document.write(
    '<div class="sign"><div>制单人: <span class="sign-line"></span></div><div>质检人: <span class="sign-line"></span></div><div>仓管人: <span class="sign-line"></span></div><div>供应商确认: <span class="sign-line"></span></div></div>'
  )
  printWindow.document.write(
    '<div style="text-align:right;font-size:9pt;color:#999;margin-top:20px">打印时间: ' +
...
.inbound-alert-item:hover {
  background: var(--color-surface-hover);
}
.inbound-alert-item:last-child {
  border-bottom: none;
}
...
.inbound-alert-supplier {
  color: var(--color-text-secondary);
  font-size: var(--font-size-sm);
  flex: 1;
}
.inbound-alert-amount {
  font-family: var(--font-mono);
  font-weight: 600;
  font-size: var(--font-size-sm);
...
.inv-table tbody tr:nth-child(n + 6) {
...
.inbound-list-item-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: var(--space-1);
}
.inbound-list-item-body {
  display: flex;
  gap: var(--space-3);
  color: var(--color-text-secondary);
  font-size: var(--font-size-sm);
...
.inbound-card-item-header {
  display: flex;
  justify-content: space-between;
  align-items: center;
  margin-bottom: var(--space-2);
}
.inbound-card-item-body {
  font-size: var(--font-size-sm);
  color: var(--color-text-secondary);
}
.inbound-card-item-body > div {
  margin-bottom: var(--space-1);
}
.inbound-card-item-actions {
  margin-top: var(--space-2);
  display: flex;
  gap: var(--space-2);
}
...
.supplier-input-group .data-select {
  flex: 2;
}
.supplier-input-group .form-input {
  flex: 1;
}

/* 表格内 DataSelect 紧凑样式 */
.cell-material-select {
  min-width: 140px;
}
.cell-material-select .data-select {
  min-width: 120px;
}
.cell-material-select .data-select-trigger {
  min-height: 28px;
  padding: var(--space-1) var(--space-6) var(--space-1) var(--space-2);
  font-size: var(--font-size-xs);
}
.cell-material-select .data-select-dropdown {
  font-size: var(--font-size-xs);
}
