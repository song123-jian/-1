  ...
  <div class="outbound-page">
    <div class="outbound-header">
      ...
      <div class="outbound-header-actions">
        <div class="view-toggle-dropdown" @click.stop>
          <button class="btn btn-ghost btn-sm" @click="outboundViewMenuOpen = !outboundViewMenuOpen">
            <Icon name="chart" :size="14" />
            视图
          </button>
          <div v-if="outboundViewMenuOpen" class="view-toggle-menu">
            <button
              :class="{ active: outboundView === 'table' }"
              @click="((outboundView = 'table'), (outboundViewMenuOpen = false))"
            >
              <Icon name="chart" :size="14" />
              表格视图
            </button>
            <button
              :class="{ active: outboundView === 'list' }"
              @click="((outboundView = 'list'), (outboundViewMenuOpen = false))"
            >
              <Icon name="list" :size="14" />
              列表视图
            </button>
            <button
              :class="{ active: outboundView === 'card' }"
              @click="((outboundView = 'card'), (outboundViewMenuOpen = false))"
            >
              <Icon name="card" :size="14" />
              卡片视图
            </button>
            <button
              :class="{ active: outboundView === 'calendar' }"
              @click="((outboundView = 'calendar'), (outboundViewMenuOpen = false))"
            >
        ...
        <button class="btn btn-secondary btn-sm" @click="handleExportOutbound">
    ...
    <div class="flow-board">
      <div
        ...
        :class="{ active: outboundStatusFilter === 'pending_review' || outboundStatusFilter === 'pending' }"
      ...
      <div
        class="flow-board-node"
        :class="{ active: outboundStatusFilter === 'approved' }"
        @click="outboundStatusFilter = outboundStatusFilter === 'approved' ? '' : 'approved'"
      >
      ...
      <div
        class="flow-board-node"
        :class="{ active: outboundStatusFilter === 'confirmed' }"
        @click="outboundStatusFilter = outboundStatusFilter === 'confirmed' ? '' : 'confirmed'"
      >
        <span class="flow-board-dot completed"></span>
        <div>
          <div class="flow-board-count">
            {{ inventoryStore.outboundOrders.filter((o) => (o.outStatus || o.status) === 'confirmed').length }}
          </div>
          ...
          <div class="inv-stat-card" style="animation-delay: 60ms">
            ...
            <div>
              <div class="inv-stat-value">
                {{ (outboundMonthAmount / 10000).toFixed(1) }}
                <span class="inv-stat-unit">万</span>
              </div>
          ...
          <div class="inv-stat-card" style="animation-delay: 120ms">
            ...
            <div>
              <div class="inv-stat-value">
                ...
                {{ inventoryStore.pendingOutboundCount }}
              </div>
          ...
          <div class="overview-card overview-ring-card">
            ...
            <div class="overview-ring-body">
              ...
              <div class="overview-ring-text">
                <span class="overview-ring-percent" :style="{ color: completionRateColor }">
    ...
    <div v-if="outboundSelectedIds.length > 0" class="outbound-bulk-bar">
      ...
      <button
        v-if="outboundSelectedPendingCount > 0 && canApproveOutbound"
        class="btn btn-secondary btn-sm"
        @click="handleBatchApproveOutbound"
      >
        批量审批({{ outboundSelectedPendingCount }})
      </button>
      <button
        v-if="outboundSelectedApprovedCount > 0"
        class="btn btn-primary btn-sm"
        @click="handleBatchConfirmOutbound"
      >
        批量确认出库({{ outboundSelectedApprovedCount }})
      </button>
            ...
            <thead>
              <tr>
                <th style="width: 36px; text-align: center">
                  <input v-model="outboundSelectAll" type="checkbox" @change="toggleOutboundSelectAll" />
                </th>
            ...
            <tbody>
              <tr v-if="paginatedOutbound.length === 0">
                <td colspan="15" class="empty-cell">
                  <div class="empty-state-icon"><Icon name="download" :size="24" /></div>
                  暂无出库记录
                </td>
              </tr>
              <tr
                v-for="(order, idx) in paginatedOutbound"
                :key="order.id"
                :style="{ animationDelay: idx * 20 + 'ms' }"
              >
                <td style="text-align: center; overflow-wrap: break-word; word-wrap: break-word">
                  <input v-model="outboundSelectedIds" type="checkbox" :value="order.id" />
                </td>
                <td style="width: 50px; text-align: center; overflow-wrap: break-word; word-wrap: break-word">
                  {{ (outboundPage - 1) * outboundPageSize + idx + 1 }}
                </td>
                <td v-if="outboundColumnVisible.outboundNo" class="cell-mono">
                ...
                <td v-if="outboundColumnVisible.unitPrice" class="cell-mono">
                  {{ (order.unitPrice || 0).toFixed(2) }}
                </td>
                <td v-if="outboundColumnVisible.outAmount" class="cell-mono" style="font-weight: 600">
                  {{ formatNumber(order.outAmount || 0) }}
                </td>
                <td v-if="outboundColumnVisible.relatedNo" class="cell-mono">{{ order.referenceId || '-' }}</td>
                <td v-if="outboundColumnVisible.outStatus">
                  <span class="status-badge" :class="'status-' + (order.outStatus || order.status)">
                    {{
                      inventoryStore.OUTBOUND_STATUS_LABELS[order.outStatus || order.status] ||
                      order.outStatus ||
                      order.status
                    }}
                  </span>
                ...
                <td class="cell-actions">
                  ...
                  <template
                    v-if="
                      (order.outStatus || order.status) === 'pending_review' ||
                      (order.outStatus || order.status) === 'pending'
                    "
                  >
                    <button class="btn btn-ghost btn-sm" @click="openEditOutbound(order)">
                      <Icon name="edit" :size="14" />
                    </button>
                    <button
                      v-if="canApproveOutbound"
                      class="btn btn-ghost btn-sm"
                      style="color: var(--color-info)"
                      @click="handleApproveOutbound(order.id)"
                    >
                      审批
                    </button>
                    <button
                      ...
                      @click="handleCancelOutbound(order.id)"
                    >
                      取消
                    </button>
                    <button
                      class="btn btn-ghost btn-sm"
                      style="color: var(--color-danger)"
                      @click="handleDeleteOutbound(order.id)"
                    >
                  ...
                  <template v-if="(order.outStatus || order.status) === 'approved'">
                    <button
                      ...
                      @click="handleConfirmOutbound(order.id)"
                    >
                      确认出库
                    </button>
                    <button
                  ...
                  <template v-if="(order.outStatus || order.status) === 'confirmed'">
                    ...
                    <button
                      ...
                      @click="handleReverseOutbound(order.id)"
                    >
                      冲销
                    </button>
                  </template>
                  <template
                    v-if="
                      (order.outStatus || order.status) === 'cancelled' ||
                      (order.outStatus || order.status) === 'reversed'
                    "
                  >
                    <button class="btn btn-ghost btn-sm" @click="handlePrintOutbound(order)">打印</button>
                  </template>
    ...
    <div v-if="outboundView === 'list'" class="panel-card" style="margin-top: var(--space-3)">
      ...
      <div class="panel-card-body no-padding">
        ...
        <div v-if="filteredOutboundOrders.length === 0" class="empty-cell" style="padding: var(--space-8)">
          <div class="empty-state-icon"><Icon name="download" :size="24" /></div>
          暂无出库记录
        </div>
      </div>
    ...
    <div v-if="outboundView === 'card'" style="margin-top: var(--space-3)">
      <div style="display: flex; align-items: center; justify-content: space-between; margin-bottom: var(--space-3)">
        <span style="font-size: var(--font-size-sm); color: var(--color-text-tertiary)">
          共 {{ filteredOutboundOrders.length }} 条
        </span>
      </div>
      <div
        v-if="filteredOutboundOrders.length === 0"
        class="empty-cell"
      ...
      <div v-else class="outbound-card-grid">
        <div
          v-for="(order, idx) in paginatedOutbound"
          :key="order.id"
          class="outbound-card-item"
          :style="{ animationDelay: idx * 60 + 'ms' }"
        >
          <div
            class="outbound-card-top-bar"
            :style="{ background: statusBarColor(order.outStatus || order.status) }"
          ></div>
          <div
          ...
          <div style="font-size: var(--font-size-xs); color: var(--color-text-tertiary)">
            ...
            <div>
              ...
              {{ (order.outQty || 0).toFixed(2) }} kg
            </div>
            <div>
              <span class="detail-label">金额</span>
              {{ formatNumber(order.outAmount || 0) }} 元
            </div>
            <div>
    ...
    <div v-if="outboundView === 'calendar'" class="panel-card" style="margin-top: var(--space-3)">
      ...
      <div v-safe-html="outboundCalHtml" class="panel-card-body no-padding"></div>
    </div>

    <div v-if="outboundView !== 'calendar' && outboundTotalPages > 1" class="pagination-bar">
      ...
      <button
        v-for="p in outboundVisiblePages"
        :key="p"
        class="pagination-btn"
        :class="{ active: p === outboundPage }"
        @click="outboundPage = p"
      >
        {{ p }}
      </button>
      ...
      <button
        class="pagination-btn"
        :disabled="outboundPage >= outboundTotalPages"
        @click="outboundPage = outboundTotalPages"
      >
        »
      </button>
      <select
        v-model="outboundPageSize"
        class="form-select"
    ...
    <div v-if="showOutboundWizard" class="wizard-overlay" @click.self="closeOutboundWizard">
      <div class="wizard-modal wizard-modal-lg">
        ...
        <div class="wizard-body">
          ...
          <div class="form-row form-row-2">
            ...
            <div class="form-group">
              ...
              <input
                v-model="outboundForm.referenceId"
                type="text"
                class="form-input"
                placeholder="生产工单/销售订单/调拨单号"
              />
            </div>
          </div>
          <div class="form-row form-row-2">
            ...
            <div class="form-group">
              ...
              <DataSelect
                v-model="outboundForm.locationId"
          ...
          <div class="form-row form-row-2">
            <div class="form-group">
              ...
              <div
                v-if="outboundForm.materialCode"
                style="font-size: 11px; margin-top: 4px; color: var(--color-text-tertiary)"
              >
                当前库存:
                <strong
                  :style="{ color: selectedOutboundItemStock > 0 ? 'var(--color-success)' : 'var(--color-danger)' }"
                >
                  {{ selectedOutboundItemStock.toFixed(1) }} kg
                </strong>
              </div>
            </div>
            <div class="form-group">
          ...
          <div v-if="outboundErrors.length > 0" class="form-errors">
            <div v-for="(err, idx) in outboundErrors" :key="idx" class="form-error">
              <Icon name="warning" :size="14" />
              {{ err }}
            </div>
          </div>
        ...
        <div class="wizard-footer">
          ...
          <button class="btn btn-secondary" @click="handleSaveOutboundDraft">
            <Icon name="save" :size="14" />
            保存草稿
          </button>
          <button class="btn btn-primary" @click="handleSubmitOutbound">
    ...
    <div v-if="viewDetail && selectedOrder" class="wizard-overlay" @click.self="viewDetail = false">
      <div class="wizard-modal">
        ...
        <div class="wizard-body">
          ...
          <div class="detail-grid">
            <div class="detail-item">
              ...
              <span class="detail-value cell-mono">{{ selectedOrder.outboundNo || selectedOrder.orderNo || '-' }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">出库类型</span>
              <span class="detail-value">{{ outboundTypeLabel(selectedOrder.outType || selectedOrder.type) }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">出库状态</span>
              <span class="status-badge" :class="'status-' + (selectedOrder.outStatus || selectedOrder.status)">
                {{ inventoryStore.OUTBOUND_STATUS_LABELS[selectedOrder.outStatus || selectedOrder.status] || '-' }}
              </span>
            </div>
            <div class="detail-item">
            ...
            <div class="detail-item">
              ...
              <span class="detail-value cell-mono">{{ selectedOrder.referenceId || '-' }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">仓库</span>
              <span class="detail-value">{{ selectedOrder.warehouseId || '-' }}</span>
            </div>
          </div>
          <div class="form-section-title" style="margin-top: var(--space-4)">
            <Icon name="package" :size="14" />
          ...
          <div class="detail-grid">
            <div class="detail-item">
              ...
              <span class="detail-value cell-mono">{{ selectedOrder.materialCode || '-' }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">物料名称</span>
              <span class="detail-value">{{ selectedOrder.materialName || '-' }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">牌号</span>
              <span class="detail-value">{{ selectedOrder.grade || '-' }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">颜色</span>
              <span class="detail-value">{{ selectedOrder.color || '-' }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">出库数量</span>
              <span class="detail-value cell-mono">{{ (selectedOrder.outQty || 0).toFixed(2) }} kg</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">单价</span>
              <span class="detail-value cell-mono">{{ (selectedOrder.unitPrice || 0).toFixed(2) }} 元/kg</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">出库金额</span>
              <span class="detail-value cell-mono" style="font-weight: 700; color: var(--color-accent)">
                {{ formatNumber(selectedOrder.outAmount || 0) }} 元
              </span>
            </div>
            <div class="detail-item">
              <span class="detail-label">批号</span>
              <span class="detail-value">{{ selectedOrder.batchNo || '-' }}</span>
            </div>
          </div>
          <div class="form-section-title" style="margin-top: var(--space-4)">
          ...
          <div class="detail-grid">
            ...
            <div class="detail-item">
              ...
              <span class="detail-value">{{ selectedOrder.updatedAt || '-' }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">审批人</span>
              <span class="detail-value">{{ selectedOrder.approvedBy || '-' }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">审批时间</span>
              <span class="detail-value">{{ selectedOrder.approvedAt || '-' }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">确认人</span>
              <span class="detail-value">{{ selectedOrder.confirmedBy || '-' }}</span>
            </div>
            <div class="detail-item">
              <span class="detail-label">确认时间</span>
              <span class="detail-value">{{ selectedOrder.confirmedAt || '-' }}</span>
            </div>
          </div>
        ...
        <div class="wizard-footer">
          ...
          <button class="btn btn-primary" @click="handlePrintOutbound(selectedOrder)">
...
<script>
export default { name: 'OutboundSection' }
</script>
<script setup>
...
const outboundColumnVisible = ref(
  Object.fromEntries(outboundColumnDefs.filter((c) => c.hideable !== false).map((c) => [c.key, true]))
)
...
useClickOutside(closeOutboundViewMenu)
...
const outboundSmartPlaceholder =
  '粘贴出库信息或表格文本，系统将自动识别并提取单号、日期、仓库、物料、数量和金额。'
...
function parseOutboundSmartInfo(text) {
  const items = []
  ...
  const typeMatch = text.match(/(?:出库|销售出库|调拨出库|报废出库|样品出库|退货出库)/)
  if (typeMatch) {
    const typeMap = {
      出库: '',
      销售出库: 'sales',
    ...
    const mapped = typeMap[typeMatch[0]]
    if (mapped) pushItem('outType', '出库类型', mapped, 80)
  }

  const dateMatch = text.match(/(?:出库日期|日期)[:\s：]*([0-9]{4}[-/年][0-9]{1,2}[-/月][0-9]{1,2})/)
  if (dateMatch) pushItem('date', '出库日期', dateMatch[1].replace(/[年月]/g, '-').replace(/\//g, '-'), 82)

  const codeMatch = text.match(/(?:编号|物料编号|物料代码|编码)[:\s：]*([A-Za-z0-9_-]{2,30})/)
  if (codeMatch) pushItem('materialCode', '编号', codeMatch[1].trim(), 85)

  const nameMatch = text.match(/(?:物料名称|名称)[:\s：]*([^\n\r]{2,40})/)
  if (nameMatch) pushItem('materialName', '物料名称', nameMatch[1].trim(), 80)

  const gradeMatch = text.match(/(?:牌号|规格)[:\s：]*([^\n\r]{1,20})/)
  if (gradeMatch) pushItem('grade', '牌号', gradeMatch[1].trim(), 72)

  const qtyMatch = text.match(/(?:出库数量|数量)[:\s：]*([0-9.]+)/)
  if (qtyMatch) pushItem('outQty', '出库数量', Number(qtyMatch[1]), 85)

  const priceMatch = text.match(/(?:单价|价格)[:\s：]*([0-9.]+)/)
  if (priceMatch) pushItem('unitPrice', '单价', Number(priceMatch[1]), 78)

  const warehouseMatch = text.match(/(?:出库仓库|仓库)[:\s：]*([^\n\r]{2,20})/)
  if (warehouseMatch) pushItem('warehouseId', '出库仓库', warehouseMatch[1].trim(), 72)
  ...
  const tableRows = parseTableText(text, [
    ...
    { key: 'outQty', label: '数量', type: 'number' },
...
const {
  ...
  resetSmartRec
} = useSmartRecognizeBase(outboundForm, parseOutboundSmartInfo, outboundSmartPlaceholder)
...
function showInlineMessage(type, message) {
  ...
  inlineTimer = setTimeout(() => {
    inlineMessage.value.show = false
  }, 3000)
}
...
const outboundTotalPages = computed(() =>
  Math.max(1, Math.ceil(filteredOutboundOrders.value.length / outboundPageSize.value))
)
...
const outboundMonthAmount = computed(() => {
  ...
  return inventoryStore.outboundOrders
    .filter((o) => (o.outStatus || o.status) === 'confirmed' && (o.date || '').startsWith(ym))
    .reduce((s, o) => s + Math.abs(o.outAmount || 0), 0)
})

const outboundApprovedCount = computed(
  () => inventoryStore.outboundOrders.filter((o) => (o.outStatus || o.status) === 'approved').length
)
    ...
    for (let j = 0; j < 7; j++) {
      ...
      for (const di of dayItems.slice(0, 2)) {
        html +=
          ...
          escapeHtml(di.outboundNo || di.orderNo) +
          '</div>'
      }
...
function applySmartRecognizeToForm() {
  ...
  smartRecResult.value.items?.forEach((item) => {
    if (Object.hasOwn(outboundForm, item.key) && item.value !== undefined && item.value !== null && item.value !== '') {
      outboundForm[item.key] = item.value
    }
  })
  if (smartRecResult.value.tableRows && smartRecResult.value.tableRows.length > 0) {
    const row = smartRecResult.value.tableRows[0]
    if (row.materialCode) outboundForm.materialCode = row.materialCode
    if (row.materialName) outboundForm.materialName = row.materialName
    if (row.grade) outboundForm.grade = row.grade
    if (row.color) outboundForm.color = row.color
    if (row.outQty !== undefined && row.outQty !== null) outboundForm.outQty = row.outQty
    if (row.unitPrice !== undefined && row.unitPrice !== null) outboundForm.unitPrice = row.unitPrice
  }
}
...
function handleSubmitOutbound() {
  if (!outboundForm.outType) {
    outboundErrors.value = ['请选择出库类型']
    return
  }
  if (!outboundForm.materialCode) {
    outboundErrors.value = ['请选择编号']
    return
  }
  if (!outboundForm.outQty || outboundForm.outQty <= 0) {
    outboundErrors.value = ['出库数量必须大于0']
    return
  }
  if (!outboundForm.date) {
    outboundErrors.value = ['请选择出库日期']
    return
  }
  if (outboundForm.date > new Date().toISOString().split('T')[0]) {
    outboundErrors.value = ['出库日期不能是未来日期']
    return
  }
  if (!outboundForm.warehouseId) {
    outboundErrors.value = ['请选择出库仓库']
    return
  }
  const outNo = outboundForm.outboundNo || inventoryStore.generateOutboundNo()
  const dupOrder = inventoryStore.outboundOrders.find(
    (o) => o.outboundNo === outNo && o.id !== (editingOutboundId.value || '')
  )
  if (dupOrder) {
    outboundErrors.value = ['出库单号 ' + outNo + ' 已存在']
    return
  }
...
function handleBatchApproveOutbound() {
  ...
  if (pendingIds.length === 0) {
    showInlineMessage('warning', '请选择待审核的出库单')
    return
  }
...
function handleBatchConfirmOutbound() {
  ...
  if (approvedIds.length === 0) {
    showInlineMessage('warning', '请选择已审核的出库单')
    return
  }
...
function handleExportOutbound() {
  const data = filteredOutboundOrders.value.map((o) => ({
    出库单号: o.outboundNo || o.orderNo,
    出库类型: outboundTypeLabel(o.outType || o.type),
    编号: o.materialCode || '',
    物料名称: o.materialName || '',
    牌号: o.grade || '',
    颜色: o.color || '',
    出库日期: o.date || '',
    出库数量: (o.outQty || 0).toFixed(2),
    单价: (o.unitPrice || 0).toFixed(2),
    出库金额: (o.outAmount || 0).toFixed(2),
    关联单号: o.referenceId || '',
    状态: inventoryStore.OUTBOUND_STATUS_LABELS[o.outStatus || o.status] || ''
  }))
  if (data.length === 0) {
    showInlineMessage('warning', '没有可导出的数据')
    return
  }
  /* CSV导出（含UTF-8 BOM） */
...
function handleExportSelectedOutbound() {
  ...
  if (selected.length === 0) {
    showInlineMessage('warning', '请选择要导出的出库单')
    return
  }
  const data = selected.map((o) => ({
    出库单号: o.outboundNo || o.orderNo,
    物料: o.materialName || '',
    数量: (o.outQty || 0).toFixed(2),
    金额: (o.outAmount || 0).toFixed(2),
    状态: inventoryStore.OUTBOUND_STATUS_LABELS[o.outStatus || o.status] || ''
  }))
  ...
  showConfirmDialog('确认冲销', '确认冲销该出库单？库存将恢复，此操作不可撤销。', () => {
    ...
    if (result.success) {
      inventoryStore.addAuditLog(
        'reverse',
        'outbound',
        '冲销出库单: ' + (result.reverseOrder ? result.reverseOrder.outboundNo : '')
      )
    } else {
...
function handlePrintOutbound(order) {
  ...
  printWindow.document.write(
    '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>出库单 - ' +
      escapeHtml(order.outboundNo || order.orderNo || '') +
  ...
  printWindow.document.write(
    '<div class="title">苏州冠久新材料科技有限公司</div><div class="title" style="font-size:16pt">出 库 单</div>'
  )
  printWindow.document.write(
    '<div class="info-grid"><div>出库单号: ' +
      escapeHtml(order.outboundNo || order.orderNo || '') +
      '</div><div>出库类型: ' +
      ...
      '</div><div>出库数量: ' +
      (order.outQty || 0).toFixed(2) +
      ' kg</div><div>出库金额: ' +
      (order.outAmount || 0).toFixed(2) +
      ' 元</div><div>仓库: ' +
      escapeHtml(order.warehouseId || '-') +
      '</div><div>牌号: ' +
      escapeHtml(order.grade || '-') +
      '</div><div>颜色: ' +
      escapeHtml(order.color || '-') +
      '</div><div>批号: ' +
      escapeHtml(order.batchNo || '-') +
      '</div><div>状态: ' +
      (inventoryStore.OUTBOUND_STATUS_LABELS[order.outStatus || order.status] || '') +
      '</div><div>备注: ' +
  ...
  printWindow.document.write(
    '<div class="sign"><div>制单人: <span class="sign-line"></span></div><div>审核人: <span class="sign-line"></span></div><div>仓管人: <span class="sign-line"></span></div><div>领用人: <span class="sign-line"></span></div></div>'
  )
  printWindow.document.write(
...
function outboundCalPrev() {
  ...
  if (outboundCalMonth.value < 0) {
    outboundCalMonth.value = 11
    outboundCalYear.value--
  }
}

function outboundCalNext() {
  outboundCalMonth.value++
  if (outboundCalMonth.value > 11) {
    outboundCalMonth.value = 0
    outboundCalYear.value++
  }
}
...
.outbound-alert-item:last-child {
  border-bottom: none;
}
