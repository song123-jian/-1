        ...
        <tbody>
          <tr v-if="pagedOrders.length === 0">
            <td colspan="12" class="empty-state">
              <div class="empty-state-icon"><Icon name="clipboardCheck" :size="24" /></div>
              暂无盘点单
            </td>
          </tr>
          <tr v-for="(order, idx) in pagedOrders" :key="order.id" :style="{ animationDelay: idx * 20 + 'ms' }">
            ...
            <td :style="{ color: order.summary.diffItems > 0 ? 'var(--color-warning)' : '' }">
            ...
            <td>
              <div style="display: flex; gap: var(--space-1); flex-wrap: nowrap">
                ...
                <button
                  v-if="order.status === 'diff_review'"
                  class="action-btn"
                  title="差异处理"
                  @click="emit('diff', order)"
                >
...
<script>
export default { name: 'StocktakingList' }
</script>
<script setup>
...
const props = defineProps({
  externalStatusFilter: { type: String, default: '' }
})
