        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-4)">
          <div>
            ...
            <div class="chart-container">
              <div v-if="revenueChartData.length === 0" class="empty-state"><div class="empty-icon"><Icon name="chart" :size="24" /></div><div class="empty-text">暂无数据</div></div>
                ...
                <div v-for="(item, idx) in revenueChartData" :key="idx" class="bar-item">
                  ...
                  <div class="bar-track">
                    <div class="bar-fill revenue" :style="{ height: barHeight(item.value, maxRevenue) + '%' }">
                      <span class="bar-value" v-if="item.value > 0">{{ formatChartValue(item.value) }}</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div>
            ...
            <div class="chart-container">
              <div v-if="collectionChartData.length === 0" class="empty-state"><div class="empty-icon"><Icon name="chart" :size="24" /></div><div class="empty-text">暂无数据</div></div>
              <div v-else class="bar-chart">
      ...
      <div class="modal-panel" style="max-width:700px">
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <select class="form-select" v-model="customSource" @change="updateCustomFields">
          ...
          <div class="form-group">
            ...
            <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:var(--space-2);max-height:200px;overflow-y:auto;padding:var(--space-2)">
              <label v-for="f in customFields" :key="f" style="display:flex;align-items:center;gap:4px;font-size:var(--font-size-xs);cursor:pointer">
                <input type="checkbox" :value="f" v-model="selectedFields"> {{ f }}
              </label>
            </div>
            <div v-if="customFields.length === 0" class="empty-state-inline"><div class="empty-icon-sm"><Icon name="package" :size="20" /></div><div class="empty-text">暂无字段</div></div>
          </div>
...
const financeSummary = computed(() => {
  ...
  const revenue = q.filter(x => x.status === 'approved' || x.status === 'accepted').reduce((s, x) => s + (parseFloat(x.total) || 0), 0)
...
const revenueChartData = computed(() => {
  ...
  return months.map(m => ({
    label: m.substring(5),
    value: q.filter(x => x.createdAt && x.createdAt.indexOf(m) === 0 && (x.status === 'approved' || x.status === 'accepted'))
...
const collectionChartData = computed(() => {
  ...
  return months.map(m => ({
    label: m.substring(5),
    value: c.filter(x => x.date && x.date.indexOf(m) === 0)
...
function generateReportWithParams() {
  ...
  if (type === 'finance' || type === 'custom') {
    ...
    const totalRev = fq2.filter(x => x.status === 'approved' || x.status === 'accepted').reduce((s, x) => s + (parseFloat(x.total) || 0), 0)
...
function exportAllReports(format = 'csv') {
  ...
  for (const t of types) {
    ...
    if (t === 'sales') {
      ...
      row.metric3 = '营收¥' + q.filter(x => x.status === 'approved' || x.status === 'accepted').reduce((s, x) => s + (parseFloat(x.total) || 0), 0).toLocaleString()
    } else if (t === 'inventory') {
      const inv = inventoryStore.inventory || []
      ...
      const q = quotationStore.quotations || []
      const c = collectionStore.collections || []
      const rev = q.filter(x => x.status === 'approved' || x.status === 'accepted').reduce((s, x) => s + (parseFloat(x.total) || 0), 0)
