    ...
    <div v-else>
      <div class="page-header">
        <div>
          ...
          <p class="page-header-subtitle">先看经营态势，再处理待办与预警，减少来回切换</p>
        </div>
      </div>
      ...
      <Suspense>
        <template #default>
          <DashChartsAsync ref="chartsRef" />
        </template>
        <template #fallback>
          <div class="content-grid content-grid-2 dash-charts-fallback">
...
const DashStatsCards = defineAsyncComponent(() => import('@/components/dashboard/DashStatsCards.vue'))
const DashAlerts = defineAsyncComponent(() => import('@/components/dashboard/DashAlerts.vue'))
const DashQuickActions = defineAsyncComponent(() => import('@/components/dashboard/DashQuickActions.vue'))
const DashChartsAsync = defineAsyncComponent(() => import('@/components/dashboard/DashCharts.vue'))
...
async function initDashboardSeedData() {
  const seedTasks = [
    quotationStore.initSeedData?.bind(quotationStore),
    contractStore.initSeedData?.bind(contractStore),
    collectionStore.initSeedData?.bind(collectionStore),
    inventoryStore.initSeedData?.bind(inventoryStore),
    customerStore.initSeedData?.bind(customerStore)
  ].filter(Boolean)

  if (seedTasks.length === 0) return

  const firstBatch = seedTasks.slice(0, 2)
  const secondBatch = seedTasks.slice(2)

  await Promise.all(firstBatch.map((task) => task()))
  isLoading.value = false

  if (secondBatch.length > 0) {
    setTimeout(() => {
      Promise.all(secondBatch.map((task) => task())).catch(() => {})
    }, 0)
  }
}
...
:global(.reduce-motion) .dashboard-page,
:global(.reduce-motion) .skeleton,
:global(.reduce-motion) .todo-quick-item.overdue,
:global(.reduce-motion) .todo-table tr.overdue,
:global(.reduce-motion) .alert-priority-bar,
:global(.reduce-motion) .compact-metric,
:global(.reduce-motion) .nav-item-badge {
