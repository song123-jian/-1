    ...
    <div class="scene-finance__body">
      <div class="scene-finance__left">
        <TechBorder color="#52c41a">
          <div class="scene-finance__panel">
            <div class="scene-finance__panel-title">
              <span class="panel-dot" style="background: #52c41a"></span>
              回款趋势
            </div>
            <div ref="collTrendRef" class="scene-finance__chart"></div>
          </div>
        </TechBorder>
        <TechBorder color="#2f54eb">
          <div class="scene-finance__panel">
            <div class="scene-finance__panel-title">
              <span class="panel-dot" style="background: #2f54eb"></span>
              利润率
            </div>
            <div ref="gaugeRef" class="scene-finance__chart"></div>
          </div>
        </TechBorder>
      </div>
      <div class="scene-finance__center">
        <TechBorder color="#eb2f96">
          <div class="scene-finance__panel">
            <div class="scene-finance__panel-title">
              <span class="panel-dot" style="background: #eb2f96"></span>
              应收应付对比
            </div>
            <div ref="compareRef" class="scene-finance__chart"></div>
          </div>
        </TechBorder>
        <TechBorder color="#52c41a">
          <div class="scene-finance__panel">
            ...
            <div ref="cashFlowRef" class="scene-finance__chart"></div>
          </div>
        </TechBorder>
      </div>
      <div class="scene-finance__right">
        <AlertPanel
          title="财务预警"
          :alerts="[...dataScreenStore.overdueReceivableAlerts, ...dataScreenStore.contractExpiryAlerts]"
        />
        <TechBorder color="#eb2f96">
          <div class="scene-finance__panel">
            <div class="scene-finance__panel-title">
            ...
            <div ref="expenseRef" class="scene-finance__chart"></div>
          </div>
        </TechBorder>
      </div>
    </div>
...
<script>
export default { name: 'FinanceScene' }
</script>
<script setup>
...
const collTrendRef = ref(null),
  gaugeRef = ref(null),
  compareRef = ref(null),
  cashFlowRef = ref(null),
  expenseRef = ref(null)
...
onUnmounted(() => {
  ;[collTrendRef, gaugeRef, compareRef, cashFlowRef, expenseRef].forEach((r) => {
    ...
    series: [
      {
        ...
        areaStyle: {
          color: {
            ...
            y2: 1,
            colorStops: [
              { offset: 0, color: colors[2] + '40' },
              { offset: 1, color: colors[2] + '05' }
            ]
          }
        },
    ...
    series: [
      {
        ...
        progress: {
          show: true,
          width: 14,
          itemStyle: { color: rate > 20 ? '#52c41a' : rate > 10 ? '#faad14' : '#ff4d4f' }
        },
...
function initCompare() {
  ...
  instance.setOption({
    ...
    series: [
      {
        type: 'bar',
        data: [dataScreenStore.totalReceivable, dataScreenStore.totalPayable],
        barWidth: '40%',
        itemStyle: {
          color: (p) => (p.dataIndex === 0 ? colors[6] + 'cc' : colors[2] + 'cc'),
    ...
    series: [
      {
        ...
        areaStyle: {
          color: {
            ...
            y2: 1,
            colorStops: [
              { offset: 0, color: colors[1] + '30' },
              { offset: 1, color: colors[1] + '05' }
            ]
          }
        },
    ...
    series: [
      {
        ...
        radius: ['35%', '60%'],
        center: ['35%', '50%'],
        data: exp.map((e, i) => ({ ...e, itemStyle: { color: colors[i % colors.length] + 'cc' } })),
        label: { show: false },
...
.scene-finance__left,
.scene-finance__right {
  display: flex;
  flex-direction: column;
  gap: 16px;
}
.scene-finance__center {
  display: flex;
  flex-direction: column;
  gap: 16px;
}
.scene-finance__panel {
  padding: 0;
  height: 100%;
  display: flex;
  flex-direction: column;
}
.scene-finance__panel-title {
