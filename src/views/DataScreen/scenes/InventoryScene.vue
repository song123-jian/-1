  ...
  <div class="scene-inventory">
    ...
    <div class="scene-inventory__body">
      <div class="scene-inventory__left">
        <TechBorder color="#00ff88">
          <div class="scene-inventory__panel">
            <div class="scene-inventory__panel-title"><span class="panel-dot" style="background:#00ff88"></span>入库出库趋势</div>
            <div class="scene-inventory__chart" ref="inoutRef"></div>
          </div>
        </TechBorder>
        <TechBorder color="#13c2c2">
          <div class="scene-inventory__panel">
            <div class="scene-inventory__panel-title"><span class="panel-dot" style="background:#13c2c2"></span>库存周转率</div>
            <div class="scene-inventory__chart" ref="gaugeRef"></div>
          </div>
        </TechBorder>
      </div>
      <div class="scene-inventory__center">
        <TechBorder color="#00ff88">
          <div class="scene-inventory__panel">
            <div class="scene-inventory__panel-title"><span class="panel-dot" style="background:#00ff88"></span>库存分布</div>
            <div class="scene-inventory__chart" ref="pieRef"></div>
          </div>
        </TechBorder>
      </div>
      <div class="scene-inventory__right">
        ...
        <TechBorder color="#faad14">
          <div class="scene-inventory__panel">
            <div class="scene-inventory__panel-title"><span class="panel-dot" style="background:#faad14"></span>库存健康度</div>
            <div class="scene-inventory__chart" ref="healthRef"></div>
          </div>
        </TechBorder>
      </div>
    </div>
...
const inoutRef = ref(null), gaugeRef = ref(null), pieRef = ref(null), healthRef = ref(null)

onMounted(() => { nextTick(() => { initInOut(); initGauge(); initPie(); initHealth() }) })
onUnmounted(() => { [inoutRef, gaugeRef, pieRef, healthRef].forEach(r => { if (r.value) disposeChart(r.value) }) })
...
function initGauge() {
  ...
  instance.setOption({
    series: [{
      type: 'gauge', startAngle: 180, endAngle: 0, min: 0, max: 200,
...
function initPie() {
  ...
  instance.setOption({
    ...
    legend: { orient: 'vertical', right: 10, top: 'center', textStyle: { color: 'rgba(255,255,255,0.6)', fontSize: 11 } },
    series: [{
      type: 'pie', radius: ['40%', '65%'], center: ['40%', '50%'],
      data: catData, label: { show: false }, emphasis: { label: { show: true, fontSize: 14, fontWeight: 600 } },
...
function initHealth() {
  ...
  instance.setOption({
    series: [{
      ...
      progress: { show: true, width: 12, itemStyle: { color: rate > 80 ? '#52c41a' : rate > 50 ? '#faad14' : '#ff4d4f' } },
      axisLine: { lineStyle: { width: 12, color: [[1, 'rgba(255,255,255,0.06)']] } },
      axisTick: { show: false }, splitLine: { show: false }, axisLabel: { show: false },
      pointer: { show: false },
      title: { offsetCenter: [0, '60%'], fontSize: 12, color: 'rgba(255,255,255,0.6)' },
      detail: { offsetCenter: [0, '20%'], fontSize: 24, fontWeight: 700, color: '#fff', formatter: '{value}%', fontFamily: 'DIN Alternate,Roboto Mono,monospace' },
...
<style scoped>
.scene-inventory { display: flex; flex-direction: column; gap: 16px; height: 100%; }
.scene-inventory__body { flex: 1; display: grid; grid-template-columns: 1fr 1.5fr 1fr; gap: 16px; min-height: 0; }
.scene-inventory__left, .scene-inventory__right { display: flex; flex-direction: column; gap: 16px; }
.scene-inventory__center { display: flex; flex-direction: column; }
.scene-inventory__panel { padding: 0; height: 100%; display: flex; flex-direction: column; }
.scene-inventory__panel-title { padding: 12px 16px 8px; font-size: 14px; font-weight: 600; color: rgba(255,255,255,0.85); display: flex; align-items: center; gap: 8px; }
.scene-inventory__chart { flex: 1; min-height: 180px; padding: 0 8px 8px; }
@media (max-width: 1200px) { .scene-inventory__body { grid-template-columns: 1fr; } }
</style>
