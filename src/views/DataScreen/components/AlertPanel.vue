  ...
  <TechBorder :color="worstColor">
    <div class="alert-panel">
      <div class="alert-panel__header">
        ...
        <div v-if="alerts.length" class="alert-panel__stats">
          <span v-if="criticalCount" class="alert-panel__badge critical">{{ criticalCount }}</span>
          <span v-if="warningCount" class="alert-panel__badge warning">{{ warningCount }}</span>
        </div>
      </div>
      <div ref="scrollBody" class="alert-panel__body">
        <div
...
<script>
export default { name: 'AlertPanel' }
</script>
<script setup>
...
const worstColor = computed(() =>
  worstLevel.value === 'critical' ? '#ff4d4f' : worstLevel.value === 'warning' ? '#faad14' : '#52c41a'
)
...
.alert-panel__dot.critical {
  background: #ff4d4f;
  box-shadow: 0 0 8px rgba(255, 77, 79, 0.5);
  animation: blink 1.5s infinite;
}
.alert-panel__dot.warning {
  background: #faad14;
  box-shadow: 0 0 6px rgba(250, 173, 20, 0.3);
}
.alert-panel__dot.info {
  background: #52c41a;
}
@keyframes blink {
  0%,
...
.alert-panel__badge.critical {
  background: rgba(255, 77, 79, 0.2);
  color: #ff4d4f;
}
.alert-panel__badge.warning {
...
.alert-panel__body::-webkit-scrollbar {
  width: 4px;
}
.alert-panel__body::-webkit-scrollbar-thumb {
...
.alert-panel__item.critical {
  animation:
    ...
    blink 2s ease-in-out 0.5s infinite;
}
@keyframes slideIn {
  from {
    opacity: 0;
...
.alert-panel__indicator.critical {
  background: #ff4d4f;
  box-shadow: 0 0 8px rgba(255, 77, 79, 0.5);
}
.alert-panel__indicator.warning {
  background: #faad14;
}
.alert-panel__indicator.info {
  background: #1890ff;
}
...
.alert-panel__type.inventory {
  background: rgba(19, 194, 194, 0.15);
  color: #13c2c2;
}
.alert-panel__type.receivable {
  background: rgba(235, 47, 150, 0.15);
  color: #eb2f96;
}
.alert-panel__type.contract {
  background: rgba(114, 46, 209, 0.15);
  color: #722ed1;
}
.alert-panel__type.delivery {
