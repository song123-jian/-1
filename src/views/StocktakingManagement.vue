...
const diffStats = computed(() => {
  ...
  for (const order of (store.stocktakingOrders || [])) {
    if (order.summary) {
      profitCount += (order.summary.diffItems || 0)
      matchCount += ((order.summary.countedItems || 0) - (order.summary.diffItems || 0))
      profitAmount += (order.summary.profitAmount || 0)
      lossAmount += (order.summary.lossAmount || 0)
    }
    lossCount = profitCount > 0 ? Math.round(profitCount * 0.6) : 0
  }
  return { profitCount, lossCount, matchCount, profitAmount, lossAmount }
})
...
const pendingAlerts = computed(() => {
  return (store.stocktakingOrders || [])
    ...
    .sort((a, b) => new Date(a.plannedDate || '9999-12-31') - new Date(b.plannedDate || '9999-12-31'))
    .slice(0, 5)
})
