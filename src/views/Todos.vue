  ...
  <div class="todo-page">
    <div class="page-header">
      <div>
        ...
        <p class="page-header-subtitle">用于盯住今天要做的事，优先处理逾期、关键和自动提醒任务</p>
      </div>
    ...
    <div class="todo-toolbar">
      <div class="todo-search">
        ...
        <input v-model="searchText" type="text" class="search-input" placeholder="搜索任务标题、描述、备注或标签..." />
      </div>
        ...
        <div class="modal-dialog">
          ...
          <div class="modal-body">
            <SmartRecognizePanel
              ...
              @apply-smart-recognize="applyTodoSmartRecognize"
...
import { makeItem } from '@/composables/useSmartRecognizeBase'
...
let todoFormTarget = null
const todoFormProxy = new Proxy(
  {},
  {
    get(_target, key) {
      return todoFormTarget ? todoFormTarget[key] : undefined
    },
    set(_target, key, value) {
      if (todoFormTarget) todoFormTarget[key] = value
      return true
    },
    has(_target, key) {
      return todoFormTarget ? key in todoFormTarget : false
    },
    ownKeys() {
      return todoFormTarget ? Reflect.ownKeys(todoFormTarget) : []
    },
    getOwnPropertyDescriptor(_target, key) {
      if (!todoFormTarget || !Object.prototype.hasOwnProperty.call(todoFormTarget, key)) return undefined
      return { configurable: true, enumerable: true, writable: true, value: todoFormTarget[key] }
    }
  }
)

function parseTodoSmartInfo(text) {
  const items = []
  ...
  const titleMatch = text.match(/(?:任务标题|标题|事项)[:\s：]*([^\n\r]{2,50})/)
  if (titleMatch) pushItem('title', '任务标题', titleMatch[1].trim(), 90)

  const notesMatch = text.match(/(?:任务描述|描述|备注)[:\s：]*([^\n\r]{3,120})/)
  if (notesMatch) pushItem('notes', '任务描述', notesMatch[1].trim(), 72)

  const priorityMatch = text.match(/(?:优先级)[:\s：]*(高|中|低|high|medium|low)/i)
  if (priorityMatch) {
    const raw = priorityMatch[1].toLowerCase()
    const map = { high: 'high', medium: 'medium', low: 'low', 高: 'high', 中: 'medium', 低: 'low' }
    pushItem('priority', '优先级', map[raw] || 'medium', 85)
  }

  const statusMatch = text.match(/(?:状态)[:\s：]*(进行中|已完成|完成|pending|completed)/i)
  if (statusMatch) {
    const raw = statusMatch[1].toLowerCase()
    const map = { pending: 'pending', completed: 'completed', 进行中: 'pending', 完成: 'completed', 已完成: 'completed' }
    pushItem('status', '状态', map[raw] || 'pending', 82)
  }

  const startMatch = text.match(/(?:开始日期|计划开始|开始)[:\s：]*([0-9]{4}[-/年][0-9]{1,2}[-/月][0-9]{1,2})/)
  if (startMatch) pushItem('startDate', '开始日期', startMatch[1].replace(/[年月]/g, '-').replace(/\//g, '-'), 78)

  const dueMatch = text.match(/(?:截止日期|截至日期|截止)[:\s：]*([0-9]{4}[-/年][0-9]{1,2}[-/月][0-9]{1,2})/)
  if (dueMatch) pushItem('dueDate', '截止日期', dueMatch[1].replace(/[年月]/g, '-').replace(/\//g, '-'), 85)

  const tagMatch = text.match(/(?:标签|标记)[:\s：]*([^\n\r]{1,30})/)
  if (tagMatch) pushItem('tag', '标签', tagMatch[1].trim(), 72)

  const reminderMatch = text.match(/(?:提醒|提前提醒)[:\s：]*(不提醒|提前1天|提前2天|提前3天|提前1周)/)
  if (reminderMatch) pushItem('reminder', '提醒', reminderMatch[1].trim(), 80)

  const repeatMatch = text.match(/(?:重复)[:\s：]*(不重复|每天|每周|每月|每年)/)
  if (repeatMatch) {
    const map = { 不重复: 'none', 每天: 'daily', 每周: 'weekly', 每月: 'monthly', 每年: 'yearly' }
    pushItem('repeat', '重复', map[repeatMatch[1]] || 'none', 76)
  }

  const progressMatch = text.match(/(?:完成度|进度)[:\s：]*([0-9]{1,3})%?/)
  if (progressMatch) pushItem('progress', '完成度', Math.min(100, Number(progressMatch[1])), 82)

  const remarkMatch = text.match(/(?:备注)[:\s：]*([^\n\r]{1,120})/)
  if (remarkMatch) pushItem('remark', '备注', remarkMatch[1].trim(), 68)

  const subtasks = []
  const subtaskRegex = /(?:子任务|待办项)[:\s：]*([^\n\r]+?)(?=\s*(?:子任务|待办项|$))/g
  let match
  while ((match = subtaskRegex.exec(text))) {
    const title = match[1].trim()
    if (title) subtasks.push({ id: 'sub_' + Date.now() + '_' + subtasks.length, title, completed: false })
  }

  const result = { items, identifiedCount, lowConfCount }
  if (subtasks.length > 0) result.subtasks = subtasks
  return result
}

const todoSmartPlaceholder = '粘贴任务列表或表格文本，系统将自动识别并提取标题、优先级、日期、标签和子任务。'
const showSmartRec = ref(false)
const smartRecInput = ref('')
const smartRecResult = ref(null)
const smartRecPlaceholder = todoSmartPlaceholder

function buildTodoInsights(result) {
  const insights = []
  if (!result) return insights
  if (result.identifiedCount > 0) insights.push(`已识别 ${result.identifiedCount} 项字段`)
  else insights.push('未识别到明显字段，建议补充更完整的文本')
  if (result.lowConfCount > 0) insights.push(`${result.lowConfCount} 项需确认`)
  if (Array.isArray(result.subtasks) && result.subtasks.length > 0) insights.push(`识别到 ${result.subtasks.length} 个子任务`)
  insights.push('当前模式为本地规则识别，无需联网')
  return insights
}
...
function runSmartRecognize() {
  ...
  const result = parseTodoSmartInfo(text)
  smartRecResult.value = { ...result, insights: buildTodoInsights(result), recognitionMode: 'local' }
}

function applyTodoSmartRecognize() {
  ...
  if (smartRecResult.value.subtasks?.length > 0) {
    form.subtasks = smartRecResult.value.subtasks.map((s, idx) => ({
