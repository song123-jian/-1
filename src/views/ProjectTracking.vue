  ...
  <div class="project-tracking-page">
    <div class="page-header">
      <div>
        ...
        <p class="page-header-subtitle">统一看项目进度、报价、风险和复盘，不再在多个页面里找同一条信息</p>
      </div>
    ...
    <div class="tab-content-scroll">
              ...
              <table class="data-table">
                ...
                <tbody>
                  ...
                  <tr v-if="filteredAutoProjects.length === 0">
                    <td :colspan="autoVisibleColCount + 2" class="empty-cell">暂无结果，先调整筛选条件</td>
                  </tr>
                </tbody>
              </table>
            </div>
              ...
              <table class="data-table">
                ...
                <tbody>
                  ...
                  <tr v-if="filteredNonAutoProjects.length === 0">
                    <td :colspan="napVisibleColCount + 1" class="empty-cell">暂无结果，先调整筛选条件</td>
                  </tr>
                </tbody>
              </table>
            </div>
      ...
      <div v-show="activeTab === 'risk'">
        ...
        <div class="filter-bar">
          ...
          <button class="btn btn-outline btn-danger" @click="openRiskAddModal">新建风险</button>
        </div>
      ...
      <div v-show="activeTab === 'stats'">
        ...
        <div class="content-grid content-grid-3" style="margin-bottom: var(--space-4)">
          <div class="panel-card">
            <div class="panel-card-header"><span class="panel-card-title">客户分布</span></div>
    ...
    <div v-if="showAutoModal" class="modal-overlay" @click.self="showAutoModal = false">
      <div class="modal-content modal-wide">
        ...
        <div class="modal-body">
          <SmartRecognizePanel
            v-if="!isAutoEditing"
            v-model:show-smart-rec="showAutoSmartRec"
            v-model:smart-rec-input="autoSmartRecInput"
            :smart-rec-result="autoSmartRecResult"
            :placeholder="autoSmartRecPlaceholder"
            @run-smart-recognize="runAutoSmartRecognize"
            @apply-smart-recognize="applyAutoSmartRecognize"
            @handle-smart-file-upload="handleAutoSmartFileUpload"
            @clear="resetAutoSmartRec"
          />
    ...
    <div v-if="showNapModal" class="modal-overlay" @click.self="showNapModal = false">
      <div class="modal-content modal-wide">
        ...
        <div class="modal-body">
          <SmartRecognizePanel
            v-if="!isNapEditing"
            v-model:show-smart-rec="showNapSmartRec"
            v-model:smart-rec-input="napSmartRecInput"
            :smart-rec-result="napSmartRecResult"
            :placeholder="napSmartRecPlaceholder"
            @run-smart-recognize="runNapSmartRecognize"
            @apply-smart-recognize="applyNapSmartRecognize"
            @handle-smart-file-upload="handleNapSmartFileUpload"
            @clear="resetNapSmartRec"
          />
          <div class="form-row form-row-3">
            <div class="form-group">
    ...
    <div v-if="showRiskModal" class="modal-overlay" @click.self="showRiskModal = false">
      <div class="modal-content">
        <div class="modal-header">
          <h3 class="modal-title">{{ isRiskEditing ? '编辑风险' : '新建风险' }}</h3>
          <button class="modal-close" @click="showRiskModal = false"><Icon name="close" :size="14" /></button>
        </div>
        <div class="modal-body">
          <SmartRecognizePanel
            v-if="!isRiskEditing"
            v-model:show-smart-rec="showRiskSmartRec"
            v-model:smart-rec-input="riskSmartRecInput"
            :smart-rec-result="riskSmartRecResult"
            :placeholder="riskSmartRecPlaceholder"
            @run-smart-recognize="runRiskSmartRecognize"
            @apply-smart-recognize="applyRiskSmartRecognize"
            @handle-smart-file-upload="handleRiskSmartFileUpload"
            @clear="resetRiskSmartRec"
          />
          <div class="form-row form-row-2">
            <div class="form-group">
    ...
    <div v-if="showOpLogModal" class="modal-overlay" @click.self="showOpLogModal = false">
      <div class="modal-content modal-wide">
        ...
        <div class="modal-footer">
          <button class="btn btn-ghost" @click="((opLogs = []), persistOpLogs())">清空日志</button>
...
import { makeItem, useSmartRecognizeBase } from '@/composables/useSmartRecognizeBase'
let Chart = null
...
let autoFormTarget = null
const autoFormProxy = new Proxy(
  {},
  {
    get(_target, key) {
      return autoFormTarget ? autoFormTarget[key] : undefined
    },
    set(_target, key, value) {
      if (autoFormTarget) autoFormTarget[key] = value
      return true
    },
    has(_target, key) {
      return autoFormTarget ? key in autoFormTarget : false
    },
    ownKeys() {
      return autoFormTarget ? Reflect.ownKeys(autoFormTarget) : []
    },
    getOwnPropertyDescriptor(_target, key) {
      if (!autoFormTarget || !Object.prototype.hasOwnProperty.call(autoFormTarget, key)) return undefined
      return { configurable: true, enumerable: true, writable: true, value: autoFormTarget[key] }
    }
  }
)

let napFormTarget = null
const napFormProxy = new Proxy(
  {},
  {
    get(_target, key) {
      return napFormTarget ? napFormTarget[key] : undefined
    },
    set(_target, key, value) {
      if (napFormTarget) napFormTarget[key] = value
      return true
    },
    has(_target, key) {
      return napFormTarget ? key in napFormTarget : false
    },
    ownKeys() {
      return napFormTarget ? Reflect.ownKeys(napFormTarget) : []
    },
    getOwnPropertyDescriptor(_target, key) {
      if (!napFormTarget || !Object.prototype.hasOwnProperty.call(napFormTarget, key)) return undefined
      return { configurable: true, enumerable: true, writable: true, value: napFormTarget[key] }
    }
  }
)

let riskFormTarget = null
const riskFormProxy = new Proxy(
  {},
  {
    get(_target, key) {
      return riskFormTarget ? riskFormTarget[key] : undefined
    },
    set(_target, key, value) {
      if (riskFormTarget) riskFormTarget[key] = value
      return true
    },
    has(_target, key) {
      return riskFormTarget ? key in riskFormTarget : false
    },
    ownKeys() {
      return riskFormTarget ? Reflect.ownKeys(riskFormTarget) : []
    },
    getOwnPropertyDescriptor(_target, key) {
      if (!riskFormTarget || !Object.prototype.hasOwnProperty.call(riskFormTarget, key)) return undefined
      return { configurable: true, enumerable: true, writable: true, value: riskFormTarget[key] }
    }
  }
)

function parseAutoProjectSmartInfo(text) {
  const items = []
  ...
  pick([/(?:客户|客户单位|客户名称)[:：\s]*([^\n\r]{2,30})/], '客户单位', 'customer', 90)
  pick([/(?:主机厂|OEM|主机厂商)[:：\s]*([^\n\r]{1,30})/i], '主机厂商', 'oem', 88)
  pick([/(?:车型|配套车型|车型平台)[:：\s]*([^\n\r]{1,60})/], '配套车型', 'vehicleModels', 84)
  pick([/(?:零件|零件名称|品名)[:：\s]*([^\n\r]{1,60})/], '零件名称', 'partName', 88)
  pick([/(?:材料|材质)[:：\s]*([^\n\r]{1,60})/], '所用材料', 'material', 85)
  pick([/(?:材料牌号|材料等级|牌号)[:：\s]*([^\n\r]{1,40})/], '材料牌号', 'materialGrade', 82)
  pick([/(?:供应商|供应商名称)[:：\s]*([^\n\r]{1,40})/], '供应商', 'supplier', 88)
  pick([/(?:竞品|竞争对手)[:：\s]*([^\n\r]{1,40})/], '竞品', 'competitor', 80)
  pick([/(?:颜色|颜色要求)[:：\s]*([^\n\r]{1,20})/], '颜色', 'color', 75)
  ...
  pick([/(?:阶段|项目阶段|当前阶段)[:：\s]*([^\n\r]{1,20})/], '项目阶段', 'stage', 82, (v) => stageMap[v.trim().toLowerCase()] || v.trim())
  pick([/(?:报价状态|报价进度)[:：\s]*([^\n\r]{1,20})/], '报价状态', 'quoteStatus', 82, (v) => v.trim())
  pick([/(?:合同状态|签约状态)[:：\s]*([^\n\r]{1,20})/], '合同状态', 'contractStatus', 82, (v) => v.trim())
  pick([/(?:量产状态|生产状态)[:：\s]*([^\n\r]{1,20})/], '量产状态', 'massStatus', 82, (v) => v.trim())
  pick([/(?:SOP|量产日期|投产日期)[:：\s]*([0-9]{4}[-/年][0-9]{1,2}[-/月][0-9]{1,2})/i], 'SOP', 'sop', 84, (v) => v.replace(/[年月]/g, '-').replace(/\//g, '-'))
  pick([/(?:负责人|项目负责人)[:：\s]*([^\n\r]{1,20})/], '负责人', 'owner', 84)
  pick([/(?:下次跟进|跟进日期|下次联系)[:：\s]*([0-9]{4}[-/年][0-9]{1,2}[-/月][0-9]{1,2})/], '下次跟进', 'nextFollowUp', 84, (v) => v.replace(/[年月]/g, '-').replace(/\//g, '-'))
  pick([/(?:备注|说明)[:：\s]*([^\n\r]{1,120})/], '备注', 'notes', 68)

  return { items, identifiedCount, lowConfCount }
}

function parseNapProjectSmartInfo(text) {
  const items = []
  let identifiedCount = 0
  let lowConfCount = 0
  const pushItem = (key, label, value, confidence) => {
  ...
  pick([/(?:阶段|项目阶段)[:：\s]*([^\n\r]{1,20})/], '阶段', 'stage', 82, (v) => stageMap[v.trim()] || v.trim())
  pick([/(?:报价状态|报价进度)[:：\s]*([^\n\r]{1,20})/], '报价状态', 'quoteStatus', 82)
  pick([/(?:合同状态|签约状态)[:：\s]*([^\n\r]{1,20})/], '合同状态', 'contractStatus', 82)
  pick([/(?:付款状态|回款状态)[:：\s]*([^\n\r]{1,20})/], '付款状态', 'paymentStatus', 82)
  pick([/(?:合同金额|金额)[:：\s]*([0-9][\d,]*(?:\.\d+)?)\s*(?:元|万)?/], '合同金额', 'contractAmount', 88, (v) => Number(String(v).replace(/,/g, '')) || 0)
  pick([/(?:月需求量|月订单量|需求量)[:：\s]*([0-9][\d,]*(?:\.\d+)?)\s*/], '月需求量', 'monthlyOrderQty', 88, (v) => Number(String(v).replace(/,/g, '')) || 0)
  pick([/(?:竞品|竞争对手)[:：\s]*([^\n\r]{1,40})/], '竞品', 'competitor', 80)
  pick([/(?:负责人|项目负责人)[:：\s]*([^\n\r]{1,20})/], '负责人', 'owner', 84)
  pick([/(?:技术要求|技术需求)[:：\s]*([^\n\r]{1,120})/], '技术要求', 'techRequirements', 72)
  pick([/(?:商务要求|商务需求)[:：\s]*([^\n\r]{1,120})/], '商务要求', 'bizRequirements', 72)
  pick([/(?:下次跟进|跟进日期|下次联系)[:：\s]*([0-9]{4}[-/年][0-9]{1,2}[-/月][0-9]{1,2})/], '下次跟进', 'nextFollowUp', 84, (v) => v.replace(/[年月]/g, '-').replace(/\//g, '-'))
  pick([/(?:备注|说明)[:：\s]*([^\n\r]{1,120})/], '备注', 'notes', 68)
...
function parseRiskSmartInfo(text) {
  const items = []
  let identifiedCount = 0
  let lowConfCount = 0
  const pushItem = (key, label, value, confidence) => {
  ...
  pick([/(?:项目ID|项目编号|编号)[:：\s]*([^\n\r]{1,30})/], '项目ID', 'projectId', 90)
  pick([/(?:项目名称|项目)[:：\s]*([^\n\r]{2,50})/], '项目名称', 'projectName', 88)
  pick([/(?:风险描述|描述|问题描述)[:：\s]*([^\n\r]{1,160})/], '风险描述', 'description', 84)
  pick([/(?:负责人|责任人)[:：\s]*([^\n\r]{1,20})/], '负责人', 'owner', 84)
  pick([/(?:应对措施|缓解措施|措施)[:：\s]*([^\n\r]{1,160})/], '应对措施', 'mitigation', 82)
  const levelMap = { 高: '高', 中: '中', 低: '低' }
  pick([/(?:等级|风险等级|级别)[:：\s]*([^\n\r]{1,20})/], '等级', 'level', 82, (v) => levelMap[v.trim()] || v.trim())
  pick([/(?:状态)[:：\s]*([^\n\r]{1,20})/], '状态', 'status', 82)
  pick([/(?:概率|发生概率)[:：\s]*([^\n\r]{1,20})/], '概率', 'probability', 82, (v) => levelMap[v.trim()] || v.trim())
  pick([/(?:影响|影响程度)[:：\s]*([^\n\r]{1,20})/], '影响', 'impact', 82, (v) => levelMap[v.trim()] || v.trim())

  return { items, identifiedCount, lowConfCount }
}

const autoSmartPlaceholder = '粘贴汽车项目文本，系统会识别客户、主机厂、车型、零件、材料、阶段和关键节点。'
const napSmartPlaceholder = '粘贴非汽车项目文本，系统会识别客户、阶段、金额、需求和跟进信息。'
const riskSmartPlaceholder = '粘贴风险描述文本，系统会识别项目、等级、状态、负责人和应对措施。'

const {
  showSmartRec: showAutoSmartRec,
  smartRecInput: autoSmartRecInput,
  smartRecResult: autoSmartRecResult,
  smartRecPlaceholder: autoSmartRecPlaceholder,
  runSmartRecognize: runAutoSmartRecognize,
  applySmartRecognize: applyAutoSmartRecognize,
  handleSmartFileUpload: handleAutoSmartFileUpload,
  resetSmartRec: resetAutoSmartRec
} = useSmartRecognizeBase(autoFormProxy, parseAutoProjectSmartInfo, autoSmartPlaceholder)

const {
  showSmartRec: showNapSmartRec,
  smartRecInput: napSmartRecInput,
  smartRecResult: napSmartRecResult,
  smartRecPlaceholder: napSmartRecPlaceholder,
  runSmartRecognize: runNapSmartRecognize,
  applySmartRecognize: applyNapSmartRecognize,
  handleSmartFileUpload: handleNapSmartFileUpload,
  resetSmartRec: resetNapSmartRec
} = useSmartRecognizeBase(napFormProxy, parseNapProjectSmartInfo, napSmartPlaceholder)

const {
  showSmartRec: showRiskSmartRec,
  smartRecInput: riskSmartRecInput,
  smartRecResult: riskSmartRecResult,
  smartRecPlaceholder: riskSmartRecPlaceholder,
  runSmartRecognize: runRiskSmartRecognize,
  applySmartRecognize: applyRiskSmartRecognize,
  handleSmartFileUpload: handleRiskSmartFileUpload,
  resetSmartRec: resetRiskSmartRec
} = useSmartRecognizeBase(riskFormProxy, parseRiskSmartInfo, riskSmartPlaceholder)
...
function initCharts() {
  if (!Chart || !chartReady || !isAlive) return
  destroyCharts()
...
watch(activeTab, (v) => {
  if (v === 'stats' && chartReady && isAlive) nextTick(() => initCharts())
})
