  ...
  <div class="project-tracking-page">
    ...
    <div class="tab-content-scroll">
      ...
      <div v-show="activeTab === 'auto'">
        ...
        <div v-show="autoView === 'table'" class="panel-card">
          ...
          <div class="panel-card-body no-padding"><div class="table-container"><table class="data-table"><thead><tr>
            ...
            <tr v-for="p in filteredAutoProjects" :key="p.id" :class="{ 'row-overdue': p.nextFollowUp && p.nextFollowUp < today && p.stage !== '量产' }">
              <td><input type="checkbox" :value="p.id" v-model="autoSelected"></td>
              <td v-if="autoColumnVisible.projectId"><span class="project-id">{{ p.projectId }}</span></td>
              <td v-if="autoColumnVisible.customer" class="cell-editable" @dblclick="startInlineEdit(p, 'customer')">
                <span v-if="!isInlineEditing(p.id, 'customer')">{{ p.customer }}</span>
                <input v-else v-model="inlineEdit.value" class="inline-input" @keydown="onInlineKeydown($event, p, 'auto')" @blur="confirmInlineEdit(p, 'auto')" />
              </td>
              ...
              <td v-if="autoColumnVisible.vehicleModels" class="cell-editable" @dblclick="startInlineEdit(p, 'vehicleModels')">
                <span v-if="!isInlineEditing(p.id, 'vehicleModels')">{{ p.vehicleModels }}</span>
                <input v-else v-model="inlineEdit.value" class="inline-input" @keydown="onInlineKeydown($event, p, 'auto')" @blur="confirmInlineEdit(p, 'auto')" />
              </td>
              <td v-if="autoColumnVisible.partName" class="cell-editable" @dblclick="startInlineEdit(p, 'partName')">
                <span v-if="!isInlineEditing(p.id, 'partName')">{{ p.partName }}</span>
                <input v-else v-model="inlineEdit.value" class="inline-input" @keydown="onInlineKeydown($event, p, 'auto')" @blur="confirmInlineEdit(p, 'auto')" />
              </td>
              <td v-if="autoColumnVisible.material" class="cell-editable" @dblclick="startInlineEdit(p, 'material')">
              ...
              <td v-if="autoColumnVisible.stage" class="cell-editable" @dblclick="startInlineEdit(p, 'stage')">
                <span v-if="!isInlineEditing(p.id, 'stage')" class="stage-tag" :class="'stage-' + stageIdx(p.stage)">{{ p.stage }}</span>
                <select v-else v-model="inlineEdit.value" class="inline-select" @change="confirmInlineEdit(p, 'auto')" @blur="confirmInlineEdit(p, 'auto')" @keydown="onInlineKeydown($event, p, 'auto')">
                  <option v-for="s in stages" :key="s" :value="s">{{ s }}</option>
                </select>
              </td>
              <td v-if="autoColumnVisible.quoteStatus" class="cell-editable" @dblclick="startInlineEdit(p, 'quoteStatus')">
                <span v-if="!isInlineEditing(p.id, 'quoteStatus')" class="quote-tag" :class="'qs-' + p.quoteStatus">{{ p.quoteStatus }}</span>
                <select v-else v-model="inlineEdit.value" class="inline-select" @change="confirmInlineEdit(p, 'auto')" @blur="confirmInlineEdit(p, 'auto')" @keydown="onInlineKeydown($event, p, 'auto')">
                  <option v-for="s in quoteStatuses" :key="s" :value="s">{{ s }}</option>
                </select>
              </td>
              <td v-if="autoColumnVisible.massStatus" class="cell-editable" @dblclick="startInlineEdit(p, 'massStatus')">
                <span v-if="!isInlineEditing(p.id, 'massStatus')" class="mass-tag" :class="'ms-' + p.massStatus">{{ p.massStatus }}</span>
                <select v-else v-model="inlineEdit.value" class="inline-select" @change="confirmInlineEdit(p, 'auto')" @blur="confirmInlineEdit(p, 'auto')" @keydown="onInlineKeydown($event, p, 'auto')">
                  <option value="未量产">未量产</option><option value="小批量">小批量</option><option value="量产">量产</option>
                </select>
              </td>
              ...
              <td v-if="autoColumnVisible.followUp" class="cell-editable" @dblclick="startInlineEdit(p, 'nextFollowUp')">
                <span v-if="!isInlineEditing(p.id, 'nextFollowUp')" v-bind="p.nextFollowUp ? { class: { overdue: p.nextFollowUp < today && p.stage !== '量产' } } : {}">{{ p.nextFollowUp || '-' }}</span>
                <input v-else v-model="inlineEdit.value" type="date" class="inline-input" @keydown="onInlineKeydown($event, p, 'auto')" @blur="confirmInlineEdit(p, 'auto')" />
              </td>
              <td><div class="action-btns"><button class="btn btn-ghost btn-sm" @click="openAutoDetail(p)">详情</button><button class="btn btn-ghost btn-sm" @click="editAutoProject(p)">编辑</button><button class="btn btn-ghost btn-sm btn-danger-text" @click="confirmDeleteAuto(p.id)">删除</button></div></td>
            </tr>
        ...
        <div v-show="autoView === 'list'" class="panel-card">
          ...
          <div class="panel-card-body no-padding">
            <div v-for="p in filteredAutoProjects" :key="p.id" class="list-item" :class="{ 'list-item-overdue': p.nextFollowUp && p.nextFollowUp < today && p.stage !== '量产' }" @click="openAutoDetail(p)">
              ...
              <div class="list-item-meta"><span>{{ p.oem }}</span><span>{{ p.owner }}</span><span :class="{ overdue: p.nextFollowUp && p.nextFollowUp < today }">{{ p.nextFollowUp || '-' }}</span></div>
            </div>
        ...
        <div v-show="autoView === 'card'" class="panel-card">
          ...
          <div class="pt-card-grid">
            <div v-for="p in filteredAutoProjects" :key="p.id" class="pt-card" :class="{ 'pt-card-overdue': p.nextFollowUp && p.nextFollowUp < today && p.stage !== '量产' }" @click="openAutoDetail(p)">
              ...
              <div class="stage-progress-mini"><div v-for="(s, i) in stages" :key="s" class="spm-dot" :class="{ active: i <= stages.indexOf(p.stage), current: s === p.stage }" :title="s"></div></div>
              <div class="pt-card-row"><span>车型:</span><span>{{ p.vehicleModels || '-' }}</span></div>
              <div class="pt-card-row"><span>材料:</span><span>{{ p.material || '-' }}</span></div>
              <div class="pt-card-row"><span>报价:</span><span class="quote-tag" :class="'qs-' + p.quoteStatus">{{ p.quoteStatus }}</span></div>
              <div class="pt-card-row"><span>量产:</span><span class="mass-tag" :class="'ms-' + p.massStatus">{{ p.massStatus }}</span></div>
              <div class="pt-card-footer"><span>{{ p.owner }}</span><span :class="{ overdue: p.nextFollowUp && p.nextFollowUp < today }">{{ p.nextFollowUp || '-' }}</span></div>
            </div>
          </div>
          <div v-if="filteredAutoProjects.length === 0" class="empty-cell" style="padding:var(--space-8)">暂无数据</div>
        </div>
      ...
      <div v-show="activeTab === 'nonauto'">
        ...
        <div v-show="nonAutoView === 'table'" class="panel-card">
          ...
          <div class="panel-card-body no-padding"><div class="table-container"><table class="data-table"><thead><tr>
            ...
            <tr v-for="p in filteredNonAutoProjects" :key="p.id" :class="{ 'row-overdue': p.nextFollowUp && p.nextFollowUp < today && p.stage !== '完成' }">
              <td v-if="napColumnVisible.projectId"><span class="project-id">{{ p.projectId }}</span></td>
              <td v-if="napColumnVisible.customerName" class="cell-editable" @dblclick="startInlineEdit(p, 'customerName')">
                <span v-if="!isInlineEditing(p.id, 'customerName')">{{ p.customerName }}</span>
                <input v-else v-model="inlineEdit.value" class="inline-input" @keydown="onInlineKeydown($event, p, 'nonauto')" @blur="confirmInlineEdit(p, 'nonauto')" />
              </td>
              <td v-if="napColumnVisible.techRequirements" class="cell-editable" @dblclick="startInlineEdit(p, 'techRequirements')">
                <span v-if="!isInlineEditing(p.id, 'techRequirements')">{{ p.techRequirements || '-' }}</span>
                <input v-else v-model="inlineEdit.value" class="inline-input" @keydown="onInlineKeydown($event, p, 'nonauto')" @blur="confirmInlineEdit(p, 'nonauto')" />
              </td>
              <td v-if="napColumnVisible.monthlyOrderQty" class="cell-editable" @dblclick="startInlineEdit(p, 'monthlyOrderQty')">
                <span v-if="!isInlineEditing(p.id, 'monthlyOrderQty')">{{ p.monthlyOrderQty || '-' }}</span>
                <input v-else v-model="inlineEdit.value" type="number" class="inline-input" @keydown="onInlineKeydown($event, p, 'nonauto')" @blur="confirmInlineEdit(p, 'nonauto')" />
              </td>
              <td v-if="napColumnVisible.contractAmount" class="cell-editable amount-cell" @dblclick="startInlineEdit(p, 'contractAmount')">
              ...
              <td v-if="napColumnVisible.stage" class="cell-editable" @dblclick="startInlineEdit(p, 'stage')">
                <span v-if="!isInlineEditing(p.id, 'stage')" class="stage-tag" :class="'stage-' + napStageIdx(p.stage)">{{ p.stage }}</span>
                <select v-else v-model="inlineEdit.value" class="inline-select" @change="confirmInlineEdit(p, 'nonauto')" @blur="confirmInlineEdit(p, 'nonauto')" @keydown="onInlineKeydown($event, p, 'nonauto')">
                  <option v-for="s in napStages" :key="s" :value="s">{{ s }}</option>
                </select>
              </td>
              <td v-if="napColumnVisible.quoteStatus" class="cell-editable" @dblclick="startInlineEdit(p, 'quoteStatus')">
                <span v-if="!isInlineEditing(p.id, 'quoteStatus')" class="quote-tag" :class="'qs-' + p.quoteStatus">{{ p.quoteStatus }}</span>
                <select v-else v-model="inlineEdit.value" class="inline-select" @change="confirmInlineEdit(p, 'nonauto')" @blur="confirmInlineEdit(p, 'nonauto')" @keydown="onInlineKeydown($event, p, 'nonauto')">
                  <option v-for="s in quoteStatuses" :key="s" :value="s">{{ s }}</option>
                </select>
              </td>
              <td v-if="napColumnVisible.contractStatus" class="cell-editable" @dblclick="startInlineEdit(p, 'contractStatus')">
                <span v-if="!isInlineEditing(p.id, 'contractStatus')">{{ p.contractStatus || '-' }}</span>
                <input v-else v-model="inlineEdit.value" class="inline-input" @keydown="onInlineKeydown($event, p, 'nonauto')" @blur="confirmInlineEdit(p, 'nonauto')" />
              </td>
              <td v-if="napColumnVisible.paymentStatus" class="cell-editable" @dblclick="startInlineEdit(p, 'paymentStatus')">
                <span v-if="!isInlineEditing(p.id, 'paymentStatus')" class="payment-tag" :class="'ps-' + (p.paymentStatus || '')">{{ p.paymentStatus || '-' }}</span>
                <select v-else v-model="inlineEdit.value" class="inline-select" @change="confirmInlineEdit(p, 'nonauto')" @blur="confirmInlineEdit(p, 'nonauto')" @keydown="onInlineKeydown($event, p, 'nonauto')">
                  <option value="未回款">未回款</option><option value="部分回款">部分回款</option><option value="已回款">已回款</option>
                </select>
              </td>
              ...
              <td v-if="napColumnVisible.followUp" class="cell-editable" @dblclick="startInlineEdit(p, 'nextFollowUp')">
                <span v-if="!isInlineEditing(p.id, 'nextFollowUp')" v-bind="p.nextFollowUp ? { class: { overdue: p.nextFollowUp < today && p.stage !== '完成' } } : {}">{{ p.nextFollowUp || '-' }}</span>
                <input v-else v-model="inlineEdit.value" type="date" class="inline-input" @keydown="onInlineKeydown($event, p, 'nonauto')" @blur="confirmInlineEdit(p, 'nonauto')" />
              </td>
              <td><div class="action-btns"><button class="btn btn-ghost btn-sm" @click="openNonAutoDetail(p)">详情</button><button class="btn btn-ghost btn-sm" @click="editNonAutoProject(p)">编辑</button><button class="btn btn-ghost btn-sm btn-danger-text" @click="confirmDeleteNonAuto(p.id)">删除</button></div></td>
            </tr>
        ...
        <div v-show="nonAutoView === 'list'" class="panel-card">
          ...
          <div class="panel-card-body no-padding">
            <div v-for="p in filteredNonAutoProjects" :key="p.id" class="list-item" :class="{ 'list-item-overdue': p.nextFollowUp && p.nextFollowUp < today && p.stage !== '完成' }" @click="openNonAutoDetail(p)">
              ...
              <div class="list-item-meta"><span>合同: {{ p.contractAmount }}万</span><span>{{ p.owner }}</span><span :class="{ overdue: p.nextFollowUp && p.nextFollowUp < today }">{{ p.nextFollowUp || '-' }}</span></div>
            </div>
        ...
        <div v-show="nonAutoView === 'card'" class="panel-card">
          ...
          <div class="pt-card-grid">
            <div v-for="p in filteredNonAutoProjects" :key="p.id" class="pt-card" :class="{ 'pt-card-overdue': p.nextFollowUp && p.nextFollowUp < today && p.stage !== '完成' }" @click="openNonAutoDetail(p)">
              ...
              <div class="stage-progress-mini"><div v-for="(s, i) in napStages" :key="s" class="spm-dot" :class="{ active: i <= napStages.indexOf(p.stage), current: s === p.stage }" :title="s"></div></div>
              <div class="pt-card-row"><span>月订单:</span><span>{{ p.monthlyOrderQty || 0 }}吨</span></div>
              <div class="pt-card-row"><span>合同金额:</span><span>{{ p.contractAmount || 0 }}万</span></div>
              <div class="pt-card-row"><span>回款:</span><span class="payment-tag" :class="'ps-' + (p.paymentStatus || '')">{{ p.paymentStatus || '-' }}</span></div>
              <div class="pt-card-footer"><span>{{ p.owner }}</span><span :class="{ overdue: p.nextFollowUp && p.nextFollowUp < today }">{{ p.nextFollowUp || '-' }}</span></div>
            </div>
          </div>
          <div v-if="filteredNonAutoProjects.length === 0" class="empty-cell" style="padding:var(--space-8)">暂无数据</div>
      ...
      <div v-show="activeTab === 'risk'">
        ...
        <div class="filter-bar">
          ...
          <select class="form-select" v-model="riskLevelFilter"><option value="">全部等级</option><option value="高">高</option><option value="中">中</option><option value="低">低</option></select>
          <select class="form-select" v-model="riskStatusFilter"><option value="">全部状态</option><option value="未处理">未处理</option><option value="处理中">处理中</option><option value="已解决">已解决</option><option value="已关闭">已关闭</option></select>
            ...
            <div class="panel-card-body no-padding"><div class="table-container"><table class="data-table"><thead><tr>
              ...
              <tr v-for="r in filteredRisks" :key="r.id">
                <td class="cell-editable" @dblclick="startInlineEdit(r, 'projectId')">
                  ...
                  <input v-else v-model="inlineEdit.value" class="inline-input" @keydown="onInlineKeydown($event, r, 'risk')" @blur="confirmInlineEdit(r, 'risk')" />
                </td>
                ...
                <td class="cell-editable" @dblclick="startInlineEdit(r, 'level')">
                  <span v-if="!isInlineEditing(r.id, 'level')" class="risk-level-tag" :class="'rl-' + r.level">{{ r.level }}</span>
                  <select v-else v-model="inlineEdit.value" class="inline-select" @change="confirmInlineEdit(r, 'risk')" @blur="confirmInlineEdit(r, 'risk')" @keydown="onInlineKeydown($event, r, 'risk')">
                ...
                <td class="cell-editable" @dblclick="startInlineEdit(r, 'status')">
                  <span v-if="!isInlineEditing(r.id, 'status')" class="risk-status-tag" :class="'rs-' + r.status">{{ r.status }}</span>
                  <select v-else v-model="inlineEdit.value" class="inline-select" @change="confirmInlineEdit(r, 'risk')" @blur="confirmInlineEdit(r, 'risk')" @keydown="onInlineKeydown($event, r, 'risk')">
                    <option value="未处理">未处理</option><option value="处理中">处理中</option><option value="已解决">已解决</option><option value="已关闭">已关闭</option>
                  </select>
                </td>
                ...
                <td><div class="action-btns"><button class="btn btn-ghost btn-sm" @click="editRisk(r)">编辑</button><button class="btn btn-ghost btn-sm btn-danger-text" @click="confirmDeleteRisk(r.id)">删除</button></div></td>
              </tr>
          ...
          <div class="panel-card">
            ...
            <div class="panel-card-body">
              <div class="risk-matrix">
                ...
                <div class="rm-row"><div class="rm-cell rm-med" :class="{ 'rm-has': rmCount('高','中') }" title="高概率×中影响">{{ rmCount('高','中') }}</div><div class="rm-cell rm-low" :class="{ 'rm-has': rmCount('中','中') }" title="中概率×中影响">{{ rmCount('中','中') }}</div><div class="rm-cell rm-low" :class="{ 'rm-has': rmCount('低','中') }" title="低概率×中影响">{{ rmCount('低','中') }}</div><div class="rm-row-label">影响 中</div></div>
                <div class="rm-row"><div class="rm-cell rm-low" :class="{ 'rm-has': rmCount('高','低') }" title="高概率×低影响">{{ rmCount('高','低') }}</div><div class="rm-cell rm-low" :class="{ 'rm-has': rmCount('中','低') }" title="中概率×低影响">{{ rmCount('中','低') }}</div><div class="rm-cell rm-low" :class="{ 'rm-has': rmCount('低','低') }" title="低概率×低影响">{{ rmCount('低','低') }}</div><div class="rm-row-label">影响 低</div></div>
                <div class="rm-blank"></div>
                <div class="rm-col-label">概率 高</div><div class="rm-col-label">概率 中</div><div class="rm-col-label">概率 低</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="content-grid content-grid-2-1" style="margin-bottom:var(--space-4)">
          ...
          <div>
            <div class="panel-card" style="margin-bottom:var(--space-3)"><div class="panel-card-header"><span class="panel-card-title"><Icon name="trendUp" :size="14" /> 统计概览</span></div><div class="panel-card-body">
              ...
              <div class="stat-overview-item"><span class="stat-overview-label">已量产项目</span><span class="stat-overview-value" style="color:var(--color-success)">{{ autoMassCount }}</span></div>
        ...
        <div class="detail-panel">
          ...
          <div class="detail-panel-body">
            ...
            <div v-if="detailType === 'auto'" class="detail-grid">
              <div class="detail-item"><span class="detail-label">项目ID</span><span class="detail-value">{{ detailData.projectId }}</span></div>
              <div class="detail-item"><span class="detail-label">客户</span><span class="detail-value">{{ detailData.customer }}</span></div>
              <div class="detail-item"><span class="detail-label">主机厂</span><span class="detail-value">{{ detailData.oem }}</span></div>
              <div class="detail-item"><span class="detail-label">配套车型</span><span class="detail-value">{{ detailData.vehicleModels }}</span></div>
              <div class="detail-item"><span class="detail-label">零件名称</span><span class="detail-value">{{ detailData.partName }}</span></div>
              <div class="detail-item"><span class="detail-label">材料</span><span class="detail-value">{{ detailData.material }}</span></div>
              <div class="detail-item"><span class="detail-label">材料牌号</span><span class="detail-value">{{ detailData.materialGrade || '-' }}</span></div>
              <div class="detail-item"><span class="detail-label">配套供应商</span><span class="detail-value">{{ detailData.supplier || '-' }}</span></div>
              <div class="detail-item"><span class="detail-label">竞争对手</span><span class="detail-value">{{ detailData.competitor || '-' }}</span></div>
              <div class="detail-item"><span class="detail-label">颜色</span><span class="detail-value">{{ detailData.color || '-' }}</span></div>
              <div class="detail-item"><span class="detail-label">项目阶段</span><span class="detail-value"><span class="stage-tag" :class="'stage-' + stageIdx(detailData.stage)">{{ detailData.stage }}</span></span></div>
              <div class="detail-item"><span class="detail-label">报价状态</span><span class="detail-value"><span class="quote-tag" :class="'qs-' + detailData.quoteStatus">{{ detailData.quoteStatus }}</span></span></div>
              <div class="detail-item"><span class="detail-label">合同状态</span><span class="detail-value">{{ detailData.contractStatus || '-' }}</span></div>
              <div class="detail-item"><span class="detail-label">量产状态</span><span class="detail-value"><span class="mass-tag" :class="'ms-' + detailData.massStatus">{{ detailData.massStatus }}</span></span></div>
              <div class="detail-item"><span class="detail-label">SOP日期</span><span class="detail-value">{{ detailData.sop || '-' }}</span></div>
              <div class="detail-item"><span class="detail-label">负责人</span><span class="detail-value">{{ detailData.owner }}</span></div>
              <div class="detail-item"><span class="detail-label">下次跟进</span><span class="detail-value" :class="{ overdue: detailData.nextFollowUp && detailData.nextFollowUp < today }">{{ detailData.nextFollowUp || '-' }}</span></div>
            </div>
            <div v-else class="detail-grid">
              <div class="detail-item"><span class="detail-label">项目ID</span><span class="detail-value">{{ detailData.projectId }}</span></div>
              <div class="detail-item"><span class="detail-label">客户名称</span><span class="detail-value">{{ detailData.customerName }}</span></div>
              <div class="detail-item"><span class="detail-label">项目阶段</span><span class="detail-value"><span class="stage-tag" :class="'stage-' + napStageIdx(detailData.stage)">{{ detailData.stage }}</span></span></div>
              <div class="detail-item"><span class="detail-label">报价状态</span><span class="detail-value"><span class="quote-tag" :class="'qs-' + detailData.quoteStatus">{{ detailData.quoteStatus }}</span></span></div>
              <div class="detail-item"><span class="detail-label">合同状态</span><span class="detail-value">{{ detailData.contractStatus || '-' }}</span></div>
              <div class="detail-item"><span class="detail-label">竞争对手</span><span class="detail-value">{{ detailData.competitor || '-' }}</span></div>
              <div class="detail-item"><span class="detail-label">月订单量(吨)</span><span class="detail-value">{{ detailData.monthlyOrderQty || '-' }}</span></div>
              <div class="detail-item"><span class="detail-label">合同金额(万)</span><span class="detail-value">{{ detailData.contractAmount || '-' }}</span></div>
              <div class="detail-item"><span class="detail-label">回款状态</span><span class="detail-value"><span class="payment-tag" :class="'ps-' + (detailData.paymentStatus || '')">{{ detailData.paymentStatus || '-' }}</span></span></div>
              <div class="detail-item"><span class="detail-label">负责人</span><span class="detail-value">{{ detailData.owner }}</span></div>
              <div class="detail-item"><span class="detail-label">下次跟进</span><span class="detail-value" :class="{ overdue: detailData.nextFollowUp && detailData.nextFollowUp < today }">{{ detailData.nextFollowUp || '-' }}</span></div>
            </div>
            <div v-if="detailData?.techRequirements" class="detail-section"><h4 class="detail-section-title">技术要求</h4><pre class="detail-pre">{{ detailData.techRequirements }}</pre></div>
            <div v-if="detailData?.bizRequirements" class="detail-section"><h4 class="detail-section-title">商务要求</h4><pre class="detail-pre">{{ detailData.bizRequirements }}</pre></div>
            <div v-if="detailData?.notes" class="detail-section"><h4 class="detail-section-title">备注</h4><pre class="detail-pre">{{ detailData.notes }}</pre></div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-if="showAutoModal" class="modal-overlay" @click.self="showAutoModal = false">
      <div class="modal-content modal-wide">
        ...
        <div class="modal-body">
          ...
          <div class="form-row form-row-3"><div class="form-group"><label class="form-label">配套供应商</label><input class="form-input" v-model="autoForm.supplier"></div><div class="form-group"><label class="form-label">竞争对手</label><input class="form-input" v-model="autoForm.competitor"></div><div class="form-group"><label class="form-label">颜色</label><input class="form-input" v-model="autoForm.color"></div></div>
          <div class="form-row form-row-3"><div class="form-group"><label class="form-label">项目阶段</label><select class="form-select" v-model="autoForm.stage"><option v-for="s in stages" :key="s" :value="s">{{ s }}</option></select></div><div class="form-group"><label class="form-label">报价状态</label><select class="form-select" v-model="autoForm.quoteStatus"><option v-for="s in quoteStatuses" :key="s" :value="s">{{ s }}</option></select></div><div class="form-group"><label class="form-label">合同签订状态</label><select class="form-select" v-model="autoForm.contractStatus"><option value="未签订">未签订</option><option value="签订中">签订中</option><option value="已签订">已签订</option></select></div></div>
          <div class="form-row form-row-3"><div class="form-group"><label class="form-label">量产状态</label><select class="form-select" v-model="autoForm.massStatus"><option value="未量产">未量产</option><option value="小批量">小批量</option><option value="量产">量产</option></select></div><div class="form-group"><label class="form-label">SOP日期</label><input class="form-input" type="date" v-model="autoForm.sop"></div><div class="form-group"><label class="form-label">负责人</label><input class="form-input" v-model="autoForm.owner"></div></div>
          <div class="form-row form-row-2"><div class="form-group"><label class="form-label">下次跟进时间</label><input class="form-input" type="date" v-model="autoForm.nextFollowUp"></div><div class="form-group"><label class="form-label">备注</label><textarea class="form-input form-textarea" v-model="autoForm.notes" rows="2"></textarea></div></div>
        </div>
        <div class="modal-footer"><button class="btn btn-ghost" @click="showAutoModal = false">取消</button><button class="btn btn-primary" @click="saveAutoProject">保存</button></div>
      </div>
    </div>
    ...
    <div v-if="showNapModal" class="modal-overlay" @click.self="showNapModal = false">
      <div class="modal-content modal-wide">
        ...
        <div class="modal-body">
          ...
          <div class="form-row form-row-3"><div class="form-group"><label class="form-label">合同状态</label><select class="form-select" v-model="napForm.contractStatus"><option value="未签订">未签订</option><option value="签订中">签订中</option><option value="已签订">已签订</option></select></div><div class="form-group"><label class="form-label">竞争对手</label><input class="form-input" v-model="napForm.competitor"></div><div class="form-group"><label class="form-label">负责人</label><input class="form-input" v-model="napForm.owner"></div></div>
          <div class="form-row form-row-2"><div class="form-group"><label class="form-label">技术要求</label><textarea class="form-input form-textarea" v-model="napForm.techRequirements" rows="2"></textarea></div><div class="form-group"><label class="form-label">商务要求</label><textarea class="form-input form-textarea" v-model="napForm.bizRequirements" rows="2"></textarea></div></div>
          <div class="form-row form-row-2"><div class="form-group"><label class="form-label">下次跟进时间</label><input class="form-input" type="date" v-model="napForm.nextFollowUp"></div><div class="form-group"><label class="form-label">备注</label><textarea class="form-input form-textarea" v-model="napForm.notes" rows="2"></textarea></div></div>
        </div>
        <div class="modal-footer"><button class="btn btn-ghost" @click="showNapModal = false">取消</button><button class="btn btn-primary" @click="saveNapProject">保存</button></div>
      </div>
    </div>
    ...
    <div v-if="showRiskModal" class="modal-overlay" @click.self="showRiskModal = false">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <div class="form-group"><label class="form-label">风险描述</label><textarea class="form-input form-textarea" v-model="riskForm.description" rows="3"></textarea></div>
          <div class="form-row form-row-2"><div class="form-group"><label class="form-label">概率</label><select class="form-select" v-model="riskForm.probability"><option value="高">高</option><option value="中">中</option><option value="低">低</option></select></div><div class="form-group"><label class="form-label">影响</label><select class="form-select" v-model="riskForm.impact"><option value="高">高</option><option value="中">中</option><option value="低">低</option></select></div></div>
          <div class="form-group"><label class="form-label">应对措施</label><textarea class="form-input form-textarea" v-model="riskForm.mitigation" rows="2"></textarea></div>
        </div>
        <div class="modal-footer"><button class="btn btn-ghost" @click="showRiskModal = false">取消</button><button class="btn btn-primary" @click="saveRisk">保存</button></div>
      </div>
    </div>
    ...
    <div v-if="showOpLogModal" class="modal-overlay" @click.self="showOpLogModal = false">
      <div class="modal-content modal-wide">
        ...
        <div class="modal-footer"><button class="btn btn-ghost" @click="opLogs = []; persistOpLogs()">清空日志</button><button class="btn btn-ghost" @click="showOpLogModal = false">关闭</button></div>
      </div>
    </div>
...
function persistLS(key, data) {
  try { localStorage.setItem(key, JSON.stringify(data)) } catch (e) { console.warn('存储失败:', e) }
}
...
function stageIdx(s) { return Math.min(stages.indexOf(s), stages.length - 1) }
function napStageIdx(s) { return Math.min(napStages.indexOf(s), napStages.length - 1) }
...
const autoOverdueCount = computed(() => autoProjects.value.filter(p => p.nextFollowUp && p.nextFollowUp < today && p.stage !== '量产').length)

const napActiveCount = computed(() => nonAutoProjects.value.filter(p => !['完成'].includes(p.stage)).length)
const napOverdueCount = computed(() => nonAutoProjects.value.filter(p => p.nextFollowUp && p.nextFollowUp < today && p.stage !== '完成').length)
const nonAutoTotalAmount = computed(() => nonAutoProjects.value.reduce((s, p) => s + (Number(p.contractAmount) || 0), 0))
const nonAutoMonthlyQty = computed(() => nonAutoProjects.value.reduce((s, p) => s + (Number(p.monthlyOrderQty) || 0), 0))
...
const resolvedRiskCount = computed(() => projectRisks.value.filter(r => ['已解决', '已关闭'].includes(r.status)).length)

function toggleAutoQuickFilter(f) { autoQuickFilter.value = autoQuickFilter.value === f ? '' : f }
function toggleNapQuickFilter(f) { napQuickFilter.value = napQuickFilter.value === f ? '' : f }

const filteredAutoProjects = computed(() => {
  let list = autoProjects.value
  const q = autoSearch.value.toLowerCase()
  if (q) list = list.filter(p => [p.customer, p.oem, p.partName, p.projectId, p.vehicleModels].some(v => (v || '').toLowerCase().includes(q)))
  ...
  if (autoQuickFilter.value === 'overdue') list = list.filter(p => p.nextFollowUp && p.nextFollowUp < today && p.stage !== '量产')
  if (autoQuickFilter.value === 'sopThisMonth') { const m = new Date().toISOString().slice(0, 7); list = list.filter(p => p.sop && p.sop.startsWith(m)) }
...
const filteredNonAutoProjects = computed(() => {
  ...
  if (napQuickFilter.value === 'overdue') list = list.filter(p => p.nextFollowUp && p.nextFollowUp < today && p.stage !== '完成')
...
const filteredRisks = computed(() => {
  ...
  if (q) list = list.filter(r => [r.projectId, r.projectName, r.description].some(v => (v || '').toLowerCase().includes(q)))
...
function rmCount(prob, imp) { return projectRisks.value.filter(r => r.probability === prob && r.impact === imp).length }

const showAutoModal = ref(false)
const isAutoEditing = ref(false)
const editingAutoId = ref(null)
const autoForm = ref({ customer: '', oem: '', vehicleModels: '', partName: '', material: '', materialGrade: '', supplier: '', competitor: '', color: '', stage: '立项', quoteStatus: '未报价', contractStatus: '未签订', massStatus: '未量产', sop: '', owner: '', nextFollowUp: '', notes: '' })

const showNapModal = ref(false)
const isNapEditing = ref(false)
const editingNapId = ref(null)
const napForm = ref({ customerName: '', stage: '接触', quoteStatus: '未报价', contractStatus: '未签订', contractAmount: 0, monthlyOrderQty: 0, paymentStatus: '未回款', competitor: '', owner: '', techRequirements: '', bizRequirements: '', nextFollowUp: '', notes: '' })

const showRiskModal = ref(false)
const isRiskEditing = ref(false)
const editingRiskId = ref(null)
const riskForm = ref({ projectId: '', projectName: '', description: '', level: '中', status: '未处理', probability: '中', impact: '中', owner: '', mitigation: '' })
...
function persistOpLogs() { persistLS(OP_LOG_KEY, opLogs.value) }
...
function openAddModal() {
  if (activeTab.value === 'auto') { isAutoEditing.value = false; editingAutoId.value = null; autoForm.value = { customer: '', oem: '', vehicleModels: '', partName: '', material: '', materialGrade: '', supplier: '', competitor: '', color: '', stage: '立项', quoteStatus: '未报价', contractStatus: '未签订', massStatus: '未量产', sop: '', owner: '', nextFollowUp: '', notes: '' }; showAutoModal.value = true }
  else if (activeTab.value === 'nonauto') { isNapEditing.value = false; editingNapId.value = null; napForm.value = { customerName: '', stage: '接触', quoteStatus: '未报价', contractStatus: '未签订', contractAmount: 0, monthlyOrderQty: 0, paymentStatus: '未回款', competitor: '', owner: '', techRequirements: '', bizRequirements: '', nextFollowUp: '', notes: '' }; showNapModal.value = true }
}

function editAutoProject(p) { isAutoEditing.value = true; editingAutoId.value = p.id; autoForm.value = { ...p }; showAutoModal.value = true }
function editNonAutoProject(p) { isNapEditing.value = true; editingNapId.value = p.id; napForm.value = { ...p }; showNapModal.value = true }
...
function saveAutoProject() {
  ...
  if (isAutoEditing.value) {
    ...
    if (idx !== -1) { autoProjects.value[idx] = { ...autoProjects.value[idx], ...f }; addOpLog('edit', '编辑', `编辑汽车项目 ${f.projectId || f.partName}`) }
  } else {
    const newP = { id: Date.now(), projectId: genAutoId(), ...f, hasRisk: false, createdAt: new Date().toISOString() }
    autoProjects.value.unshift(newP); addOpLog('add', '新增', `新增汽车项目 ${newP.projectId}`)
  }
  persistLS(AUTO_KEY, autoProjects.value); showAutoModal.value = false
}
...
function saveNapProject() {
  ...
  if (isNapEditing.value) {
    ...
    if (idx !== -1) { nonAutoProjects.value[idx] = { ...nonAutoProjects.value[idx], ...f }; addOpLog('edit', '编辑', `编辑非汽车项目 ${f.projectId || f.customerName}`) }
  } else {
    const newP = { id: Date.now(), projectId: genNapId(), ...f, hasRisk: false, createdAt: new Date().toISOString() }
    nonAutoProjects.value.unshift(newP); addOpLog('add', '新增', `新增非汽车项目 ${newP.projectId}`)
  }
  persistLS(NONAUTO_KEY, nonAutoProjects.value); showNapModal.value = false
}

function openRiskAddModal() { isRiskEditing.value = false; editingRiskId.value = null; riskForm.value = { projectId: '', projectName: '', description: '', level: '中', status: '未处理', probability: '中', impact: '中', owner: '', mitigation: '' }; showRiskModal.value = true }
function editRisk(r) { isRiskEditing.value = true; editingRiskId.value = r.id; riskForm.value = { ...r }; showRiskModal.value = true }

function saveRisk() {
  const f = riskForm.value
  if (isRiskEditing.value) {
    const idx = projectRisks.value.findIndex(r => r.id === editingRiskId.value)
    if (idx !== -1) { projectRisks.value[idx] = { ...projectRisks.value[idx], ...f }; addOpLog('edit', '编辑', `编辑风险 ${f.projectId}`) }
  } else {
    projectRisks.value.unshift({ id: Date.now(), ...f, createdAt: new Date().toISOString() }); addOpLog('add', '新增', `新增风险 ${f.projectId}`)
  }
  persistLS(RISK_KEY, projectRisks.value); showRiskModal.value = false
}
...
function confirmInlineEdit(item, type) {
  ...
  if (type === 'auto') { list = autoProjects; key = AUTO_KEY; logName = '汽车项目' }
  else if (type === 'nonauto') { list = nonAutoProjects; key = NONAUTO_KEY; logName = '非汽车项目' }
  else { list = projectRisks; key = RISK_KEY; logName = '风险' }
...
function confirmDeleteAuto(id) { confirmMsg.value = '确定要删除该汽车项目吗？此操作不可恢复。'; confirmAction = () => { autoProjects.value = autoProjects.value.filter(p => p.id !== id); persistLS(AUTO_KEY, autoProjects.value); addOpLog('delete', '删除', '删除汽车项目'); }; showConfirmDialog.value = true }
function confirmDeleteNonAuto(id) { confirmMsg.value = '确定要删除该非汽车项目吗？此操作不可恢复。'; confirmAction = () => { nonAutoProjects.value = nonAutoProjects.value.filter(p => p.id !== id); persistLS(NONAUTO_KEY, nonAutoProjects.value); addOpLog('delete', '删除', '删除非汽车项目'); }; showConfirmDialog.value = true }
function confirmDeleteRisk(id) { confirmMsg.value = '确定要删除该风险记录吗？此操作不可恢复。'; confirmAction = () => { projectRisks.value = projectRisks.value.filter(r => r.id !== id); persistLS(RISK_KEY, projectRisks.value); addOpLog('delete', '删除', '删除风险记录'); }; showConfirmDialog.value = true }
function confirmBatchDeleteAuto() { confirmMsg.value = `确定要批量删除选中的 ${autoSelected.value.length} 个汽车项目吗？`; confirmAction = () => { autoProjects.value = autoProjects.value.filter(p => !autoSelected.value.includes(p.id)); persistLS(AUTO_KEY, autoProjects.value); autoSelected.value = []; addOpLog('batchDelete', '批量删除', '批量删除汽车项目'); }; showConfirmDialog.value = true }
function doConfirm() { if (confirmAction) confirmAction(); showConfirmDialog.value = false; confirmAction = null }

function toggleAutoSelectAll(e) { autoSelected.value = e.target.checked ? filteredAutoProjects.value.map(p => p.id) : [] }
function batchChangeStage() { if (!batchStageVal.value) return; autoSelected.value.forEach(id => { const p = autoProjects.value.find(x => x.id === id); if (p) p.stage = batchStageVal.value }); persistLS(AUTO_KEY, autoProjects.value); batchStageVal.value = ''; autoSelected.value = []; addOpLog('batchEdit', '批量修改', '批量修改项目阶段') }

function openAutoDetail(p) { detailType.value = 'auto'; detailData.value = { ...p }; showDetailPanel.value = true }
...
function exportCSV() {
  ...
  const headers = isAuto ? ['项目编码','客户单位','主机厂商','配套车型型号','零部件名称','所用材料','项目所处阶段','报价跟进状态','量产执行状态','项目负责人','下次跟进'] : ['项目ID','客户名称','项目阶段','报价状态','合同金额','月订单量','回款状态','负责人','下次跟进']
  const rows = data.map(p => isAuto ? [p.projectId,p.customer,p.oem,p.vehicleModels,p.partName,p.material,p.stage,p.quoteStatus,p.massStatus,p.owner,p.nextFollowUp] : [p.projectId,p.customerName,p.stage,p.quoteStatus,p.contractAmount,p.monthlyOrderQty,p.paymentStatus,p.owner,p.nextFollowUp])
  const csv = [headers, ...rows].map(r => r.map(c => `"${(c ?? '').toString().replace(/"/g, '""')}"`).join(',')).join('\n')
  const blob = new Blob(['\uFEFF' + csv], { type: 'text/csv;charset=utf-8' })
  const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `${isAuto ? '汽车项目' : '非汽车项目'}_${today}.csv`; a.click(); URL.revokeObjectURL(a.href)
...
function importCSV(e) {
  ...
  const reader = new FileReader(); reader.onload = (ev) => {
    const lines = ev.target.result.split('\n').filter(l => l.trim()); if (lines.length < 2) { alert('CSV文件为空'); return }
    const isAuto = activeTab.value === 'auto'
    for (let i = 1; i < lines.length; i++) {
      const clean = lines[i].split(',').map(c => c.replace(/^"|"$/g, '').trim())
      if (isAuto) {
        autoProjects.value.unshift({ id: Date.now() + i, projectId: clean[0] || genAutoId(), customer: clean[1] || '', oem: clean[2] || '', vehicleModels: clean[3] || '', partName: clean[4] || '', material: clean[5] || '', stage: clean[6] || '立项', quoteStatus: clean[7] || '未报价', massStatus: clean[8] || '未量产', owner: clean[9] || '', nextFollowUp: clean[10] || '', hasRisk: false })
      } else {
        nonAutoProjects.value.unshift({ id: Date.now() + i, projectId: clean[0] || genNapId(), customerName: clean[1] || '', stage: clean[2] || '接触', quoteStatus: clean[3] || '未报价', contractAmount: Number(clean[4]) || 0, monthlyOrderQty: Number(clean[5]) || 0, paymentStatus: clean[6] || '未回款', owner: clean[7] || '', nextFollowUp: clean[8] || '', hasRisk: false })
      }
    }
...
function printView() { window.print(); addOpLog('print', '打印', '打印报表') }
...
function calToday() { const d = new Date(); calYear.value = d.getFullYear(); calMonth.value = d.getMonth() + 1 }
...
const calCells = computed(() => {
  ...
  for (let d = 1; d <= lastDay; d++) {
    ...
    const events = autoProjects.value.filter(p => (p.nextFollowUp === dateStr || p.sop === dateStr)).map(p => ({ id: p.id, label: p.partName || p.projectId, type: p.sop === dateStr ? 'sop' : 'follow' }))
...
function weekToday() { weekOffset.value = 0 }
const weekDates = computed(() => {
  const now = new Date(); const dow = (now.getDay() + 6) % 7
  const mon = new Date(now); mon.setDate(now.getDate() - dow + weekOffset.value * 7)
  return Array.from({ length: 7 }, (_, i) => { const d = new Date(mon); d.setDate(mon.getDate() + i); return `${d.getMonth() + 1}/${d.getDate()}` })
})
const weekRangeLabel = computed(() => { const d = weekDates.value; return d.length ? `${d[0]} ~ ${d[6]}` : '' })
const weekDaysData = computed(() => {
  ...
  return Array.from({ length: 7 }, (_, i) => {
    ...
    return { isToday: dateStr === today, events: autoProjects.value.filter(p => p.nextFollowUp === dateStr || p.sop === dateStr).map(p => ({ id: p.id, label: p.partName || p.projectId, type: p.sop === dateStr ? 'sop' : 'follow', meta: p.owner })) }
  })
})

const napCalYear = ref(new Date().getFullYear())
const napCalMonth = ref(new Date().getMonth() + 1)
function napCalPrev() { if (napCalMonth.value === 1) { napCalMonth.value = 12; napCalYear.value-- } else napCalMonth.value-- }
function napCalNext() { if (napCalMonth.value === 12) { napCalMonth.value = 1; napCalYear.value++ } else napCalMonth.value++ }
function napCalToday() { const d = new Date(); napCalYear.value = d.getFullYear(); napCalMonth.value = d.getMonth() + 1 }
...
const napCalCells = computed(() => {
  ...
  for (let d = 1; d <= lastDay; d++) {
    ...
    const isToday = dateStr === today
    const events = nonAutoProjects.value.filter(p => p.nextFollowUp === dateStr).map(p => ({ id: p.id, label: p.customerName || p.projectId, type: 'follow' }))
    cells.push({ day: d, events, isToday })
  }
  while (cells.length % 7 !== 0) cells.push({ day: 0, events: [], isToday: false })
  return cells
...
function napWeekPrev() { napWeekOffset.value-- }
function napWeekNext() { napWeekOffset.value++ }
function napWeekToday() { napWeekOffset.value = 0 }
const napWeekDates = computed(() => {
  const now = new Date(); const dow = (now.getDay() + 6) % 7
  const mon = new Date(now); mon.setDate(now.getDate() - dow + napWeekOffset.value * 7)
  return Array.from({ length: 7 }, (_, i) => { const d = new Date(mon); d.setDate(mon.getDate() + i); return `${d.getMonth() + 1}/${d.getDate()}` })
})
const napWeekRangeLabel = computed(() => { const d = napWeekDates.value; return d.length ? `${d[0]} ~ ${d[6]}` : '' })
const napWeekDaysData = computed(() => {
  ...
  return Array.from({ length: 7 }, (_, i) => {
    const d = new Date(mon); d.setDate(mon.getDate() + i)
    const dateStr = d.toISOString().slice(0, 10)
    return { isToday: dateStr === today, events: nonAutoProjects.value.filter(p => p.nextFollowUp === dateStr).map(p => ({ id: p.id, label: p.customerName || p.projectId, type: 'follow', meta: p.owner })) }
  })
})
...
function initCharts() {
  ...
  const chartOpts = { responsive: true, maintainAspectRatio: false, plugins: { legend: { labels: { color: textColor } } }, scales: { x: { ticks: { color: textColor }, grid: { color: gridColor } }, y: { ticks: { color: textColor }, grid: { color: gridColor } } } }

  if (stageChartRef.value) {
    const stageCounts = stages.map(s => autoProjects.value.filter(p => p.stage === s).length)
    charts.push(new Chart(stageChartRef.value, { type: 'bar', data: { labels: stages, datasets: [{ label: '汽车项目', data: stageCounts, backgroundColor: 'rgba(59,130,246,0.6)' }] }, options: chartOpts }))
  }
  if (trendChartRef.value) {
    ...
    const qtyData = months.map(m => nonAutoProjects.value.filter(p => p.createdAt && p.createdAt.startsWith(m)).reduce((s, p) => s + (Number(p.monthlyOrderQty) || 0), 0))
    charts.push(new Chart(trendChartRef.value, { type: 'line', data: { labels: months, datasets: [{ label: '月订单量', data: qtyData, borderColor: '#3b82f6', backgroundColor: 'rgba(59,130,246,0.1)', fill: true, tension: 0.3 }] }, options: chartOpts }))
  }
  if (customerChartRef.value) {
    const custMap = {}; autoProjects.value.forEach(p => { custMap[p.customer] = (custMap[p.customer] || 0) + 1 })
    const labels = Object.keys(custMap).slice(0, 8); const data = labels.map(l => custMap[l])
    charts.push(new Chart(customerChartRef.value, { type: 'doughnut', data: { labels, datasets: [{ data, backgroundColor: ['#3b82f6','#22c55e','#f59e0b','#ef4444','#a855f7','#06b6d4','#f43f5e','#6366f1'] }] }, options: { responsive: true, maintainAspectRatio: false, plugins: { legend: { position: 'bottom', labels: { color: textColor } } } } }))
  }
  if (ownerChartRef.value) {
    const ownerMap = {}; [...autoProjects.value, ...nonAutoProjects.value].forEach(p => { if (p.owner) ownerMap[p.owner] = (ownerMap[p.owner] || 0) + 1 })
    const labels = Object.keys(ownerMap); const data = labels.map(l => ownerMap[l])
    charts.push(new Chart(ownerChartRef.value, { type: 'bar', data: { labels, datasets: [{ label: '项目数', data, backgroundColor: 'rgba(34,197,94,0.6)' }] }, options: { ...chartOpts, indexAxis: 'y' } }))
  }
  if (funnelChartRef.value) {
    const funnelData = stages.map(s => autoProjects.value.filter(p => stages.indexOf(p.stage) >= stages.indexOf(s)).length)
    charts.push(new Chart(funnelChartRef.value, { type: 'bar', data: { labels: stages, datasets: [{ label: '累计项目数', data: funnelData, backgroundColor: 'rgba(168,85,247,0.6)' }] }, options: chartOpts }))
  }
}
...
<style scoped>
.project-tracking-page { display:flex; flex-direction:column; height:100%; min-height:0; }
.page-header { display:flex; justify-content:space-between; align-items:flex-start; padding:var(--space-4) var(--space-6); border-bottom:1px solid var(--color-border); flex-shrink:0; flex-wrap:wrap; gap:var(--space-3); }
.page-header-title { font-size:var(--font-size-2xl); font-weight:700; color:var(--color-text-primary); }
.page-header-subtitle { font-size:var(--font-size-sm); color:var(--color-text-tertiary); margin-top:var(--space-1); }
.page-header-actions { display:flex; gap:var(--space-2); flex-wrap:wrap; }
.tab-bar { display:flex; gap:var(--space-2); border-bottom:1px solid var(--color-border); padding:0 var(--space-6); flex-shrink:0; }
.tab-btn { padding:var(--space-2) var(--space-4); border:1px solid var(--color-border); background:var(--color-surface); color:var(--color-text-secondary); font-size:var(--font-size-base); cursor:pointer; border-radius:var(--radius-md); transition:all 0.2s; }
.tab-btn:hover { color:var(--color-text-primary); border-color:var(--color-accent); }
.tab-btn.active { color:var(--color-accent); border-color:var(--color-accent); background:var(--color-accent-subtle); font-weight:600; }
.tab-content-scroll { flex:1; overflow-y:auto; padding:var(--space-4) var(--space-6); min-height:0; }
.stats-row { display:flex; gap:var(--space-3); margin-bottom:var(--space-4); flex-wrap:wrap; }
.stat-card { display:flex; align-items:center; gap:var(--space-3); padding:var(--space-3) var(--space-4); background:var(--color-surface); border:1px solid var(--color-border); border-radius:var(--radius-lg); flex:1; min-width:140px; animation: statCardIn 0.4s ease-out both; transition: transform 0.2s ease, box-shadow 0.2s ease; }
...
.stat-card-icon { width:40px; height:40px; border-radius:var(--radius-md); display:flex; align-items:center; justify-content:center; font-size:1.2rem; flex-shrink:0; transition: transform 0.2s ease; }
.stat-card:hover .stat-card-icon { transform: scale(1.1); }
.stat-card-body { display:flex; flex-direction:column; }
.stat-card-value { font-size:var(--font-size-xl); font-weight:700; color:var(--color-text-primary); line-height:1.2; font-family:var(--font-mono); }
.stat-card-label { font-size:var(--font-size-sm); color:var(--color-text-tertiary); }
.quick-filters { display:flex; gap:var(--space-2); margin-bottom:var(--space-3); flex-wrap:wrap; }
.quick-filter-tag { padding:var(--space-1) var(--space-3); border:1px solid var(--color-border-light, var(--color-border)); border-radius:20px; background:var(--color-surface); color:var(--color-text-primary); cursor:pointer; font-size:var(--font-size-sm); transition:all 0.2s; display:flex; align-items:center; gap:var(--space-1); }
.quick-filter-tag:hover { border-color:var(--color-accent); color:var(--color-accent); }
.quick-filter-tag.active { background:var(--color-accent-subtle); border-color:var(--color-accent); color:var(--color-accent-hover, var(--color-accent)); font-weight:600; }
.quick-filter-tag.clear { color:var(--color-danger); border-color:var(--color-danger); }
.qf-badge { background:var(--color-danger); color:#fff; border-radius:10px; padding:0 var(--space-2); font-size:12px; min-width:18px; text-align:center; line-height:18px; }
.filter-bar { display:flex; gap:var(--space-2); margin-bottom:var(--space-4); flex-wrap:wrap; align-items:center; }
...
.view-switcher { display:flex; gap:var(--space-1); border-radius:var(--radius-md); padding:0; }
.view-btn { padding:var(--space-2) var(--space-3); border:1px solid var(--color-border); background:var(--color-surface); color:var(--color-text-secondary); font-size:var(--font-size-sm); cursor:pointer; border-radius:var(--radius-md); transition:all 0.2s; white-space:nowrap; }
.view-btn:hover { color:var(--color-text-primary); border-color:var(--color-accent); }
.view-btn.active { background:var(--color-accent); color:#fff; border-color:var(--color-accent); }
.panel-card { background:var(--color-surface); border:1px solid var(--color-border); border-radius:var(--radius-lg); margin-bottom:var(--space-4); }
...
.panel-card-title { font-size:var(--font-size-base); font-weight:600; color:var(--color-text-primary); }
.panel-card-count { font-size:var(--font-size-sm); color:var(--color-text-tertiary); }
...
.data-table th { padding:var(--space-3) var(--space-4); text-align:left; font-weight:600; color:var(--color-text-secondary); border-bottom:2px solid var(--color-border); white-space:nowrap; overflow:hidden; text-overflow:ellipsis; }
.data-table td {padding:var(--space-3) var(--space-4); border-bottom:1px solid var(--color-border); color:var(--color-text-primary); overflow-wrap: break-word; word-wrap: break-word}
.data-table td.wrap-cell {white-space:normal; word-break:break-all; overflow-wrap: break-word; word-wrap: break-word}
.data-table tr:hover td {background:var(--color-surface-hover); overflow-wrap: break-word; word-wrap: break-word}
...
.row-overdue td {background: var(--color-danger-subtle); overflow-wrap: break-word; word-wrap: break-word}
@keyframes overduePulse { 0%, 100% { background: var(--color-danger-subtle); } 50% { background: rgba(239, 68, 68, 0.08); } }
.row-overdue td {animation: overduePulse 3s ease-in-out infinite; overflow-wrap: break-word; word-wrap: break-word}
.project-id { font-family:var(--font-mono); font-size:var(--font-size-sm); color:var(--color-accent); }
.stage-tag { display:inline-block; padding:var(--space-1) var(--space-2); border-radius:10px; font-size:12px; font-weight:500; }
...
.quote-tag { display:inline-block; padding:var(--space-1) var(--space-2); border-radius:10px; font-size:12px; }
...
.mass-tag { display:inline-block; padding:var(--space-1) var(--space-2); border-radius:10px; font-size:12px; }
...
.payment-tag { display:inline-block; padding:var(--space-1) var(--space-2); border-radius:10px; font-size:12px; }
...
.risk-level-tag { display:inline-block; padding:var(--space-1) var(--space-2); border-radius:10px; font-size:12px; font-weight:500; }
...
.risk-status-tag { display:inline-block; padding:var(--space-1) var(--space-2); border-radius:10px; font-size:12px; }
...
.overdue-badge { background:var(--color-danger); color:#fff; padding:var(--space-1) var(--space-2); border-radius:8px; font-size:12px; font-weight:600; animation: overdueBadgePulse 2s ease-in-out infinite; }
@keyframes overdueBadgePulse { 0%, 100% { box-shadow: 0 0 0 0 rgba(239, 68, 68, 0); } 50% { box-shadow: 0 0 8px rgba(239, 68, 68, 0.4); } }
...
.text-truncate { overflow-wrap: break-word; word-wrap: break-word; }
...
.stat-overview-item { display:flex; justify-content:space-between; padding:var(--space-2) 0; border-bottom:1px solid var(--color-border); }
.stat-overview-label { color:var(--color-text-secondary); font-size:var(--font-size-base); }
.stat-overview-value { font-weight:600; color:var(--color-text-primary); }
...
.rm-has { box-shadow:inset 0 0 0 2px var(--color-accent), 0 0 8px rgba(59, 130, 246, 0.15); }
...
.rm-col-label { text-align:center; font-size:var(--font-size-sm); color:var(--color-text-secondary); padding-top:var(--space-1); }
.list-item { padding:var(--space-3) var(--space-4); border-bottom:1px solid var(--color-border); cursor:pointer; transition:all 0.2s ease; animation: listSlideIn 0.3s ease-out both; }
...
.list-item-main { display:flex; align-items:center; gap:var(--space-2); flex-wrap:wrap; }
.list-item-title { font-weight:600; color:var(--color-text-primary); }
.list-item-meta { display:flex; gap:var(--space-4); margin-top:var(--space-1); font-size:var(--font-size-sm); color:var(--color-text-tertiary); }
.card-view-area { margin-bottom:var(--space-4); }
.card-view-header { display:flex; justify-content:space-between; align-items:center; margin-bottom:var(--space-3); }
.pt-card-grid { display:grid; grid-template-columns:repeat(auto-fill, minmax(280px, 1fr)); gap:var(--space-3); }
.pt-card { background:var(--color-surface); border:1px solid var(--color-border); border-radius:var(--radius-lg); padding:var(--space-4); cursor:pointer; transition:all 0.25s ease; animation: cardFadeIn 0.4s ease-out both; }
@keyframes cardFadeIn { from { opacity: 0; transform: translateY(10px); } to { opacity: 1; transform: translateY(0); } }
.pt-card:hover { border-color:var(--color-accent); box-shadow:var(--shadow-md); transform: translateY(-2px); }
.pt-card-overdue { border-left:3px solid var(--color-danger); }
.pt-card-header-row { display:flex; align-items:center; gap:var(--space-2); margin-bottom:var(--space-2); }
.pt-card-title { font-size:var(--font-size-md); font-weight:600; color:var(--color-text-primary); margin-bottom:var(--space-1); }
.pt-card-sub { font-size:var(--font-size-sm); color:var(--color-text-tertiary); margin-bottom:var(--space-2); }
.stage-progress-mini { display:flex; gap:var(--space-1); margin-bottom:var(--space-2); }
.spm-dot { width:8px; height:8px; border-radius:50%; background:var(--color-bg-tertiary); }
.spm-dot.active { background:var(--color-accent); opacity:0.5; }
.spm-dot.current { background:var(--color-accent); opacity:1; box-shadow:0 0 4px var(--color-accent); animation: dotPulse 2s ease-in-out infinite; }
@keyframes dotPulse { 0%, 100% { box-shadow: 0 0 4px var(--color-accent); } 50% { box-shadow: 0 0 8px var(--color-accent); } }
.pt-card-row { display:flex; justify-content:space-between; font-size:var(--font-size-sm); color:var(--color-text-secondary); padding:var(--space-1) 0; }
.pt-card-footer { display:flex; justify-content:space-between; margin-top:var(--space-2); padding-top:var(--space-2); border-top:1px solid var(--color-border); font-size:var(--font-size-sm); color:var(--color-text-tertiary); }
...
.cal-cell { min-height:80px; padding:var(--space-1); border:1px solid var(--color-border); border-top:none; transition: background 0.2s; }
.cal-cell:hover { background: var(--color-surface-hover); }
.cal-cell.today { background: var(--color-accent-subtle); }
.cal-date { font-size:var(--font-size-sm); color:var(--color-text-secondary); margin-bottom:var(--space-1); }
.cal-cell.today .cal-date { color: var(--color-accent); font-weight: 700; animation: todayPulse 2s ease-in-out infinite; }
@keyframes todayPulse { 0%, 100% { text-shadow: 0 0 0 transparent; } 50% { text-shadow: 0 0 6px var(--color-accent); } }
.cal-events { display:flex; flex-direction:column; gap:var(--space-1); }
.cal-event { font-size:12px; padding:var(--space-1) var(--space-1); border-radius:3px; cursor:pointer; white-space:nowrap; overflow:hidden; text-overflow:ellipsis; transition: transform 0.15s, box-shadow 0.15s; }
.cal-event:hover { transform: scale(1.02); box-shadow: 0 1px 4px rgba(0,0,0,0.1); }
...
.week-event { padding:var(--space-1) var(--space-2); border-radius:var(--radius-sm); margin-bottom:var(--space-1); cursor:pointer; transition: transform 0.15s; }
.week-event:hover { transform: translateX(2px); }
.week-event-title { font-size:var(--font-size-sm); font-weight:500; }
.week-event-meta { font-size:12px; opacity:0.7; }
.gantt-container { overflow-x:auto; }
.gantt-header { display:flex; border-bottom:2px solid var(--color-border); }
.gantt-label-col { min-width:180px; padding:var(--space-2); font-size:var(--font-size-sm); font-weight:600; color:var(--color-text-secondary); }
.gantt-bars-col { flex:1; }
.gantt-stages-header { display:flex; }
.gantt-stage-col { flex:1; padding:var(--space-2); text-align:center; font-size:var(--font-size-sm); color:var(--color-text-tertiary); border-left:1px solid var(--color-border); }
.gantt-row { display:flex; border-bottom:1px solid var(--color-border); cursor:pointer; transition:background 0.15s; }
...
.gantt-current { background:var(--color-accent-subtle); border:2px solid var(--color-accent); box-shadow: 0 0 8px rgba(59, 130, 246, 0.2); }
.gantt-future { background:transparent; }
.gantt-empty { text-align:center; color:var(--color-text-tertiary); padding:var(--space-8); }
.detail-panel-overlay { position:fixed; inset:0; background:rgba(0,0,0,0.5); z-index:100; display:flex; justify-content:flex-end; }
.detail-panel { width:480px; max-width:90vw; background:var(--color-bg-primary); border-left:1px solid var(--color-border); overflow-y:auto; display:flex; flex-direction:column; animation: slideInRight 0.25s ease-out; }
@keyframes slideInRight { from { transform: translateX(100%); opacity: 0; } to { transform: translateX(0); opacity: 1; } }
.detail-panel-header { display:flex; justify-content:space-between; align-items:center; padding:var(--space-4); border-bottom:1px solid var(--color-border); }
.detail-panel-title { font-size:var(--font-size-lg); font-weight:600; color:var(--color-text-primary); }
.detail-panel-body { padding:var(--space-4); flex:1; }
.stage-progress-full { display:flex; align-items:center; gap:0; margin-bottom:var(--space-4); }
.spf-item { display:flex; align-items:center; gap:var(--space-1); flex:1; }
.spf-dot { width:10px; height:10px; border-radius:50%; background:var(--color-bg-tertiary); flex-shrink:0; }
.spf-item.active .spf-dot { background:var(--color-accent); opacity:0.5; }
.spf-item.current .spf-dot { background:var(--color-accent); opacity:1; box-shadow:0 0 6px var(--color-accent); animation: dotPulse 2s ease-in-out infinite; }
.spf-label { font-size:var(--font-size-sm); color:var(--color-text-tertiary); }
.spf-item.active .spf-label { color:var(--color-text-secondary); }
.spf-item.current .spf-label { color:var(--color-accent); font-weight:600; }
...
.detail-label { font-size:var(--font-size-sm); color:var(--color-text-tertiary); }
.detail-value { font-size:var(--font-size-base); color:var(--color-text-primary); }
.detail-section { margin-top:var(--space-4); padding-top:var(--space-3); border-top:1px solid var(--color-border); }
.detail-section-title { font-size:var(--font-size-base); font-weight:600; color:var(--color-text-primary); margin-bottom:var(--space-2); }
.detail-pre { font-size:var(--font-size-base); color:var(--color-text-secondary); white-space:pre-wrap; background:var(--color-bg-secondary); padding:var(--space-3); border-radius:var(--radius-md); }
...
.modal-content { background:var(--color-bg-primary); border:1px solid var(--color-border); border-radius:var(--radius-xl); width:90%; max-width:640px; max-height:90vh; display:flex; flex-direction:column; }
.modal-wide { max-width:860px; }
.modal-header { display:flex; justify-content:space-between; align-items:center; padding:var(--space-4); border-bottom:1px solid var(--color-border); }
.modal-title { font-size:var(--font-size-lg); font-weight:600; color:var(--color-text-primary); }
.modal-close { background:none; border:none; color:var(--color-text-tertiary); font-size:1.2rem; cursor:pointer; padding:var(--space-1); }
.modal-close:hover { color:var(--color-text-primary); }
.modal-body { padding:var(--space-4); overflow-y:auto; flex:1; }
.modal-footer { display:flex; justify-content:flex-end; gap:var(--space-2); padding:var(--space-4); border-top:1px solid var(--color-border); }
...
.op-log-action { padding:var(--space-1) var(--space-2); border-radius:10px; font-size:12px; font-weight:500; white-space:nowrap; }
...
@media (max-width: 768px) {
  ...
  .view-switcher { margin-left:0; width:100%; overflow-x:auto; }
  .content-grid-2-1, .content-grid-3 { grid-template-columns:1fr; }
  .form-row-2, .form-row-3 { grid-template-columns:1fr; }
  .detail-grid { grid-template-columns:1fr; }
  .detail-panel { width:100%; }
  .tab-bar { overflow-x:auto; }
  .cal-grid, .week-grid { font-size:0.7rem; }
  .cal-cell { min-height:50px; }
  .gantt-label-col { min-width:100px; }
}
</style>