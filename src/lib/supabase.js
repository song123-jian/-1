...
async function init(url, anonKey, options = {}) {
  ...
  if (_client && _connected && _config.url === url && _config.anonKey === anonKey) {
  ...
  if (_initPromise && _config.url === url && _config.anonKey === anonKey) {
    const client = await _initPromise
    if (client && options.autoSubscribe && options.onRealtimeChange) {
      autoSubscribeTables(options.onRealtimeChange)
    }
    return client
  }
