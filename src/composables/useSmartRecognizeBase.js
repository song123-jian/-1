 ...
 */
export function useSmartRecognizeBase(form, parseFn, placeholder = '粘贴文本，系统将自动识别并提取关键字段...') {
  ...
  function buildInsights(result) {
    ...
    if (result.identifiedCount > 0) {
      insights.push(`已识别 ${result.identifiedCount} 项字段`)
    } else {
      insights.push('未识别到明确字段，建议补充更完整的文本')
    }

    if (result.lowConfCount > 0) {
      insights.push(`${result.lowConfCount} 项建议人工确认`)
    }

    if (Array.isArray(result.tableRows) && result.tableRows.length > 0) {
      insights.push(`识别到 ${result.tableRows.length} 行明细数据`)
    }

    insights.push('当前模式为本地规则识别，无需联网，适合免费使用')
    return insights
  }
  ...
  function runSmartRecognize() {
    ...
    smartRecResult.value = { ...result, insights: buildInsights(result), recognitionMode: 'local' }
  }
