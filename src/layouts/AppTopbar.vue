  ...
  <header class="app-topbar" :class="{ 'no-sidebar': !hasSidebar, 'sidebar-collapsed': collapsed }">
    ...
    <div class="topbar-right">
      ...
      <div class="topbar-user" @click="showUserMenu = !showUserMenu">
        <div class="user-avatar">{{ sessionStore.displayName?.charAt(0) || '?' }}</div>
        <div class="user-meta">
          <span class="user-role-name">{{ sessionStore.displayName }}</span>
          <span class="user-role-tag">{{ sessionStore.roleName }}</span>
        </div>
        <svg viewBox="0 0 16 16" fill="none" stroke="currentColor" stroke-width="1.5" class="user-chevron">
          <path d="M4 6L8 10L12 6" />
        </svg>
        <div v-if="showUserMenu" class="role-switch-dropdown">
          <div class="role-dropdown-header">账号设置</div>
          <button class="role-dropdown-item active" type="button">
            <span class="role-check">●</span>
            <span>{{ sessionStore.displayName }}</span>
          </button>
          <div class="role-dropdown-divider"></div>
          <button class="role-dropdown-item role-logout" type="button" @click.stop="handleLogout">
            <span class="role-check">↩</span>
            <span>退出登录</span>
          </button>
        </div>
...
function handleClickOutside(e) {
  ...
  if (!e.target.closest('.topbar-theme-switcher')) {
