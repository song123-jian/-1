<template>
  <div class="smart-recognize-panel" :class="{ expanded: showSmartRec }">
    ...
    <div v-if="showSmartRec" class="smart-recognize-body">
      <textarea :value="smartRecInput" @input="emit('update:smartRecInput', $event.target.value)" class="form-textarea" rows="3" :placeholder="placeholder"></textarea>
      <div class="sr-actions">
        ...
        <button class="btn btn-primary btn-sm" @click="emit('runSmartRecognize')"><Icon name="search" :size="14" /> 开始识别</button>
      </div>
      <div v-if="smartRecResult" class="sr-result-panel">
        ...
        <div v-if="smartRecResult.tableRows && smartRecResult.tableRows.length > 0" class="sr-table-section">
          <div class="sr-table-header">
            <Icon name="table" :size="14" /> 识别到 <strong>{{ smartRecResult.tableRows.length }}</strong> 行明细数据
              ...
              <tbody>
                <tr v-for="(row, ridx) in smartRecResult.tableRows" :key="ridx">
                  ...
                  <td v-for="h in smartRecResult.tableHeaders" :key="h.key">
                    <input v-if="h.type === 'number'" class="sr-table-input" type="number" step="any" :value="row[h.key]" @input="row[h.key] = parseFloat($event.target.value) || 0" />
                    <input v-else class="sr-table-input" :value="row[h.key]" @input="row[h.key] = $event.target.value" />
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <!-- 无结果提示 -->
        <div v-if="(!smartRecResult.items || smartRecResult.items.length === 0) && (!smartRecResult.tableRows || smartRecResult.tableRows.length === 0)" class="sr-empty-tip">未能识别出有效信息，请检查输入内容格式</div>
        <!-- 操作按钮 -->
        <div v-if="(smartRecResult.items && smartRecResult.items.length > 0) || (smartRecResult.tableRows && smartRecResult.tableRows.length > 0)" class="sr-result-actions">
          <button class="btn btn-ghost btn-sm" @click="emit('runSmartRecognize')">重新识别</button>
          <button class="btn btn-primary btn-sm" @click="emit('applySmartRecognize')"><Icon name="checkCircle" :size="14" /> 确认填入</button>
        </div>
      </div>
    </div>
  </div>
...
/* 表格区域 */
.sr-table-section { border-top: 1px solid var(--color-border); }
.sr-table-header { padding: var(--space-2) var(--space-3); background: rgba(59,130,246,0.06); font-size: var(--font-size-sm); display: flex; align-items: center; gap: var(--space-2); }
.sr-table-scroll { overflow-x: auto; max-height: 240px; overflow-y: auto; }
.sr-table { width: 100%; border-collapse: collapse; font-size: var(--font-size-xs); }
.sr-table th { background: var(--color-surface-elevated); padding: var(--space-1) var(--space-2); text-align: left; font-weight: 600; white-space: nowrap; border-bottom: 1px solid var(--color-border); position: sticky; top: 0; z-index: 1; }
.sr-table td { padding: var(--space-1) var(--space-2); border-bottom: 1px solid var(--color-border); }
.sr-table-rownum { width: 28px; text-align: center; color: var(--color-text-tertiary); font-size: 10px; }
.sr-table-input { width: 100%; min-width: 50px; padding: 2px var(--space-1); font-size: var(--font-size-xs); border: 1px solid transparent; border-radius: var(--radius-sm); background: transparent; color: var(--color-text-primary); }
.sr-table-input:focus { border-color: var(--color-primary); background: var(--color-bg-primary); outline: none; }
</style>
