...
export const useDeliveryStore = defineStore('delivery', () => {
  ...
  const pendingCount = computed(
    () => deliveries.value.filter((d) => d.status === 'created' || d.status === 'pending').length
  )
  ...
  const receivedCount = computed(
    () => deliveries.value.filter((d) => d.status === 'received' || d.status === 'accepted').length
  )
  ...
  const overdueCount = computed(() => {
    ...
    return deliveries.value.filter(
      (d) => d.expectedArrivalDate && d.expectedArrivalDate < now && d.status !== 'received' && d.status !== 'accepted'
    ).length
  })
  const totalAmount = computed(() => deliveries.value.reduce((sum, d) => sum + (parseFloat(d.totalAmount) || 0), 0))
  ...
  function runAssessment() {
    ...
    for (const d of all) {
      ...
      if (d.expectedArrivalDate && d.expectedArrivalDate < now && d.status !== 'received' && d.status !== 'accepted')
        overdueC++
  ...
  function initSeedData() {
    ...
    const twoDaysAgo = toLocalDateStr(new Date(now.getTime() - 86400000 * 2))
    const threeDaysAgo = toLocalDateStr(new Date(now.getTime() - 86400000 * 3))
    const nextWeek = toLocalDateStr(new Date(now.getTime() + 86400000 * 7))
    const nextThreeDays = toLocalDateStr(new Date(now.getTime() + 86400000 * 3))

    const seeds = [
      {
        id: 'dlv_seed_1',
        deliveryNo: 'GJXC-' + today.replace(/-/g, '') + '-0001',
        date: today,
        orderId: 'PO-2024-001',
        urgency: 'urgent',
        status: 'pending',
        customerName: '上海精密仪器有限公司',
        address: '上海市浦东新区张江高科技园区',
        contact: '王经理',
        phone: '021-55551234',
        expectedDate: nextThreeDays,
        expectedArrivalDate: nextWeek,
      ...
      {
        id: 'dlv_seed_2',
        deliveryNo: 'GJXC-' + yesterday.replace(/-/g, '') + '-0001',
        date: yesterday,
        orderId: 'PO-2024-003',
        urgency: 'high',
        status: 'shipped',
        ...
        phone: '010-88889999',
        expectedDate: today,
        expectedArrivalDate: nextThreeDays,
        transportMethod: 'dedicated',
        carrier: '冠久专车',
        driver: '赵师傅',
        ...
        grandTotal: 28815,
        hasException: '0',
        createdAt: yesterday + 'T10:30:00.000Z'
      },
      {
        id: 'dlv_seed_3',
        deliveryNo: 'GJXC-' + twoDaysAgo.replace(/-/g, '') + '-0001',
        date: twoDaysAgo,
        orderId: 'PO-2024-005',
        urgency: 'normal',
        status: 'transit',
        ...
        grandTotal: 12091,
        hasException: '0',
        createdAt: twoDaysAgo + 'T14:00:00.000Z'
      },
      {
        id: 'dlv_seed_4',
        deliveryNo: 'GJXC-' + threeDaysAgo.replace(/-/g, '') + '-0001',
        date: threeDaysAgo,
        orderId: 'PO-2024-002',
        urgency: 'normal',
        status: 'received',
        ...
        expectedArrivalDate: yesterday,
        transportMethod: 'logistics',
        carrier: '安能物流',
        actualDate: yesterday,
        acceptanceResult: 'passed',
        acceptPerson: '刘工',
        acceptDate: yesterday,
        items: [
          {
            seq: 1,
            productName: '碳钢板Q235',
        ...
        grandTotal: 7232,
        hasException: '0',
        createdAt: threeDaysAgo + 'T08:00:00.000Z'
      },
      {
        id: 'dlv_seed_5',
        deliveryNo: 'GJXC-' + threeDaysAgo.replace(/-/g, '') + '-0002',
        date: threeDaysAgo,
        orderId: '',
        urgency: 'urgent',
        status: 'exception',
        customerName: '杭州机械制造公司',
        address: '杭州市余杭区未来科技城',
        contact: '周工',
        phone: '0571-22223333',
        expectedDate: twoDaysAgo,
        ...
        exceptionReason: '运输途中货物发生碰撞导致部分产品变形',
        exceptionSolution: '已联系承运方理赔，安排补发',
        exceptionResponsible: '赵经理',
        _prevStatus: 'transit',
        items: [
          {
            seq: 1,
