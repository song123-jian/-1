...
function loadDeletedAutoIds() {
  try {
    ...
    console.warn('[todoStore] loadDeletedAutoIds失败:', e.message)
  }
  return new Set()
}
...
function persistDeletedAutoIds(ids) {
  try {
    ...
    console.warn('[todoStore] persistDeletedAutoIds失败:', e.message)
  }
}
    ...
    const seed = [
      {
        ...
        remark: '重要客户',
        subtasks: [
          { id: 's1', title: '准备续签方案', completed: true },
          { id: 's2', title: '联系客户确认', completed: false },
          { id: 's3', title: '法务审核', completed: false }
        ]
      },
      {
        id: 't2',
