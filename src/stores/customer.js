  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      dbRequest.onsuccess = (event) => {
        const db = event.target.result
        if (!db.objectStoreNames.contains('keyValue')) { resolve([]); return }
...
export const useCustomerStore = defineStore('customer', () => {
  ...
  const allRegions = computed(() => {
    ...
    customers.value.forEach(c => { if (c.region) regions.add(c.region) })
    return [...regions].sort()
  })

  const allDepartments = computed(() => {
    const depts = new Set()
    customers.value.forEach(c => { if (c.department) depts.add(c.department) })
    return [...depts].sort()
  })

  const levelStats = computed(() => {
    const stats = { A: 0, B: 0, C: 0 }
    customers.value.forEach(c => { if (stats[c.level] !== undefined) stats[c.level]++ })
    return stats
  })
  ...
  const totalCreditLimit = computed(() =>
    customers.value.reduce((sum, c) => sum + (c.creditLimit || 0), 0)
  )
  ...
  function deleteCustomer(id) {
    // 检查关联数据：报价、合同、送货、回款
    const useQuotationStore = () => require('./quotation').useQuotationStore()
    const useContractStore = () => require('./contract').useContractStore()
    const useDeliveryStore = () => require('./delivery').useDeliveryStore()
    const useCollectionStore = () => require('./collection').useCollectionStore()
  ...
  function importCustomers(data, options = {}) {
    ...
    data.forEach((item, idx) => {
      ...
      if (!fullName && !phone && !contactName) { skipped++; return }
      const dedupeKey = (fullName || '') + '|' + (phone || '')
      if (dedupeKey !== '|' && existingKeys.has(dedupeKey)) { skipped++; return }
  ...
  function initSeedData() {
    ...
    customers.value = [
      ...
      { id: 'c3', customerNo: 'KH-2023-0001', name: '广州进出口有限公司', fullName: '广州进出口有限公司', shortName: '广州进出口', contact: '王经理', contactName: '王经理', phone: '020-3888XXXX', email: 'wang@gz-ie.com', region: '华南', level: 'A', decisionAuthority: '决策者', coreConcerns: '价格优势', creditLimit: 800000, balance: 320000, address: '广州市天河区珠江新城', status: 'active', tags: ['VIP', '大客户'], createdAt: '2023-11-01' },
      { id: 'c4', customerNo: 'KH-2024-0003', name: '深圳智能制造有限公司', fullName: '深圳智能制造有限公司', shortName: '深圳智造', contact: '陈总', contactName: '陈总', phone: '0755-2666XXXX', email: 'chen@sz-smart.com', region: '华南', level: 'C', decisionAuthority: '推荐者', coreConcerns: '技术支持', creditLimit: 100000, balance: 45000, address: '深圳市南山区科技园', status: 'active', tags: ['新客户'], createdAt: '2024-06-10' },
      { id: 'c5', customerNo: 'KH-2024-0004', name: '成都精密机械有限公司', fullName: '成都精密机械有限公司', shortName: '成都精密', contact: '赵经理', contactName: '赵经理', phone: '028-8555XXXX', email: 'zhao@cd-jm.com', region: '西南', level: 'B', decisionAuthority: '影响者', coreConcerns: '售后服务', creditLimit: 200000, balance: 0, address: '成都市高新区天府大道', status: 'active', tags: [], createdAt: '2024-04-05' },
      { id: 'c6', customerNo: 'KH-2023-0002', name: '武汉钢铁贸易有限公司', fullName: '武汉钢铁贸易有限公司', shortName: '武汉钢贸', contact: '刘经理', contactName: '刘经理', phone: '027-8777XXXX', email: 'liu@wh-steel.com', region: '华中', level: 'C', decisionAuthority: '把关者', coreConcerns: '付款条件', creditLimit: 50000, balance: 32000, address: '武汉市江汉区建设大道', status: 'dormant', tags: ['回款慢'], createdAt: '2023-08-12' }
    ]
    persist(STORAGE_KEY, customers.value)
  ...
  return {
    customers, tags,
    activeCount, dormantCount, allRegions, allDepartments, levelStats,
