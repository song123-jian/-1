...
export const useProductionStore = defineStore('production', () => {
  ...
  const monthlyCompletedOrders = computed(() => {
    ...
    return productionOrders.value.filter(o =>
      o.status === 'completed' && o.actualEndDate && o.actualEndDate.startsWith(yearMonth)
    )
  })
    ...
    productionOrders.value = [
      {
        ...
        materialRequisitions: [
          { id: generateId('mr'), materialCode: 'MTL-002', materialName: '不锈钢板304', requiredQty: 750, issuedQty: 750, unit: 'kg' },
          { id: generateId('mr'), materialCode: 'MTL-003', materialName: '铝合金型材6063', requiredQty: 400, issuedQty: 400, unit: 'kg' },
          { id: generateId('mr'), materialCode: 'MTL-006', materialName: '轴承钢GCr15', requiredQty: 250, issuedQty: 200, unit: 'kg' }
        ],
        progress: 65,
        notes: '重点订单，优先排产',
      ...
      },
      {
        ...
        operator: '李师傅',
        materialRequisitions: [
          { id: generateId('mr'), materialCode: 'MTL-007', materialName: '碳钢Q235', requiredQty: 2500, issuedQty: 0, unit: 'kg' },
          { id: generateId('mr'), materialCode: 'MTL-008', materialName: '铜合金H59', requiredQty: 300, issuedQty: 0, unit: 'kg' },
          { id: generateId('mr'), materialCode: 'MTL-004', materialName: 'POM塑料', requiredQty: 150, issuedQty: 0, unit: 'kg' }
        ],
        progress: 0,
        notes: '',
      ...
      },
      {
        ...
        operator: '王师傅',
        materialRequisitions: [
          { id: generateId('mr'), materialCode: 'MTL-003', materialName: '铝合金型材6063', requiredQty: 500, issuedQty: 0, unit: 'kg' },
          { id: generateId('mr'), materialCode: 'MTL-007', materialName: '碳钢Q235', requiredQty: 200, issuedQty: 0, unit: 'kg' }
        ],
        progress: 0,
        notes: '客户加急订单',
      ...
      },
      {
        ...
        operator: '张师傅',
        materialRequisitions: [
          { id: generateId('mr'), materialCode: 'MTL-002', materialName: '不锈钢板304', requiredQty: 450, issuedQty: 450, unit: 'kg' },
          { id: generateId('mr'), materialCode: 'MTL-003', materialName: '铝合金型材6063', requiredQty: 240, issuedQty: 240, unit: 'kg' }
        ],
        progress: 100,
        notes: '按时完成',
  ...
  function addProductionOrder(data) {
    ...
    if (order.bomId) {
      try {
        const { useBomStore } = require('@/stores/bom')
        ...
        if (bom) {
          ...
          order.materialRequisitions = bom.components.map(comp => ({
            ...
            requiredQty: (parseFloat(comp.quantity) || 0) * (parseFloat(order.quantity) || 0) * (1 + (parseFloat(comp.scrapRate) || 0) / 100),
            issuedQty: 0,
            unit: comp.unit
          }))
        }
  ...
  function startProduction(id) {
    ...
    productionOrders.value[idx].actualStartDate = new Date().toISOString().split('T')[0]
  ...
  function completeProduction(id) {
    ...
    productionOrders.value[idx].actualEndDate = new Date().toISOString().split('T')[0]
    ...
    try {
      ...
      const inventoryStore = useInventoryStore()
      const order = productionOrders.value[idx]
      const existingProduct = inventoryStore.inventory.find(i => i.code === order.productName || i.name === order.productName)
      if (existingProduct) {
  ...
  function issueMaterials(id, materials) {
    ...
    try {
      ...
      for (const mat of materials) {
        ...
        if (reqIdx !== -1) {
          order.materialRequisitions[reqIdx].issuedQty = (parseFloat(order.materialRequisitions[reqIdx].issuedQty) || 0) + issueQty
        }
