...
const STORE_IMPORTS = {
  ...
  warehouse: () => useWarehouseLocationStore(),
  warehouseLocation: () => useWarehouseLocationStore(),
  cost: () => useCostStore(),
  todo: () => useTodoStore(),
...
export const useDataCenterStore = defineStore('dataCenter', () => {
  ...
  function applyStoreData(module, store, dataKey, dataArray) {
    const nextData = Array.isArray(dataArray) ? [...dataArray] : []

    if (module === 'inventory') {
      if (dataKey === 'inboundOrders' && typeof store.replaceInbound === 'function') {
        store.replaceInbound(nextData)
        return
      }
      if (dataKey === 'outboundOrders' && typeof store.replaceOutbound === 'function') {
        store.replaceOutbound(nextData)
        return
      }
    }

    if (typeof store.replaceData === 'function') {
      store.replaceData(nextData)
      return
    }

    store[dataKey] = nextData
  ...
  function batchUpdate(module, dataArray, options = {}) {
    ...
    /* 直接替换整个数据数组 */
    applyStoreData(module, store, dataKey, dataArray)
  ...
  async function _pullAndMerge(sbStore) {
    ...
    for (const [resource, data] of Object.entries(remoteData)) {
      ...
      applyStoreData(module, store, dataKey, [...data, ...localOnly])
    ...
    SupabaseClient.autoSubscribeTables({
      onInsert(tableName, record) {
        ...
        if (store && store[dataKey] && !store[dataKey].find((i) => i.id === record.id)) {
          applyStoreData(module, store, dataKey, [...store[dataKey], record])
        }
      },
      onUpdate(tableName, record) {
        ...
        if (store && store[dataKey]) {
          ...
          if (idx !== -1) {
            const nextData = [...store[dataKey]]
            nextData[idx] = record
            applyStoreData(module, store, dataKey, nextData)
          }
        }
      },
      onDelete(tableName, oldRecord) {
        ...
        if (store && store[dataKey]) {
          applyStoreData(
            ...
            store,
            dataKey,
            store[dataKey].filter((i) => i.id !== oldRecord.id)
          )
        }
      }
    })
