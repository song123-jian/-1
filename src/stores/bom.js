...
export const useBomStore = defineStore('bom', () => {
    ...
    bomList.value = [
      {
        ...
        type: 'multi',
        components: [
          { id: generateId('cmp'), materialCode: 'MTL-002', materialName: '不锈钢板304', spec: '2B/BA', unit: 'kg', quantity: 15, scrapRate: 3, isOptional: false, notes: '壳体材料' },
          { id: generateId('cmp'), materialCode: 'MTL-003', materialName: '铝合金型材6063', spec: 'T5', unit: 'kg', quantity: 8, scrapRate: 2, isOptional: false, notes: '端盖材料' },
          { id: generateId('cmp'), materialCode: 'MTL-006', materialName: '轴承钢GCr15', spec: 'Φ50', unit: 'kg', quantity: 5, scrapRate: 1, isOptional: false, notes: '轴承套圈' },
          { id: generateId('cmp'), materialCode: 'MTL-001', materialName: 'ABS树脂', spec: '通用级', unit: 'kg', quantity: 2, scrapRate: 5, isOptional: true, notes: '防护罩' }
        ],
        totalCost: 0,
        status: 'active',
      ...
      },
      {
        ...
        type: 'single',
        components: [
          { id: generateId('cmp'), materialCode: 'MTL-007', materialName: '碳钢Q235', spec: '热轧板卷', unit: 'kg', quantity: 25, scrapRate: 4, isOptional: false, notes: '缸筒' },
          { id: generateId('cmp'), materialCode: 'MTL-008', materialName: '铜合金H59', spec: 'Φ30', unit: 'kg', quantity: 3, scrapRate: 2, isOptional: false, notes: '导向套' },
          { id: generateId('cmp'), materialCode: 'MTL-004', materialName: 'POM塑料', spec: 'M90-44', unit: 'kg', quantity: 1.5, scrapRate: 3, isOptional: false, notes: '密封件支撑环' }
        ],
        totalCost: 0,
        status: 'active',
      ...
      },
      {
        ...
        type: 'multi',
        components: [
          { id: generateId('cmp'), materialCode: 'MTL-002', materialName: '不锈钢板304', spec: '2B/BA', unit: 'kg', quantity: 30, scrapRate: 5, isOptional: false, notes: '柜体' },
          { id: generateId('cmp'), materialCode: 'MTL-005', materialName: '尼龙66', spec: '自然色', unit: 'kg', quantity: 0.8, scrapRate: 2, isOptional: false, notes: '线槽' },
          { id: generateId('cmp'), materialCode: 'MTL-001', materialName: 'ABS树脂', spec: '通用级', unit: 'kg', quantity: 3, scrapRate: 5, isOptional: false, notes: '面板' }
        ],
        totalCost: 0,
        status: 'draft',
      ...
      },
      {
        ...
        type: 'phantom',
        components: [
          { id: generateId('cmp'), materialCode: 'MTL-003', materialName: '铝合金型材6063', spec: 'T5', unit: 'kg', quantity: 50, scrapRate: 3, isOptional: false, notes: '框架' },
          { id: generateId('cmp'), materialCode: 'MTL-007', materialName: '碳钢Q235', spec: '热轧板卷', unit: 'kg', quantity: 20, scrapRate: 4, isOptional: false, notes: '支撑结构' },
          { id: generateId('cmp'), materialCode: 'MTL-006', materialName: '轴承钢GCr15', spec: 'Φ50', unit: 'kg', quantity: 8, scrapRate: 1, isOptional: false, notes: '传动轴承' },
          { id: generateId('cmp'), materialCode: 'MTL-004', materialName: 'POM塑料', spec: 'M90-44', unit: 'kg', quantity: 5, scrapRate: 3, isOptional: false, notes: '导轨滑块' }
        ],
        totalCost: 0,
        status: 'active',
  ...
  function updateBom(id, data) {
    ...
    if (idx !== -1) {
      bomList.value[idx] = { ...bomList.value[idx], ...data, updateDate: new Date().toISOString().split('T')[0] }
      calculateCost(id)
      persist()
      return true
    }
  ...
  function deleteBom(id) {
    ...
    /* 检查是否有关联生产工单 - 需要动态导入避免循环依赖 */
    try {
      const useProductionStore = () => require('@/stores/production').useProductionStore()
  ...
  function activateBom(id) {
    ...
    bomList.value[idx].updateDate = new Date().toISOString().split('T')[0]
    persist()
    return true
  }
  ...
  function calculateCost(id) {
    ...
    let inventoryItems = []
    try {
      const useInventoryStore = () => require('@/stores/inventory').useInventoryStore()
