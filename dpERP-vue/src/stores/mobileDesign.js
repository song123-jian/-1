...
const pageTypeLabels = {
  dashboard: '仪表盘', list: '列表页', form: '表单页', detail: '详情页', chart: '图表页', custom: '自定义'
}

export const useMobileDesignStore = defineStore('mobileDesign', () => {
  const pages = ref(load(PAGES_KEY, []))
  const navItems = ref(load(NAV_KEY, []))
  const theme = ref(load(THEME_KEY, {
    primaryColor: '#4F46E5', fontSize: 14, borderRadius: 8,
    darkMode: false, fontFamily: 'system', compactMode: false
  }))
  ...
  function _persistPages() { persist(PAGES_KEY, pages.value) }
  function _persistNav() { persist(NAV_KEY, navItems.value) }
  function _persistTheme() { persist(THEME_KEY, theme.value) }
  ...
  function reorderNav(newOrder) {
    navItems.value = newOrder.map((id, i) => {
  ...
  function resetTheme() {
    theme.value = { primaryColor: '#4F46E5', fontSize: 14, borderRadius: 8, darkMode: false, fontFamily: 'system', compactMode: false }
    _persistTheme()
  }
  ...
  function initSeedData() {
    ...
    pages.value = [
      { id: 'mp1', name: '销售首页', type: 'dashboard', status: 'published', layout: 'single', components: ['销售概览', '快捷操作', '待办事项'], createdAt: '2024-01-15', updatedAt: '2024-06-20', publishedAt: '2024-01-20' },
      { id: 'mp2', name: '产品列表', type: 'list', status: 'published', layout: 'single', components: ['搜索栏', '筛选器', '产品卡片列表'], createdAt: '2024-02-10', updatedAt: '2024-05-15', publishedAt: '2024-02-15' },
      { id: 'mp3', name: '订单详情', type: 'detail', status: 'published', layout: 'single', components: ['订单信息', '物流跟踪', '操作按钮'], createdAt: '2024-03-01', updatedAt: '2024-06-10', publishedAt: '2024-03-05' },
      { id: 'mp4', name: '报表中心', type: 'chart', status: 'draft', layout: 'grid', components: ['销售趋势图', '客户分布图', '库存统计'], createdAt: '2024-04-20', updatedAt: '2024-06-25', publishedAt: '' },
      { id: 'mp5', name: '审批表单', type: 'form', status: 'draft', layout: 'single', components: ['表单字段', '附件上传', '审批流程'], createdAt: '2024-05-10', updatedAt: '2024-06-30', publishedAt: '' }
    ]
    _persistPages()
  ...
  return {
    pages, navItems, theme, pageTypeLabels,
