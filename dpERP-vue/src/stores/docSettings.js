...
const qualTypeLabels = {
  business_license: '营业执照', tax_cert: '税务登记证', iso_cert: 'ISO认证',
  import_export: '进出口许可', special_permit: '特种经营许可', other: '其他'
}
const qualStatusLabels = {
  active: '有效', expiring_soon: '即将到期', expired: '已过期',
  revoked: '已作废', pending_review: '待审核'
}
...
export const useDocSettingsStore = defineStore('docSettings', () => {
  ...
  const settings = ref(load(DS_SETTINGS_KEY, {
    licenseDays: 30, completeness: 80, autoFreeze: true,
    notifySys: true, notifyEmail: false, notifyWecom: false, remindFreq: 'weekly'
  }))
  ...
  const riskCount = computed(() => qualifications.value.filter(q => q.status === 'expired' || q.status === 'expiring_soon').length)

  function _persistQuals() { persist(QUAL_KEY, qualifications.value) }
  function _persistInvoices() { persist(INVOICE_KEY, invoiceProfiles.value) }
  function _persistSettings() { persist(DS_SETTINGS_KEY, settings.value) }
  function _persistLogs() { persist(DS_LOGS_KEY, logs.value) }
  ...
  function initSeedData() {
    ...
    const seeds = [
      { id: 'q1', name: '营业执照', type: 'business_license', certNo: '91310000MA1FL8XX3K', issuer: '上海市市场监督管理局', issueDate: '2020-06-15', expiryDate: '2030-06-14', status: 'active', warning: '' },
      { id: 'q2', name: 'ISO 9001质量管理体系认证', type: 'iso_cert', certNo: 'CNCA-2023-Q-12345', issuer: '中国质量认证中心', issueDate: '2023-01-10', expiryDate: '2026-01-09', status: 'active', warning: '' },
      { id: 'q3', name: '进出口经营权', type: 'import_export', certNo: 'SH-IMP-2021-0088', issuer: '上海市商务委员会', issueDate: '2021-03-20', expiryDate: '2025-03-19', status: 'expiring_soon', warning: '即将到期' },
      { id: 'q4', name: '税务登记证', type: 'tax_cert', certNo: 'SH-TAX-2020-12345', issuer: '国家税务总局上海市税务局', issueDate: '2020-06-15', expiryDate: '2024-06-14', status: 'expired', warning: '已过期' },
      { id: 'q5', name: '特种经营许可证', type: 'special_permit', certNo: 'SP-2022-0056', issuer: '上海市应急管理局', issueDate: '2022-08-01', expiryDate: '2027-07-31', status: 'active', warning: '' }
    ]
    ...
    if (invoiceProfiles.value.length === 0) {
      invoiceProfiles.value = [
        { id: 'inv1', companyName: '冠久科技有限公司', taxNo: '91310000MA1FL8XX3K', bank: '中国工商银行浦东支行', bankAccount: '1001 2088 0920 0123 456', address: '上海市浦东新区XXX路XXX号', phone: '021-6888XXXX', invoiceType: '增值税专用发票', isDefault: true },
        { id: 'inv2', companyName: '冠久科技（苏州）有限公司', taxNo: '91320500MA1WXXX9XN', bank: '中国建设银行苏州分行', bankAccount: '3220 1988 0360 5123 789', address: '苏州市工业园区XXX路XXX号', phone: '0512-6888XXXX', invoiceType: '增值税普通发票', isDefault: false }
      ]
      _persistInvoices()
    }
  ...
  return {
    qualifications, invoiceProfiles, settings, logs,
    qualTypeLabels, qualStatusLabels,
    qualTotal, qualActive, qualExpiring, qualExpired,
