  ...
  <div class="archive-page">
    <div class="page-header">
      ...
      <div class="page-header-actions">
        <button v-if="canManage" class="btn btn-primary" @click="openUploadModal"><Icon name="upload" :size="14" /> 上传文件</button>
        <button class="btn btn-ghost" @click="batchReview"><Icon name="list" :size="14" /> 批量审核</button>
        <button class="btn btn-ghost" @click="exportArchives"><Icon name="download" :size="14" /> 导出清单</button>
      </div>
    </div>
    ...
    <div class="tab-bar">
      ...
      ><Icon v-if="tab.icon" :name="tab.icon" :size="14" /> {{ tab.label }}</button>
    </div>
    ...
    <div v-if="activeTab === 'files'">
      ...
      <div class="archive-stats-bar">
        <span><span class="stats-dot" style="background:var(--color-accent)"></span> 总计 <strong>{{ archiveStore.totalCount }}</strong></span>
        <span><span class="stats-dot" style="background:var(--color-warning)"></span> 待审核 <strong>{{ archiveStore.pendingCount }}</strong></span>
        <span><span class="stats-dot" style="background:var(--color-success)"></span> 已审核 <strong>{{ archiveStore.approvedCount }}</strong></span>
        <span><span class="stats-dot" style="background:var(--color-purple, #8b5cf6)"></span> 已归档 <strong>{{ archiveStore.archivedCount }}</strong></span>
        <span><span class="stats-dot" style="background:var(--color-danger)"></span> 已过期 <strong>{{ expiredCount }}</strong></span>
      </div>
      ...
      <div v-if="selectedIds.length > 0" class="batch-action-bar">
        ...
        <button v-if="canManage" class="btn btn-ghost btn-sm" style="color:var(--color-danger)" @click="batchDelete">批量删除</button>
            ...
            <table class="data-table">
              <thead>
                <tr>
                  ...
                  <th @click="doSort('projectNo')" style="cursor:pointer">项目编号 <Icon name="filter" :size="14" /></th>
                  <th @click="doSort('fileName')" style="cursor:pointer">文件名称 <Icon name="filter" :size="14" /></th>
                  <th @click="doSort('category')" style="cursor:pointer">分类 <Icon name="filter" :size="14" /></th>
                  <th>子分类</th>
                  <th @click="doSort('version')" style="cursor:pointer">版本 <Icon name="filter" :size="14" /></th>
                  <th>格式</th>
                  <th>大小</th>
                  <th @click="doSort('status')" style="cursor:pointer">状态 <Icon name="filter" :size="14" /></th>
                  <th>密级</th>
                  <th>标签</th>
                  <th @click="doSort('validUntil')" style="cursor:pointer">有效期 <Icon name="filter" :size="14" /></th>
                  <th>上传人</th>
                  <th @click="doSort('createdAt')" style="cursor:pointer">上传时间 <Icon name="filter" :size="14" /></th>
                  <th>操作</th>
                </tr>
              </thead>
              <tbody>
                <tr v-if="paginatedArchives.length === 0">
                  <td colspan="15" class="empty-state">
                    <div class="empty-state-icon"><Icon name="folder" :size="14" /></div>暂无档案文件
                    <div style="font-size:var(--font-size-xs);color:var(--color-text-tertiary);margin-top:var(--space-1)">点击"上传文件"开始添加项目档案</div>
                  </td>
                </tr>
                <tr v-for="arc in paginatedArchives" :key="arc.id">
                  ...
                  <td>{{ arc.version }}</td>
                  <td><Icon :name="formatIconName(arc.format)" :size="14" /> {{ arc.format }}</td>
                  <td>{{ arc.size }}</td>
                  ...
                  <td>{{ arc.createdAt }}</td>
                  <td>
                    <button class="btn btn-ghost btn-sm" @click="previewArchive(arc)"><Icon name="eye" :size="14" /> 预览</button>
                    <button class="btn btn-ghost btn-sm" @click="viewVersions(arc)"><Icon name="list" :size="14" /> 版本</button>
                    <button v-if="canManage" class="btn btn-ghost btn-sm" @click="editArchive(arc)"><Icon name="edit" :size="14" /> 编辑</button>
                    <button v-if="arc.status === 'pending_review'" class="btn btn-ghost btn-sm" style="color:var(--color-success)" @click="approveArchive(arc.id)"><Icon name="checkCircle" :size="14" /> 审核</button>
                    <button v-if="arc.status === 'approved'" class="btn btn-ghost btn-sm" @click="archiveArchive(arc.id)"><Icon name="package" :size="14" /> 归档</button>
                    <button v-if="canManage" class="btn btn-ghost btn-sm" style="color:var(--color-danger)" @click="deleteArchive(arc.id)"><Icon name="close" :size="14" /> 删除</button>
                  </td>
                </tr>
              </tbody>
            </table>
    ...
    <div v-if="activeTab === 'stats'">
      <div class="stats-row" style="margin-bottom:var(--space-4)">
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-accent-subtle);color:var(--color-accent)"><Icon name="folder" :size="14" /></div><div class="stat-card-value">{{ archiveStore.totalCount }}</div><div class="stat-card-label">档案总数</div></div>
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-success-subtle);color:var(--color-success)"><Icon name="check" :size="14" /></div><div class="stat-card-value">{{ archiveStore.approvedCount }}</div><div class="stat-card-label">已审核</div></div>
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-warning-subtle);color:var(--color-warning)"><Icon name="clock" :size="14" /></div><div class="stat-card-value">{{ archiveStore.pendingCount }}</div><div class="stat-card-label">待审核</div></div>
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-danger-subtle);color:var(--color-danger)"><Icon name="package" :size="14" /></div><div class="stat-card-value">{{ archiveStore.archivedCount }}</div><div class="stat-card-label">已归档</div></div>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-4);margin-bottom:var(--space-4)">
        <div class="panel-card">
          <div class="panel-card-header"><span class="panel-card-title"><Icon name="trendUp" :size="14" /> 月度上传趋势</span></div>
          <div class="panel-card-body"><canvas ref="monthlyChartRef" style="width:100%;height:240px"></canvas></div>
        </div>
        <div class="panel-card">
          <div class="panel-card-header"><span class="panel-card-title"><Icon name="table" :size="14" /> 分类分布</span></div>
          <div class="panel-card-body"><canvas ref="categoryChartRef" style="width:100%;height:240px"></canvas></div>
        </div>
      </div>
      <div class="panel-card">
        <div class="panel-card-header"><span class="panel-card-title"><Icon name="list" :size="14" /> 审核效率统计</span><div><select class="form-select" v-model="reportPeriod" style="width:auto;font-size:var(--font-size-xs)"><option value="month">本月</option><option value="quarter">本季</option><option value="year">本年</option><option value="all">全部</option></select> <button class="btn btn-ghost btn-sm" @click="exportArchiveReport"><Icon name="download" :size="14" /> 导出报表</button></div></div>
        <div class="panel-card-body">
          <div style="display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:var(--space-3);margin-bottom:var(--space-3)">
            <div style="padding:var(--space-3);background:var(--color-surface-elevated);border-radius:var(--radius-sm);text-align:center">
              <div style="font-size:var(--font-size-2xl);font-weight:700;color:var(--color-accent)">{{ reportStats.periodUploads }}</div>
              <div style="font-size:var(--font-size-xs);color:var(--color-text-secondary)">本期上传</div>
            </div>
            <div style="padding:var(--space-3);background:var(--color-surface-elevated);border-radius:var(--radius-sm);text-align:center">
              <div style="font-size:var(--font-size-2xl);font-weight:700;color:var(--color-success)">{{ reportStats.approved }}</div>
              <div style="font-size:var(--font-size-xs);color:var(--color-text-secondary)">审核通过</div>
            </div>
            <div style="padding:var(--space-3);background:var(--color-surface-elevated);border-radius:var(--radius-sm);text-align:center">
              <div style="font-size:var(--font-size-2xl);font-weight:700;color:var(--color-warning)">{{ reportStats.avgReviewDays }}</div>
              <div style="font-size:var(--font-size-xs);color:var(--color-text-secondary)">平均审核天数</div>
            </div>
            <div style="padding:var(--space-3);background:var(--color-surface-elevated);border-radius:var(--radius-sm);text-align:center">
              <div style="font-size:var(--font-size-2xl);font-weight:700;color:var(--color-purple, #8b5cf6)">{{ reportStats.archived }}</div>
      ...
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:var(--space-4)">
        ...
        <div class="panel-card">
          <div class="panel-card-header"><span class="panel-card-title"><Icon name="trendUp" :size="14" /> 状态分布</span></div>
          <div class="panel-card-body">
    ...
    <div v-if="activeTab === 'download'">
      <div class="stats-row" style="margin-bottom:var(--space-4)">
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-info-subtle);color:var(--color-info)"><Icon name="download" :size="14" /></div><div class="stat-card-value">{{ archiveStore.totalDownloadCount }}</div><div class="stat-card-label">总下载次数</div></div>
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-success-subtle);color:var(--color-success)"><Icon name="calendar" :size="14" /></div><div class="stat-card-value">{{ archiveStore.todayDownloadCount }}</div><div class="stat-card-label">今日下载</div></div>
        <div class="stat-card"><div class="stat-card-icon" style="background:var(--color-warning-subtle);color:var(--color-warning)">[用户]</div><div class="stat-card-value">{{ archiveStore.downloadUserCount }}</div><div class="stat-card-label">下载用户数</div></div>
      </div>
      <div class="filter-bar" style="flex-wrap:wrap;gap:var(--space-2);margin-bottom:var(--space-3)">
        ...
        <button class="btn btn-ghost btn-sm" @click="exportDownloadLogs"><Icon name="download" :size="14" /> 导出日志</button>
      </div>
      <div class="panel-card">
    ...
    <div v-if="activeTab === 'share'">
      <div class="page-header" style="margin-bottom:var(--space-3)">
        <div></div>
        <div class="page-header-actions"><button class="btn btn-primary" @click="openShareModal"><Icon name="link" :size="14" /> 创建分享</button></div>
      </div>
      <div class="filter-bar" style="flex-wrap:wrap;gap:var(--space-2);margin-bottom:var(--space-3)">
            ...
            <table class="data-table">
              ...
              <tbody>
                ...
                <tr v-for="log in filteredShareLogs" :key="log.id">
                  <td>{{ log.shareTime }}</td><td>{{ log.sharer }}</td><td>{{ log.fileName }}</td><td>{{ log.validUntil }}</td><td>{{ log.password ? '[锁定] 是' : '否' }}</td><td>{{ log.accessCount }}</td>
    ...
    <div v-if="activeTab === 'archive'">
      <div class="content-grid content-grid-2-1">
        <div class="panel-card">
          <div class="panel-card-header"><span class="panel-card-title"><Icon name="setting" :size="14" /> 自动归档规则</span></div>
          <div class="panel-card-body">
            ...
            <div style="margin-bottom:var(--space-3)">
              ...
              <div style="font-size:var(--font-size-sm);color:var(--color-text-secondary);margin-top:var(--space-1)">
                <Icon name="check" :size="14" /> 归档文件自动设为只读，禁止修改<br><Icon name="check" :size="14" /> 保留查看和下载权限<br><Icon name="check" :size="14" /> 修改归档文件需管理员审批
              </div>
            </div>
            <div style="margin-bottom:var(--space-3)">
              <label class="form-label">档案目录自动生成</label>
              <div style="font-size:var(--font-size-sm);color:var(--color-text-secondary);margin-top:var(--space-1)">
                <Icon name="check" :size="14" /> 按项目结构和文件类型自动编排目录<br><Icon name="check" :size="14" /> 支持导出为PDF格式（含目录索引和文件元数据）
              </div>
            </div>
            <button class="btn btn-primary btn-sm" @click="saveRules"><Icon name="save" :size="14" /> 保存规则</button>
          </div>
        </div>
        <div>
          <div class="panel-card" style="margin-bottom:var(--space-3)">
            <div class="panel-card-header"><span class="panel-card-title"><Icon name="list" :size="14" /> 待归档项目</span></div>
          ...
          <div class="panel-card">
            <div class="panel-card-header"><span class="panel-card-title"><Icon name="package" :size="14" /> 批量归档</span><button class="btn btn-primary btn-sm" @click="batchArchiveProjects">批量归档选中项</button></div>
            <div class="panel-card-body" style="max-height:300px;overflow-y:auto">
    ...
    <div v-if="activeTab === 'security'">
      <div class="content-grid content-grid-2-1">
          ...
          <div class="panel-card" style="margin-bottom:var(--space-3)">
            ...
            <div class="panel-card-body">
              ...
              <div style="margin-top:var(--space-3);padding:var(--space-2);background:var(--color-warning-subtle);border-radius:var(--radius-sm);font-size:var(--font-size-sm)">
                <strong><Icon name="warning" :size="14" /> 水印系统</strong>：敏感文件预览时自动添加动态水印（用户名+时间+IP），防篡改防去除
              </div>
            </div>
          </div>
          <div class="panel-card">
            <div class="panel-card-header"><span class="panel-card-title"><Icon name="warning" :size="14" /> 异常操作预警</span></div>
            <div class="panel-card-body">
        ...
        <div>
          <div class="panel-card" style="margin-bottom:var(--space-3)">
            <div class="panel-card-header"><span class="panel-card-title"><Icon name="list" :size="14" /> 操作审计日志</span><button class="btn btn-ghost btn-sm" @click="exportAuditLogs"><Icon name="download" :size="14" /> 导出</button></div>
    ...
    <div v-if="activeTab === 'recycle'">
      <div class="page-header" style="margin-bottom:var(--space-3)">
        ...
        <div class="page-header-actions">
          <button v-if="canManage" class="btn btn-ghost" @click="restoreAll"><Icon name="delete" :size="14" /> 全部恢复</button>
          <button v-if="canManage" class="btn btn-danger" @click="emptyBin"><Icon name="delete" :size="14" /> 清空回收站</button>
        </div>
      </div>
      <div class="panel-card">
        <div class="panel-card-body no-padding">
          <div class="table-container">
            <table class="data-table">
              ...
              <tbody>
                <tr v-if="archiveStore.recycleBin.length === 0"><td colspan="6" class="empty-state"><Icon name="delete" :size="14" /> 回收站为空</td></tr>
                <tr v-for="item in archiveStore.recycleBin" :key="item.id">
                  ...
                  <td>
                    <button class="btn btn-ghost btn-sm" @click="restoreItem(item.id)"><Icon name="chevronLeft" :size="14" /> 恢复</button>
                    <button v-if="canManage" class="btn btn-ghost btn-sm" style="color:var(--color-danger)" @click="permanentDelete(item.id)"><Icon name="delete" :size="14" /> 永久删除</button>
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <div class="upload-drop-zone" @click="triggerFileInput" @dragover.prevent @drop.prevent="handleFileDrop">
              <div style="font-size:var(--font-size-2xl);color:var(--color-text-tertiary);margin-bottom:var(--space-2)"><Icon name="folder" :size="14" /></div>
              <div>点击选择文件或拖拽到此处</div>
              <div style="font-size:var(--font-size-xs);color:var(--color-text-tertiary);margin-top:var(--space-1)">支持 PDF/DOC/XLS/PPT/JPG/PNG/ZIP/RAR 等，单文件最大500MB</div>
            </div>
            ...
            <div v-if="pendingFiles.length > 0" style="margin-top:var(--space-2)">
              <div v-for="(f, i) in pendingFiles" :key="i" style="display:flex;align-items:center;gap:var(--space-2);padding:var(--space-1) 0;font-size:var(--font-size-sm)">
                <span><Icon name="file" :size="14" /></span>
                <span style="flex:1">{{ f.name }}</span>
                <span style="color:var(--color-text-tertiary)">{{ f.size }}</span>
                <button class="btn btn-ghost btn-sm" style="color:var(--color-danger);padding:0 4px" @click="pendingFiles.splice(i, 1)"><Icon name="close" :size="14" /></button>
              </div>
            </div>
          </div>
          ...
          <div class="form-row form-row-2">
            <div class="form-group"><label class="form-label">文件分类 <span class="required">*</span></label><select class="form-select" v-model="uploadForm.category"><option v-for="c in archiveStore.categoryOptions" :key="c" :value="c">{{ c }}</option></select></div>
          ...
          <div class="form-row form-row-2">
            ...
            <div class="form-group"><label class="form-label">版本号 <span class="required">*</span></label><input type="text" class="form-input" v-model="uploadForm.version" placeholder="V1.0"></div>
          </div>
          <div class="form-group"><label class="form-label">保密等级 <span class="required">*</span></label><select class="form-select" v-model="uploadForm.securityLevel"><option value="public">公开</option><option value="internal">内部</option><option value="confidential">机密</option><option value="secret">绝密</option></select></div>
          <div class="form-group"><label class="form-label">版本说明 <span class="required">*</span></label><textarea class="form-input" v-model="uploadForm.versionNote" rows="2" placeholder="描述本次版本修改内容和变更原因"></textarea></div>
          <div class="form-row form-row-2">
            <div class="form-group"><label class="form-label">标签（逗号分隔）</label><input type="text" class="form-input" v-model="uploadForm.tagsStr" placeholder="如：重要,机密,待审核"></div>
            <div class="form-group"><label class="form-label">有效期</label><input type="date" class="form-input" v-model="uploadForm.validUntil"></div>
          </div>
          <div class="form-group"><label class="form-label">备注</label><textarea class="form-input" v-model="uploadForm.remark" rows="2" placeholder="额外说明信息"></textarea></div>
        </div>
    ...
    <div v-if="showShareModal" class="modal-overlay" @click.self="showShareModal = false">
      <div class="modal-dialog" @click.stop>
        ...
        <div class="modal-body">
          <div class="form-group"><label class="form-label">选择文件 <span class="required">*</span></label><select class="form-select" v-model="shareForm.fileId"><option value="">请选择文件</option><option v-for="a in archiveStore.archives.filter(a => a.status !== 'deleted')" :key="a.id" :value="a.id">{{ a.fileName }} ({{ a.version }})</option></select></div>
          <div class="form-group"><label class="form-label">分享有效期</label><select class="form-select" v-model="shareForm.expiry"><option value="1h">1小时</option><option value="1d">1天</option><option value="7d">7天</option><option value="30d">30天</option><option value="permanent">永久</option></select></div>
          <div class="form-group"><label class="form-label">访问密码</label><input type="text" class="form-input" v-model="shareForm.password" placeholder="留空则无需密码"></div>
          <div class="form-group"><label class="form-label">发送至邮箱</label><input type="email" class="form-input" v-model="shareForm.email" placeholder="输入接收者邮箱"></div>
        </div>
        <div class="modal-footer">
...
const formatIconMap = {
  pdf: 'file', doc: 'file', xls: 'file', ppt: 'file',
  jpg: 'image', png: 'image', zip: 'package', rar: 'package',
  dwg: 'layout', other: 'attachment'
}
...
const uploadForm = reactive({
  projectNo: '', fileName: '', category: '技术文件', subCategory: '',
  version: 'V1.0', securityLevel: 'internal', validUntil: '',
  tagsStr: '', versionNote: '', remark: ''
})

const shareForm = reactive({
  fileId: '', expiry: '1d', password: '', email: ''
})
...
function previewArchive(arc) { alert('预览文件: ' + arc.fileName) }
function viewVersions(arc) { alert('版本历史: ' + arc.fileName) }
function editArchive(arc) { alert('编辑档案: ' + arc.fileName) }
function approveArchive(id) { archiveStore.batchReview([id], 'approve') }
function archiveArchive(id) { archiveStore.batchReview([id], 'archive') }
function batchArchive() {
  const ids = selectedIds.value.filter(id => {
    const arc = archiveStore.archives.find(a => a.id === id)
    return arc && arc.status === 'approved'
  })
  if (ids.length === 0) { alert('请选择已审核的档案'); return }
...
function clearSelection() { selectedIds.value = []; selectAll.value = false }
...
function submitUpload() {
  ...
  pendingFiles.value = []
  Object.assign(uploadForm, { projectNo: '', fileName: '', category: '技术文件', subCategory: '', version: 'V1.0', securityLevel: 'internal', validUntil: '', tagsStr: '', versionNote: '', remark: '' })
}

function openShareModal() { showShareModal.value = true }

function submitShare() {
  if (!shareForm.fileId) { alert('请选择文件'); return }
