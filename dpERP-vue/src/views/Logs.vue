...
function exportLogs(format) {
  ...
  if (format === 'csv') {
    ...
    const html = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>操作日志</title><style>body{font-family:sans-serif;padding:20px}table{width:100%;border-collapse:collapse;margin-top:10px}th,td{border:1px solid #ccc;padding:6px 8px;font-size:12px;text-align:left}th{background:#f0f0f0;font-weight:bold}.sensitive{color:red;font-weight:bold}h1{font-size:18px}</style></head><body>'
    ...
    for (const l of list) {
      const isSensitive = isSensitiveAction(l.action)
      body += '<tr><td>' + (l.time || '') + '</td><td>' + (l.user || '') + '</td><td>' + (l.action || '') + '</td><td>' + (logStore.moduleLabels[l.module] || l.module || '') + '</td><td>' + (l.detail || '') + '</td><td class="' + (isSensitive ? 'sensitive' : '') + '">' + (isSensitive ? '<Icon name="warning" :size="14" />' : '') + '</td></tr>'
    }
