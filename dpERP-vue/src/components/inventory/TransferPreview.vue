...
function handlePrint() {
  ...
  printWindow.document.write(`
    <html>
    <head>
      <title>调拨单 - ${props.order.orderNo}</title>
      <style>
        body { font-family: -apple-system, 'Microsoft YaHei', sans-serif; padding: 20px; color: #333; }
        table { width: 100%; border-collapse: collapse; }
        th, td { border: 1px solid #ccc; padding: 6px 10px; text-align: left; font-size: 13px; }
        th { background: #f5f5f5; font-weight: 600; }
        .print-info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 8px; font-size: 13px; }
        .print-info-label { color: #888; margin-right: 8px; }
        .print-signatures { display: flex; justify-content: space-around; margin-top: 40px; font-size: 13px; }
      </style>
    </head>
