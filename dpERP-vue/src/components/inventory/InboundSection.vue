    ...
    <div class="inbound-header">
                ...
                <table class="inv-table">
                  ...
                  <tbody>
                    <tr v-if="paginatedInbound.length === 0">
                      <td colspan="10" class="empty-cell">暂无入库记录</td>
                    </tr>
                ...
                <table class="inv-table">
                  ...
                  <tbody>
                    <tr v-if="paginatedInboundDetails.length === 0">
                      <td colspan="12" class="empty-cell">暂无入库明细</td>
                    </tr>
      ...
      <div v-else-if="inboundView === 'list'">
        <div class="panel-card">
          ...
          <div class="panel-card-body">
            <div v-if="filteredInboundOrders.length === 0" class="empty-cell">暂无入库记录</div>
            <div v-for="order in paginatedInbound" :key="order.id" class="inbound-list-item" @dblclick="handleDoubleClickInbound(order)">
      ...
      <div v-else-if="inboundView === 'card'">
        <div class="inbound-card-grid">
          <div v-if="filteredInboundOrders.length === 0" class="empty-cell">暂无入库记录</div>
          <div v-for="order in paginatedInbound" :key="order.id" class="inbound-card-item" @dblclick="handleDoubleClickInbound(order)">
...
.inv-stat-value { font-size: var(--font-size-xl); font-weight: 700; }
...
.cell-mono { font-family: 'Cascadia Code', 'Consolas', monospace; font-size: var(--font-size-xs); }
...
.inbound-list-item { padding: var(--space-3); border: 1px solid var(--color-border); border-radius: var(--radius-md); margin-bottom: var(--space-2); cursor: pointer; transition: box-shadow 0.15s; }
.inbound-list-item:hover { box-shadow: 0 2px 8px rgba(0,0,0,0.06); }
...
.inbound-card-item { border: 1px solid var(--color-border); border-radius: var(--radius-lg); padding: var(--space-3); transition: box-shadow 0.15s; cursor: pointer; }
.inbound-card-item:hover { box-shadow: 0 2px 8px rgba(0,0,0,0.06); }
